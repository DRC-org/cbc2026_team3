...
**両ハンドを 1 本のシーケンスで駆動する。** 機体ごとに独立した確認だと 2 つを
同時に起動でき、両機が同時に動いて可動域の重なる位置で干渉しうる。順序を 1 本に
固定すれば、いつ何が動くかがシーケンスの並びから読める。
...
class MotorCheckController:
    ...
    def payload(self) -> dict:
        ...
        progress / record / done / error のように種類を分けると、受け取る側は
        複数通を継ぎ合わせて 1 つの状態を組み立てることになり、途中の 1 通を
        取りこぼすと画面と機体が食い違ったまま復旧しない (再送も無いので、
        リロードするまで直らない)。
