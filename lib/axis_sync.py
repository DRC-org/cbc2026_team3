...
位置がずれたまま駆動すると押し合いになってその場で機構が壊れる。同じ
``sync_tolerance`` (位置定数 yaml の 1 つの値) を 3 層から参照して守る
(docs/invariants.md「左右ペア軸は 3 重に保護している」)。
...
ここへ依存する向き)。**ここから上位モジュールを import してはならない。**
"""
