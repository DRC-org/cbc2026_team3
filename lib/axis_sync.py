...
機構的に直結した左右ペアは 4 組 (``y_axis`` / ``rotate`` / ``sub_rotate`` /
``sub_pitch``) で、位置がずれたまま駆動すると押し合いになってその場で機構が壊れる。
偏差監視は同じ ``sync_tolerance`` (位置定数 yaml の 1 つの値) を 3 段から参照して
守るが、**3 段が揃うのは ``y_axis`` だけ**である (下の「効く軸」)。サーボの 2 組は
``sync_tolerance`` を意図的に持たないのでどの段にも載らず、ずれの検出は目視だけに
なる (理由は ``config/sub_hand_positions.yaml``)。

判定と換算をこのモジュールに閉じ込めるのは、段ごとに別実装を持つと片方だけを
直したときに気付けないため。特に逆回転ペアは ``scale`` の符号で向きを表すので、
逆換算を書き写した箇所が 1 つでも符号を落とすと「ずれていないのに止まる」か
「ずれているのに止まらない」のどちらかになる。

3 段は同じ判定の重複ではなく役割分担であり、debounce とラッチの有無が段ごとに
違うのは意図的である:
...
2. ``control.sync_guard.SyncGuard`` (``_check_deviation``。位置制御ループが毎周期呼ぶ)
   効く軸: ``y_axis`` のみ —— 両メンバが同じ位置制御ループに載る組しか登録されない
   (``main._attach_sync_groups``)。
...
class SyncGroup:
    ...
    def violation(self, positions: Mapping[str, float]) -> float | None:
        ...
        3 段すべてが「ずれているか」をこの 1 メソッドで決める。段ごとに違うのは
    ...
    def corrections(self, positions: Mapping[str, float]) -> dict[str, float]:
        ...
        偏差監視の 3 段 (``violation``) は「ずれたら止める」しかできない。位置制御は
