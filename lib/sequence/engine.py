...
class Sequence:
    ...
    async def move_to(
        ...
        *,
        ...
        # リミット保護が介入した軸は到達扱いにしない。保護は目標をその瞬間の実測位置へ
        # 書き換えるので到達判定は必ず成立し、放置すると位置定数とは別の場所で成功する
        # ステップができる。判定を介入カウンタの前後比較に置くのは、入口のクランプだけを
        # 見ると移動の途中で触れた場合を拾えず、終了時のラッチを見ると接点のバウンドを
        # 取りこぼすため
        table = self.positions
        # コルーチンは溜めない。到達待ちを溜めている途中の set_target_value が例外を
        # 投げると、既に作った wait_reached が await されないまま捨てられ、本当の死因の
        # 隣に RuntimeWarning が並ぶ
        ...
        for axis, position_name in targets.items():
            ...
            # 控えるのは指令の前。入口のクランプは set_target_value の中で起きるので、
            # 送った後に控えるとその 1 件が「元から数えられていた」ことになる
