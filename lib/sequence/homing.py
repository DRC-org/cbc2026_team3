...
なぜ要るかは docs/invariants.md「零点確定 (ホーミング) には止める仕組みが 3 つ要る」。
...
4. **1 歩ごとの再アンカー** — 指令は毎回**そのときの実測位置** + `step` で組む。
   機構が引っかかっても 1 step ぶんの偏差しか掛からない代わりに探索距離の上限へ
   永久に届かなくなるので、**停滞判定 (`_STALL_LIMIT`) が対になる**。その閾値は
   1 歩ぶんの追従を待つ側 (`_wait_step`) と共有する (`_progress_threshold`) ——
   基準が分かれると、待たずに抜けたことを「進まなかった」と数えてしまう
...
**探索の到達判定はラッチで見る。「今 ON か」では取りこぼす** (理由と実機事例は
docs/invariants.md「探索の到達判定はセンサのラッチで見る」)。**離脱は現在値のまま**
という非対称は意図したもので、理由は `HomingRunner._sensor_reached` にある。

**触れた状態から始めたら、一度離れてから寄せ直す。** ON 区間には幅があるので、
触れたその場を原点にすると「区間のどこで探索を始めたか」がそのまま原点のばらつきに
なる (症状は「原点合わせをしたのに位置がずれる」だけで、毎回違うので再現もしない)。
**離脱は探索と逆向き**なので、機構端で始まったときに押し込まない性質は保たれる。

**原点確定はグループ単位でしか行わない。** 左右直結ペアを別々の時刻に確定すると、
その間に片方が動いたぶんだけ消えないオフセットが残る。
