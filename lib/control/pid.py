...
class PIDController:
    ...
    ゲイン ``kp`` / ``ki`` / ``kd`` は生成時に決め、実行中に差し替える手段は持たない。
    ``ki`` の差し替えは積分器の作り直しを必ず伴う (そのままだと出力に乗る
    ``ki * integral`` が新旧ゲインの比の分だけ段差になって電流が跳ね、かといって
    クリアすると重力を支えていた保持電流ごと抜けて昇降軸が落ちる) ので、ゲインを
    変えるときは ``scripts/tune_y_axis.py`` のようにインスタンスごと作り直す。
    """
