...
``y_axis`` の**導入当時の**実測値 (``scale`` 55.0131 deg/mm, ``kp`` 32 counts/deg,
...
実運用ストローク (位置定数は home 0.0 〜 work_shared 650.0mm で、シーケンス 1 回の
移動は 130〜530mm) は最小の移動でもその 56 倍あり、原理的に行き過ぎる。
``kd`` を上げても直らない
(飽和中は合計がクランプされ D 項も出力に現れない)。

そこで PID には最終目標ではなく「速度・加速度で制限した中間目標」を毎周期与え、
追従誤差だけを見せる。速度は従属変数ではなく明示的なつまみになる。
...
行き過ぎる (``y_axis`` の ``sync_tolerance`` 10.0mm の 2.9%)。ここでは代わりに
