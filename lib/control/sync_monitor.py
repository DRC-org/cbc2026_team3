...
class SyncMonitor(PeriodicTask):
    ...
    EDULITE 05 は位置ループがドライバ内蔵で PC 側に常駐ループが無く、偏差検知の
    置き場所が無い。またシーケンス実行中以外 (動作確認中・待機中・手動操作中) にも
    機構がずれうるため、シーケンスから独立した常駐監視としてここに置く。

    ``y_axis`` は ``M3508PositionLoop`` 側の 200Hz 判定と二重になるが、これは意図的な
    多重防護である —— ループ側は「電流を即 0 にする」局所的な保護、こちらは
    「試合を止めて人間に知らせる」全体的な保護。誤発報の代償が「試合が止まる」ことな
    ので、この層だけ ``violation_samples`` による debounce を持つ
    (3 層の比較は lib/axis_sync.py のモジュール docstring)。
    """
