    ...
    **差は 3 つだけで、そこは各サブクラスに残す**:
    ...
    def clear_targets(self) -> None:
        ...
        **副作用として、その瞬間に ``move_to`` の到達待ちに入っている
        ``MotorHandle.wait_reached`` があれば ``WaitInterruptedError`` で中断させる**
        (中断された動作を「到達した」として扱わせないため)。
        """
    ...
    def clear_target(self, name: str) -> None:
        ...
        こちらは再励磁 (`RobotServer._reenergize_motors`) が使う —— フォルトが
        起きたのは 1 台だけで残りは動作中かもしれないので、全台へ広げると無関係な軸の
        `wait_reached` まで巻き込んで中断させる。
        """
        for handle in self._handles:
            if handle.name == name:
                handle.clear_target()
...
class GenericTargetRefresher(_TargetRefresherBase):
    ...
    安全側の挙動:
      ...
      - ``pause()`` 中は送らない。**アクチュエータ動作確認はこのタスクを
...
class QueryDrivenTargetRefresher(_TargetRefresherBase):
    ...
    1. **フィードバックが問い合わせ駆動である。** PC が黙ると 1 通も届かなくなり、
       ``feedback_timeout_ms`` (既定 500ms) を過ぎた時点でモータは ``STALE`` に
       なる。症状は「手動操縦すると動くのに常に赤い」だけ (実機で EDULITE 05 を
       励磁したまま 13 秒放置してフィードバック 0 通)
    2. DM3520 の TIMEOUT レジスタ (0x09) が有効な個体では、指令の途絶がそのまま
       励磁解除になる (自作モタドラのウォッチドッグと同じ扱いが要る)

    **目標をまだ持たないモータへも送る。** ここが ``GenericTargetRefresher`` との
    決定的な違い (あちらは「目標が無い = 送らない」で正しい —— 送れば起動直後に
    コンベアが回り出す)。送る中身は **``idle_target_value()`` をラッチした値**で、
    **毎周期そのときの実測角を書き直してはならない** (負荷で下がったぶんへ目標が
    追従し、誰も操作していないのに軸がクリープする)。

    **アクチュエータ動作確認中もこのタスクを止めてはならない**
    (`RobotServer._motor_check_pausables`)。

    緊急停止中も送り続ける (停止中だけ画面から機体の状態が消えるのを避ける。
    このフレームは機構を動かせない —— 励磁は enable でしか起きず、このタスクは
    enable を 1 通も送らない)。ただし**停止中はラッチを取らず毎回測り直す** ——
    惰走中にラッチすると解除後 1 周期目にその位置へ戻す動きが出る。
    """
