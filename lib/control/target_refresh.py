...
class GenericTargetRefresher(_TargetRefresherBase):
    ...
    安全側の挙動:
      ...
      - ``pause()`` 中は送らない (送信経路を別の主が握るときのための口)。
        **アクチュエータ動作確認はこのタスクを pause しない** —— 止めると
        `conveyor` とポンプが `settle_s` (0.5s) の目視・聴音確認の最中に
        ウォッチドッグで出力を落とし、サーボスロットは現在角で凍結して
        `reached` が永久に立たない (`RobotServer._motor_check_pausables`)
...
class QueryDrivenTargetRefresher(_TargetRefresherBase):
    ...
    **アクチュエータ動作確認中もこのタスクを止めてはならない。** `move_to` の
    到達待ち (``AxisHandle.wait_reached``) はドライバのキャッシュを polling する
    だけで自分では再送しないので、止めるとそのモータ宛へ飛ぶのは指令 1 通だけに
    なる。返るフィードバックも**動き出す前の位置 1 通**で以後は更新されず、
    実際に動ききっても到達判定を通らない (`RobotServer._motor_check_pausables`)。
