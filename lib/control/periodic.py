...
各クラスが自前で ``run`` / ``start`` / ``stop`` を書くと、二重 ``start()`` の扱い
(例外 / 無視)、``stop()`` の cancel の有無、停止要求のクリア場所が食い違う。同じ
パッケージの兄弟クラスで作法が違うと、片方の知識でもう片方を触ったときに事故る
(「止めたつもりが止まっていない」が最も危ない) ため、ライフサイクルはここに
一本化する。
...
サンプル列は持たず、最大値・超過回数だけを O(1) で積む。**tick の中で回る処理は
1 周期の仕事を定数時間に保たなければならない** —— サンプル数に比例する集計を
ここへ置くと、測っている当人が周期を伸ばす側になり、乱れの数字そのものが信用
できなくなる。
"""
