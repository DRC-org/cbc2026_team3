...
しかも WebSocket は開いたままなので UI は「接続中」を出し続ける (操縦者のノート PC が
スリープに入る・Wi-Fi が切れるだけで起きうる)。
...
集合と送信をここへ閉じ込めると、**経路を増やすには本クラスへメソッドを足すしかなく
なる**ので、この 4 つを守る場所が構造的に 1 つに保たれる。
**サーバー本体へ配信の生 API を戻してはならない。**
"""
