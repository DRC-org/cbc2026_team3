...
コマンド名が「許可フェーズ表」「フェーズ拒否文」「緊急停止拒否文」
「_handle_command の if-elif」へ分かれていると、どのゲート表にも載らないコマンドが
生まれ、「意図してゲート対象外にした」のか「単に書き忘れた」のかコードから読めなくなる。
...
class CommandSpec:
    ...
    #: (docs/invariants.md)。塞ぐのは「シーケンスの制御権を使う」コマンドだけで、
    ...
    #: 直せない。docs/invariants.md「制御権の奪い合いは両方向を塞ぐ」に対する意図的な例外で、
