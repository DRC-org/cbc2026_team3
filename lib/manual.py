...
シーケンスとまったく同じなので、緊急停止インターロック・M3508 の PC 側 PID への迂回・
自作モタドラの 20Hz 再送・左右ペアの 3 層保護はすべてそのまま効く。

手動が新たに背負う責務は 2 つだけである:

1. **可動範囲の宣言を強制する** (docs/invariants.md「手動で連続値を送ってよいのは
   `axes.<軸>.manual` を書いた軸だけ」)
2. **ジョグの起点を目標値で積む。** 毎回フィードバックから取ると、追従中に連打した
   ぶんが吸われて「押した回数だけ動かない」。ただし緊急停止を挟んだら捨てる ——
   停止中に自重で下がっていた場合、解除後の 1 回目が古い起点から飛ぶ。
"""
