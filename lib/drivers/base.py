...
class TelemetrySupport:
    ...
    ``MotorState`` の 4 値は制御経路が float であることに依存しているので、測る手段が
    無い項目もそこでは 0.0 のまま運ぶ。**その 0.0 をそのまま UI やヘルス判定へ流すと
    「測ったように見える 0」になる**ので、測定可否を制御値とは別のメタ情報として持ち、
    配信の境界で ``None`` へ倒すために使う (docs/invariants.md「測れない項目は配信の
    境界で `null` へ倒す」)。

    **4 値は必ずこの 1 組で運ぶ。** バラの bool 引数に分解すると、一部だけ配線した
    経路が作れてしまい、残りが黙って既定値のまま効く。判定をドライバ側に置くのは、
    UI にドライバ種別を書き写さないため。
    """
