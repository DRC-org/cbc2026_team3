...
class Dm3520Driver(MotorDriver):
    ...
    def __init__(
        ...
        *,
        ...
        # !!! この min は単位の違う 2 つを比べている。安全上の歯止めと読んではならない !!!
        # `limit_speed` (v_des) は**出力軸の rad/s** で、`v_max` は**フィードバックを
        # 復号する固定小数点レンジ**である (2026-09-09 の実測で単位が確定した。
        # config/sub_hand.yaml)。同じ次元の量ではないので、この比較には意味が無い。
        #
        # **現状は無害**: 実機の v_max は 200 で、出力軸の物理上限は無負荷 435rpm を
        # 減速比 19.2 で割った ≒2.4rad/s しかないため、この min が発火する
        # `limit_speed` は物理的に出せない領域にしかない (現行値は 2.0 / 1.0)。
        #
        # **害が出るのは 2 つ**: ①v_max を実機のレジスタに合わせて小さく書き直した
        # 構成で `limit_speed` がそれを超えると、速度上限が**黙って**下がる (症状は
        # 「なぜか遅い」だけ) ②この行を速度の歯止めと読むと、桁を打ち間違えた
        # `limit_speed: 20.0` が 20 < 200 で素通りすることに気付けない。
        #
        # **それでも据え置く。** 正しい歯止めは「出力軸の最高速度」という別の値を
        # config へ持ち込むことになるが、その値は実測ではなく減速比からの導出しか
        # 無く、低く置けば正当な指令を黙って削る。この min を外すだけの変更も、
        # 実機で確かめられない今は挙動を動かすだけで何も得ない。**触るなら実機で
        # 出力軸の最高速度を測ってから**、この min を専用の上限へ置き換えること。
    ...
    def initialization_steps(self) -> list[tuple[can.Message, float]]:
        ...
        p_max もフラッシュへ保存されず電源断で出荷値 12.5 へ戻るので、CTRL_MODE と
        同じように書き直せばよいと考えたが、**書き終わるまでの窓で復号レンジが
        食い違う**。ドライバが 12.5 で送ったフィードバックを config の 1000 で
        復号すると位置が 80 倍に読め、直後の ``activation_steps`` がその値を
        「現在角」として保持目標に書く —— **機構が 80 倍先へ走り、リミット
        スイッチを踏み越えて機構端まで行く** (2026-09-09 に実機で発生)。
        現在角を書いてから励磁するのは飛び出しを塞ぐための仕掛けなのに、
    ...
    def feedback_probe_message(self) -> can.Message | None:
        ...
        **この前提が成り立つのは、無励磁のモータへ送る場合だけである。**
        励磁中のモータへ送れば保持トルクをその場で失い、`sub_lift` は自重で落ちる。
        送ってよいかの判断は `CANManager._may_probe_for_feedback` が
        `is_energized()` の三値で 1 箇所だけ持つ (ここへ書き写さないこと)。
        """
