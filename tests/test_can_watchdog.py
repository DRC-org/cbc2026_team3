...
`ip` と `tc` を PATH のスタブへ差し替えて動かす。ウォッチドッグは `ip link set <if>
down` / `up` 以外の副作用を持たないので、**スタブが記録した `ip` の呼び出し列がそのまま
「何をしたか」**になる。判定はスクリプト側の本物のロジックを通る。

スタブにするのはカーネルとの境界 (`ip` / `tc` / `sudo`) だけ。
"""
