...
class TestLifecycle:
    ...
    async def test_実行中に届いた2通目の開始要求は停止後に発火しない(self):
        ...
        2 通目の START は現実に届く (操縦者 2 名 + 予備タブ / 配信周期 50ms 以内の
        二度押し / 詰まったクライアントが最大 1 秒古い `running:false` を描く)。それを
        再開イベントとして残すと、次の通常停止で `run()` が降りた瞬間に `run_forever`
        が拾い、操縦者が何も押していないのに機体が先頭から動き出す。
        """
        seq = SampleSequence()
        task = asyncio.create_task(seq.run_forever())
        seq.request_start()
...
class TestStepLog:
    ...
    **記録はエンジンの 1 箇所だけが持つ** (docs/invariants.md §5)。ここで固定するのは
    書式そのものではなく、「シーケンス名・番号・総数・ラベルの 4 つが揃うこと」と
