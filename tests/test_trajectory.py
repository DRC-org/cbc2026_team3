...
class TestNoOvershoot:
    ...
    導入当時の PID (output_limit 2000) は偏差 1.14mm で P 項が飽和し、減速に使える距離が
    移動距離に依らず一定になる。実運用ストローク (130〜530mm) では原理的に行き過ぎる
    ので、プロファイル側でこの性質を持たせる。
    """
...
class TestRetarget:
    ...
    def test_巡航中にすぐ手前へ再ターゲットしても目標を通り過ぎない(self) -> None:
        ...
        1 周期ぶんの進みより近くへ再ターゲットされると、その周期は着地代入で目標へ置かれる
        が**速度は巡航速度のまま残る**。次の周期は `remaining == 0` で着地条件が成立せず、
        素通しだと **着地したはずの目標を通り過ぎて進み続ける**。

        `TestNoOvershoot` は単発移動しか見ていないのでこの経路は網に無い。手動ジョグの
        連打と `sequence_jump` 後の `move_to` で踏む。
        """
        profile = _profile()
