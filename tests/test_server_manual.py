...
**制御権が同時に 2 つ立たないこと**だけを見る (半自動・動作確認・手動はどれも同じ
モータへ周期的に指令を出すので、2 つ走った時点で操作と動きが対応しなくなる)。
"""
...
class _SlowSequence(Sequence):
    ...
    通常停止 (``request_stop``) は走っているステップの完了まで待ってから降りるので、
    「止めた瞬間に is_running が False」を見ると実際には無い性質を見たことになる。
