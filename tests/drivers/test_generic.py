...
class TestSensorInput:
    ...
    def test_count_keeps_a_contact_that_is_already_over(self):
        # 零点確定はこれが無いと止まらない。探索は settle_s (50ms) ごとにしか観測
        # できないのに FEEDBACK は 100Hz で届くので、ON 区間が homing.step より狭い
        # 機構では指令 1 回で区間を通り抜け、観測時にはもう OFF になっている
        self._feed(sensor=True)
        self._feed()
    ...
    def test_count_survives_any_number_of_readers(self):
        # 読んでも減らない。リミットスイッチ保護と零点確定が同じセンサを見るので、
        # 先に読んだ側が相手のぶんまで消す形にしてはならない
        self._feed(sensor=True)
        self._feed()
