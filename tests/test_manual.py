...
class TestAxesInfo:
    ...
    def test_全軸が並びプリセット名と値を持つ(self) -> None:
        ...
        # 値は「そこがどこか」を UI に描かせるためだけに載る。指令は名前で送る
    ...
    def test_コート別の位置は現在のコートの値で載る(self) -> None:
        # 可動範囲バーの目盛りはコートで動く。載せる値が固定だと、赤コートの
        # 目盛りを見ながら青コートの機体を操ることになる
        manual, _, _ = _build()
    ...
    def test_値を引けない位置は空値で載せ配信を落とさない(self, monkeypatch) -> None:
        # ここは 20Hz の配信経路。1 つの位置が引けないだけで state 全体が飛ぶと、
        # 全クライアントのテレメトリが止まる (画面は「接続中」のまま凍る)。
        # **0 で埋めない** —— 可動範囲の下端に居ないプリセットが下端に描かれる
        manual, _, _ = _build()
