...
async def _spin_resident_loop(seq: GatedSequence, *, settle_s: float = 0.05) -> None:
    ...
    内部フラグを覗くと *フラグの名前* を固定するだけになる。常駐ループを実際に回し、
    要求が残っていれば必ず現れる「最初のステップの実行」を観測する。
    """
...
class TestEStopReleaseRequiresActiveEStop:
    ...
    停止していない試合中に 1 通届くだけで同期ずれラッチが全解除され、全モータへ再励磁が
    飛ぶ (リロード直後の UI やリトライで実際に届きうる)。
    """
...
class TestReleaseDoesNotBlockTheCommandLoop:
    ...
    `async for msg in ws` は 1 接続あたり完全に直列なので、解除ハンドラが再励磁を待つと
    次の 1 通が処理されない。応答の返らないモータは 1 台 0.5 秒待つため、CAN が落ちて
    いる状況 —— まさに緊急停止を押した状況 —— では **E-STOP の押し直しすら効かない**。
    """
...
class TestUnenergizedMotorsAreVisible:
    ...
    実機で起きた形: 物理緊急停止で DM3520 の電源が落ち、専用バスに 1 台しか居ない
    ので ACK が返らず送信が全滅する。解除しても再励磁は最初のモータの例外で打ち切られ、
    以降へ enable が 1 通も飛ばない。それでもフィードバックは復電後に正常に届いて
    `is_fault()` にも掛からないので、操縦者に見えるのは「指令しても動かない」だけで
    原因を示す表示がどこにも無い。
    """
...
class TestEStopStopsM3508:
    ...
    M3508 は ``emergency_stop_message()`` を持たず 0x7FF も解釈しない。位置制御ループが
    電流 0 を送り続けることに頼ると、そのタスクが死んだ瞬間に止める手段が無くなる。
    """
...
class TestEStopCompletionLogIsOneLine:
    ...
    台数ぶんの行が流れると**直前の `logger.exception` (どのバスへ送れなかったか) が
    押し流される** —— この行が言えるのは「全機ぶん流し終えた」の 1 事実だけである。

    同時に「試行」も落とせない。ループは送信失敗を握ったまま先へ進むので、
    **1 通も届いていなくてもこの行は出る。**「完了」と書くと緊急停止が実際に効いたと
    読めてしまう。
    """
...
class TestTargetRefresherLivenessBroadcast:
    ...
    再送が止まるとファームのウォッチドッグが 500ms で出力を落とすが、WS は繋がったまま
    モータ状態も届き続けるので画面は正常に見える (位置制御ループ・同期監視と同じ)。
    """
...
class TestEStopReasonIsRetained:
    ...
    `_broadcast_state` は停止中に毎ティック送り直すので、理由をサーバーが持たないと
    最初の 1 通だけが本当の原因を載せ、以降の再配信が表示を塗り替える。
    """
...
class TestBoardReportedEStop:
    ...
    自作 DC モタドラは物理停止スイッチの押下と CAN 初期化失敗をラッチへ落とす。拾わないと
    **機体は止まっているのに UI は平常のまま**になり、理由を画面から知る手段が無い。
    """
    ...
    async def test_release_is_not_undone_by_feedback_from_before_the_clear(self) -> None:
        ...
        解除は「解除フレーム送信 → 基板がラッチを外す → 次の FEEDBACK」の順に伝わる。
        送信前のフィードバックに残ったビットを信じると**二度と解除できない機体**になる。
        """
        fx, _ = self._fixture_with_generic(e_stop=True)
        await fx.publish_state()
        assert fx.e_stop_active is True
    ...
    async def test_再励磁の最中は基板の報告で止め直さない(self) -> None:
        ...
        再励磁は別タスクなので「解除済みだが基板はまだ停止中」という窓ができる。ここで
        緊急停止ビットを拾うと**二度と解除できない機体**になる (判定に使う
        `_board_e_stop_ignore_before` が確定するのも再励磁の後)。
        """
        fx, _ = self._fixture_with_generic(e_stop=True)
        gate = asyncio.Event()
...
def _fixture_with_boards(*, with_energized_motor: bool = True) -> tuple[ServerFixture, _Sent, dict]:
    ...
    **バスごとに数えると見たい不変条件が消える** —— 守りたい「全ロボットのラッチ解除が
    どのロボットの励磁よりも先に出ること」は、ロボットをまたいだ *順序* でしか表せない。

    各ロボットの 2 本目のバスには自作モタドラを 1 台も登録しない (実機の `can_m3508`)。
    **ブロードキャスト解除はそこへも出なければならない。**
    """
    fx = ServerFixture.build()
    sent: _Sent = []
...
class TestLatchClearIsNeverAborted:
    ...
    解除の途中で緊急停止が再発動すると、ロボットを順に処理する再励磁は 2 台目へ解除
    フレームを 1 通も送らない。ラッチの外れない基板は緊急停止ビットを報告し続け、
    それを拾ったサーバーが停止を再発動する —— 解除操作のたびに同じロボットだけが
    取り残され、**永久に復帰できない** (実機で発生)。
    """
...
class TestEStopClearIsBroadcast:
    ...
    停止はブロードキャスト `0x0FF` で全基板・全チャンネルをラッチさせるのに、解除が
    device_id 宛の個別送信だけだと **yaml に登録されたモータにしか届かない**。PC の
    管轄外 (ベンチ設定 / yaml に無い増設基板 / 片方だけ起動) は永久にラッチされ、
    基板の LED は 1 チャンネルでもラッチがあれば橙になるので全基板が橙のまま戻らない。
    """
