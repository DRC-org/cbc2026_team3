...
設計 8 項の全文は docs/invariants.md「アクチュエータ動作確認は両ハンド 1 本の
シーケンス」。このファイルが直接背負うのは次の 3 つ。

**数値をこのファイルに書かない。** 確認するのは「指令どおり動くか」なので、運用で
使う位置名へ動かせば足りる。試験用の値を作ると、その値だけが機構の変更に
取り残される (動作確認では動くのに運用では動かない、が成立する)。

**必ず初期姿勢で終わる。** 途中の姿勢のまま終わると、操縦者は試合開始前に手動で
戻すことになる。各ハンドの最後を初期姿勢への復帰にしてある。

**構成に無い軸のステップは登録から外す。ただし外したことを必ず画面とログに出す。**
判定は `Sequence.restrict_to_axes()` が 1 箇所で持ち、各ステップは
`@step(axes=...)` で自分が指令する軸を宣言する。
"""
