...
/**
 * 指令値であって実出力ではないことを、記号だけに頼らず言葉でも出す。幅 300px の診断
 * カラムに凡例を置く余地は無いのでホバー (title) に載せる。**「送った」と「出ている」の
 * 差はここでしか説明できない** —— ファームの `max_duty` クランプ・`everFed_` ゲート・
 * ウォッチドッグ満了・緊急停止ラッチのどれでも実出力は 0 になりうるが、この欄は変わらない。
 */
const COMMAND_TITLE =
...
/**
 * 数値 1 つ。**測れない項目に `0` を描いてはならない** —— 本当に 0 なのか測る手段が
 * 無いのかを操縦者が区別できなくなる。単位は値があるときだけ付ける (`—℃` は意味を
 * 持たない)。`MALFORMED` (欄の欠落・型違い) だけは異常側へ倒して `?` を出す ——
 * `—` と同じ記号にすると、配信の不具合が「測れない」として画面から消える。
 */
function Cell({
  value,
  unit,
...
/**
 * POS 欄。**位置を測れるモータでは実測値だけを出す** —— 実測値がある行に指令値を
 * 併記すると、この欄が実測なのか指令なのかを行ごとに読み分けさせる形になる。
 *
 * 位置を測れないモータ (DC 基板の duty・電磁弁基板の on_off) だけが、代わりに **PC が
 * 最後に送った指令値**を出す。**これは実出力ではない** (`MotorState.command` を参照)。
 * 取り違えを防ぐのが `→` と `title` の 2 つ。
 */
...
export function MotorStatus({
  ...
  return (
    <div className={cx("flex flex-col py-[0.15rem]", className)}>
      ...
      {/* ドライバからの補足。平常時は null なので 1px も占めない。色は付けない ——
          状態はチップが示す約束なので、着色テキストを重ねない */}
      {health?.detail ? (
