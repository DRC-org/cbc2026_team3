 ...
 *
 * **モータ一覧には混ぜない。** サーバーも `sensors:` を `motors:` と別セクションに
 * 持っており (モータ一覧に「常に 0 のモータ」を並べないため)、混ぜるとその判断が消える。
 *
 * ここが無いと `origin_sensor_react` の指差喚呼は `candump` を打たない限り確かめられない。
 * さらに**未配線・極性違いのセンサは STALE にすらならない** —— 基板は配線の有無に
 * 関わらず FEEDBACK を送り「接触なし」を報告し続けるので、触れてみる以外に検出手段が無い。
 *
 * 全体の健全性判定はここでは行わない (サーバーの `CANManager.health` がセンサも同じ鮮度で
 * ヘルスへ載せているので、途絶は既にその件数へ数えられている)。
 */
