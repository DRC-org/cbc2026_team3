 ...
 *
 * **モーダルにしてはならない。** `.modal` は全画面の fixed オーバーレイなので、
 * **駆動しているあいだずっとヘッダーの EMG STOP がクリックできなくなる**（クリックは
 * 背景として吸われ、パネルが閉じるだけ）うえ、`ModalProvider` の `openCount` で
 * ホットキーまで封じられる。全アクチュエータが順に動いている最中に止める手段だけが
 * 画面から消える形で、起動確認ダイアログの「実行中も緊急停止は即時優先で動作します」
 * という文面もそのあいだ嘘になる。ここは操作の隣で開くだけの面にして、画面の他の
 * どこも覆わないこと。
 *
 * 出すのはシーケンスのステップ一覧と、今どこを走っているか。判定はシーケンス
 * エンジンが担い、失敗はシーケンスが止まる形で現れる
