...
export interface ManualAxis {
  ...
  /**
   * 半自動シーケンス制御のままでも手動指令 (`manual_move`) を受け付ける軸か
   * (位置定数 yaml の `axes.<軸>.manual_always`)。正はサーバーだけが持つ。
   *
   * **受信境界では素通しのまま**にしてある (`parseManual` が見るのは `positions`
   * だけ)。軸名も可動範囲も UI へ書かない性質はそこで成立しており、この欄のために
   * `manual.axes` の検査を始めると、その性質を 1 欄のために崩すことになる。
   *
   * **代わりに読む側が `=== true` で厳密に見る。** 欄が落ちた配信 (古いサーバー /
   * 版ずれ) では対象軸が 1 本も出ず、押せるボタンが 1 つも増えないという安全側へ
   * 倒れる。**`?? true` のような向きの既定値を置いてはならない** —— シーケンス
   * 実行中に押せるボタンが、配信の欠落で勝手に増える。
   *
   * `MALFORMED` へ倒さないのは、欠落が**そのまま操作の可否として画面に出る**ため
   * (パネルごと消える)。`safety` の 1 欄が落ちて全画面が白くなる類とは別で、
   * `positions` の 1 要素を落とす扱いと同じ切り分けにある。
   */
