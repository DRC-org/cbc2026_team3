 ...
 *
 * 確認ダイアログの代わりで、目的は**カーソルを動かさずに確認を取ること**。詳細は
 * docs/invariants.md 「試合の開始・終了の確認は同じボタンの二度押しで取る」。
 *
 * 誤爆を防ぐのは 2 つの時間だけで、どちらも欠かせない:
 * - **不感時間** — 無いと物理的なダブルクリック 1 回がそのまま「二度押し」として成立する
 * - **自動解除** — 無いと、武装したまま忘れられたボタンが「次に触れた 1 回で試合が
 *   始まる」状態で画面に残り続ける
 *
 * 呼び出し側は、操作が成立しなくなった時点（切断・フェーズ遷移など）で `disarm()` を
 * 呼ぶこと。**武装は押した瞬間の状況に紐づく。**
 */
