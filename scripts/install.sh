...
# 機体を動かさない (down/up しかしない) ので、制御プログラムと違って enable してよい
WATCHDOG_SERVICE_NAME="cbc-can-watchdog.service"
WATCHDOG_SERVICE_PATH="/etc/systemd/system/${WATCHDOG_SERVICE_NAME}"
...
log "systemd unit を配置: ${WATCHDOG_SERVICE_PATH}"
sed "s|@PROJECT_DIR@|${PROJECT_DIR}|g" "${SCRIPT_DIR}/${WATCHDOG_SERVICE_NAME}" > "$WATCHDOG_SERVICE_PATH"
...
systemctl --no-pager --lines=5 status "$WATCHDOG_SERVICE_NAME" || true
