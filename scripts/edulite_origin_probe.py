...
確かめるために使う。使い方は --help を参照。

**サーバー (main.py) と同時に走らせてはならない。** 同じバスへ 2 つのプロセスが
問い合わせると、どちらの読みも相手の送ったフレームへの応答と混ざる。

既定で送るのは disable (フィードバック要求) と READ_PARAM だけで、どちらも
モータを励磁しない。SET_ZERO は --set-zero を明示したときにしか送らない。

**--set-zero は調査専用である。** 運用中の原点は PC 側のオフセットが持つので、
ここで生の座標系を付け替えると原点が二重定義になる (docs/invariants.md §3)。
"""
