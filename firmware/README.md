...
| **サーボ** SV0 – SV4 | `0x40` – `0x44` | `gripper` / `wall_f` / `wall_r` / `rotate_origin_sensor` / `origin_sensor` |
...
**役割は基板番号（DIP）ごとの表 `kSlotRoleByBoard` が持つ。** 全基板へ**同じファームを
焼く**ので、表が 1 つしか無いと基板 #0 の SV3 をスイッチへ変えた瞬間に基板 #1 の SV3 も
道連れになる（1 枚だけ別のファームを焼くのは §3.4 の焼き忘れ検出を無力化するので採らない）。
行を選ぶのは `setup()` の DIP 読み取り。

| 基板 | スロット | ピン | 役割 | デバイス ID | モータ / 用途 |
|---|---|---|---|---|---|
| #0 | SV0 | D4 | `Servo` | `0x40` | `gripper`（メインハンド） |
| #0 | SV1 | D5 | `Servo` | `0x41` | `wall_f`（メインハンド） |
| #0 | SV2 | D6 | `Servo` | `0x42` | `wall_r`（メインハンド） |
| #0 | SV3 | D7 | **`TouchSensor`** | `0x43` | `rotate_origin_sensor`（`rotate` の原点） |
| #0 | SV4 | D8 | **`TouchSensor`** | `0x44` | `origin_sensor`（`y_axis` の原点） |
| #1 | SV0 | D4 | `Servo` | `0x48` | `sub_gripper`（サブハンド） |
...
| `kSlotRoleByBoard[][]` | #0 = サーボ 3 + センサ 2 / #1 = サーボ 1 + 空き 4 | 実際に**どの基板に**何を何本繋ぐか（個数に制約は無い） | 役割がずれると、指令した先と違うものが動く。**行を間違えると隣の基板の役割になる** |
...
使用率を必ず見ること（現状は **Flash 54.2% / 16,660B、RAM 41.9% / 858B**。
...
| センサ入力 | 無し | **スロットに割り当て（現構成は基板 #0 の SV3 / D7 と SV4 / D8 の 2 個。個数自由）。1 個ずつ独立した CAN デバイスとして FEEDBACK のセンサ入力ビットで報告するだけ** | 仕様書 §5.2。判断は PC 側。接触は異常ではないのでヘルスにも動作確認にも影響させない |
