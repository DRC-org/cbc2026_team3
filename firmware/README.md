...
片方しか見ない嘘になる**。経緯は `../docs/history/incidents.md` の 2026-09-07 の項。
...
「電源を入れても何も起きない基板」ではなくなっている。**初期角も可動範囲もまだ仮値**
なので、この値のまま機構を付けてはならない（棚卸しは `docs/mechanism_handoff.md`）。
...
| `kServoBoards[][].sensorActiveLow` | 7 本とも **`false`**（スイッチは 2026-09-09 に全部 NC 接点へ交換。`INPUT_PULLUP` で非接触 = LOW・接触 = HIGH）。基板 #0 SV3 と基板 #1 SV1〜SV4 は実測確定、残る 2 本（#0 SV4 と #1 SV0 = `y_axis` 左右）は仮値 | センサの極性。**1 スロットずつ** `candump` で非接触・接触の両方の `FEEDBACK` を見て確定する（**確定した 1 つを他のスロットへ広げてはならない** —— 極性はスロットごとの配線でしか決まらない） | 逆だと「触れていないのに触れている」と報告し続け、原点合わせが即座に終わる |
