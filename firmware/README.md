...
| `kServoBoards[][].sensorActiveLow` | 基板 #0 SV3 のみ **`false` で実測確定**。残る 6 本（#0 SV4 と #1 の 5 本）は `true` の仮値 | センサの極性。**1 スロットずつ** `candump` で非接触・接触の両方の `FEEDBACK` を見て確定する（**確定した 1 つを他のスロットへ広げてはならない** —— 極性はスロットごとの配線でしか決まらない） | 逆だと「触れていないのに触れている」と報告し続け、原点合わせが即座に終わる |
