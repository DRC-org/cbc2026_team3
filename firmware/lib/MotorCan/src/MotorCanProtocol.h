...
// 電磁弁基板の bxCAN フィルタを `0x000 << 5` / `0x600 << 5` / `0x200 << 5` /
// `0x700 << 5` のリテラルで書くと、「上位 3bit が種別」「E_STOP=0b000 /
// SET_TARGET=0b001 / SET_PARAM=0b010」を CommandType と無関係にもう一度宣言する
// ことになる。**この enum は動きうる**（§2.1 の調停順の都合）ので、動かすと DC 用と
...
//
// 3 枚の main.cpp / app.cpp が各自でフラグを OR すると、規則が Arduino / HAL の
// 翻訳単位へ移って native テストが 1 件も届かなくなる —— DC 基板の
// buildStatusFlags() に `flags |= kReached;` を 1 行足しても全件緑になり、仕様書
// §9.3 の「断線したソレノイドも到達と報告される」が誰にも検査されなくなる。
//
