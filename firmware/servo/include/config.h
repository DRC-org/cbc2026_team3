...
// 実測して入れること。**狭すぎる分にはクランプで止まるだけだが、広すぎるとメカストッパに
// 当たったまま停動して焼損する。**
//
// !!! 現在この値は 0〜270deg = サーボの全可動域で、クランプは実質効いていない !!!
// かつては「positions が 0〜6deg の微小ストロークしか使わない」ことに合わせた
// 安全側の仮値 (0〜30deg) だったが、位置定数が実運用値
// (gripper 0/65・wall_f 270/180/90・wall_r 90/90/180) へ更新されたときに
// ここを全域へ開けた。**機構に当たる角度を送っても止まらない状態である。**
// 実可動域が決まったら必ず狭めること (棚卸しは docs/mechanism_handoff.md §0)。
//
...
constexpr ServoSlotConfig kSlotsByBoard[][kServoSlotCount] = {
    // 基板 #0（DIP=0）: メインハンド
    {
        ...
        {SlotRole::Servo, 5, 270.0f, kProvisionalLimits, kServoPulse270, false},  // SV1 wall_f
