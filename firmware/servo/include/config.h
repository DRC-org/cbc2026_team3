...
// （Nano 用バイナリを R4 へ焼くことは物理的にできないので、MCU 境界で 2 つに割っても
// 焼き間違いは起きない）。
//
...
//
// もう 1 つの役割は**ゼロ埋めの捕獲**。kServoBoards の書き忘れた要素は role が
// Unused（=0）で安全側へ倒れるが pin も 0 になるので、D0 を予約していないと
// 1 スロットぶんの書き忘れが衝突検査を素通りする。
...
// 通る配線」ができ、しかもその構成でしか症状が出ない。
// もう 1 つの役割は**ゼロ埋めの捕獲**（R4 側の同じ定数のコメントを参照）。
constexpr uint8_t kPinUartRx = 0;
constexpr uint8_t kPinUartTx = 1;
...
// **5 本の信号線（SV0〜SV4）はどれもサーボにもセンサにもなり、何個ずつでも動く**
// （センサだけの基板も可）。成立の根拠は **1 スロット = 1 CAN デバイス**にしてあること
// （docs/invariants.md「1 スロット = 1 CAN デバイス」）。
constexpr uint8_t kServoSlotCount = 5;

// **Unused を 0 にしてあるのは、書き忘れを安全側へ倒すため。** kServoBoards の足りない
// 要素はゼロ埋めされるので、先頭が Servo だと書き忘れたスロットが黙って「サーボとして
// 駆動するピン」になり、そこにスイッチが繋がっていれば通電したまま叩く。
//
// **サーボの型（270 / 180）をここへ足してはならない。** 型は pulse が表す ——
// Servo270 / Servo180 のような値を足すと isServoSlot() が「どちらかである」に変わり、
// 呼び出し側すべてが型を意識することになる。
...
struct ServoSlotConfig {
    ...
    // TouchSensor として使うとき、LOW を「入力あり」とみなすか。
    //
    // **極性はスロットごとの配線でしか決まらないので、1 スロットずつ実機で確かめる。**
    // 隣のスロットや別の基板の値へ合わせてはならない。逆に設定すると、零点確定
    // （lib/sequence/homing.py）の離脱段で**どこまで動かしても OFF にならず**、
    // HomingError で止まる（原因は配線不良と区別が付かない）。
...
// 焼き忘れ検出を無力化するので採らない）。
//
// **基板番号は行の添字ではなく boardNumber として明示的に持つ**（理由は冒頭「ピン配置」
// の節と同じ —— 3 枚ぶんを 1 つの表に並べると、各ビルドの静的検査が担当外の行を
// 実在しない衝突として落とす）。添字と基板番号の暗黙の結合を解けば、各バイナリが
// 自分の担当基板の行だけを持てる。
//
...
// 3 値セットで直す理由は kServoPulse270 のコメント）。
...
constexpr ServoBoardConfig kServoBoards[] = {
    ...
    // 赤の速い点滅になる。故障ではない。** 全スロットが Unused だと「名乗れるスロットが
    // 1 つも無い」ことになり BoardIndication が urgent に数える（判定は MotorLoopTimer.h）。
    // FEEDBACK も INFO も 1 通も送らず、どのコマンドも受け付けない。
    //
    // **LED からは「DIP を回しすぎて kServoBoards に行が無い」と区別が付かない。**
    // どちらも同じ赤の速い点滅で 0x50〜0x54 が 1 通も流れないので、**candump では
    // 切り分けられない。切り分けは DIP の値を目で確認すること。**
    // 役割を 1 つでも与えた時点で赤は消えるので、用途が決まったかの目印にもなる。
    //
...
// ウォッチドッグもスロットごとに独立して動く**（片方の壁だけ通信が切れる状況が実在する）。
// **サーボは満了しても現在角を保持する**ので機構は落ちないが、そこから先は動かせない。
//
// 0 にすると途絶しても指令を受け付け続け、FEEDBACK の bit2 も報告しなくなる。ベンチ確認
// のための逃げ道であって、試合では既定の 1 のまま使う —— 再送が間に合わない状態は運用上の
// 異常なので、ここや command_timeout_ms を触って覆い隠してはならない（仕様書 §8）。
//
// この値は setup() が MotorSafety::setWatchdogEnabled() へ写す。判定を #if で main.cpp
// 側に置くと同じ分岐を両ファームが各自で持つことになり、片方に入れ忘れても気付けない。
