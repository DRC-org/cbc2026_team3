...
**DM3520 のフィードバックは問い合わせ駆動。PC が黙ると 1 通も届かない。**
自分の CAN ID 宛のフレームを受けたときにしか状態を返さないため、`Dm3520TargetRefresher`
（20Hz）が送り続ける。**目標を持たないモータへも送る**のが `GenericTargetRefresher` との
