...
ドライバ種別を書き写さない）。**残っている穴は 2 つ**: `sub_y_axis` / `sub_lift`
（DM3520）はスイッチを付けても `HomingError` になる（`SET_ZERO` の安全な順序が
`disable` を要求し、`sub_lift` は disable すると自重で落ちるため宣言しない）。
`y_axis`（M3508）は手段はあるがスイッチが未装着で、ファーム・`sensors:`・`homing:` の
3 箇所を揃えて外してある。有効なのは `rotate`（EDULITE）だけ。**手段が無い軸は探索を
始める前に落ち、起動ログにも `ERROR` で出る。** 現況は `docs/checks_and_health.md` の
