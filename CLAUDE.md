...
- **`bus.recv` 自体の失敗でも降りない。待って呼び直し、そのあいだ `rx_down` を立てる。**
  ...
  まま受信を再開する。降りる実装だと、bus-off 復旧ウォッチドッグが down/up を出す
  1 秒ごとに受信タスクが永久に失われ、症状は「UI は接続中のまま全モータが STALE」に
  なる。**送信側（`PeriodicTask._run`）は tick ごとに例外を握って回り続けるので、
  受信側だけが片道だった。** ただし黙って回り続けてはならない —— 読めていない間は
  `BusHealth.DOWN` として必ず見えるようにする（`bus_off` とは原因も手当ても別なので
  相乗りさせない）。**復帰を確定してよいのは実際に 1 通読めたときだけ** ——
  python-can の socketcan は select がタイムアウトすると socket に触れずに `None` を
  返すので、down 中も `None` は返り続ける。これを復帰扱いにすると `rx_down` が
  数十 ms で勝手に外れ、画面は「読めていない」ことを一度も出さない
