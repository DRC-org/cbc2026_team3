...
`sub_y_axis`（DM3520。原点は前端スイッチ `0x49`）の 2 本。`y_axis`（M3508）は
**手段もスイッチもあるが有効化していない** —— ファームも `sensors:` も揃っていて、
`homing:` だけを外してある（極性と探索パラメータが未実測のため）。`sub_lift`（DM3520）は
**手段はあるが探索パラメータが未実測**なので `homing:` を書いていない。
**「スイッチが付いた」と「零点確定が有効になった」を混同しないこと** —— スイッチは
7 本繋がっているが、`homing:` から参照しているのは 2 本だけで、残りは `sensors:` に
登録して途絶を見ているだけである。
