...
中間目標（`lib/control/trajectory.py`）が要る。** `kp`=32 / `output_limit`=2000 /
`scale`=55.0131 では**偏差 1.14mm で P 項が飽和**し、飽和中の PID は制御ではなく
...
50mm/s = 2750deg/s では D 項だけで −2750counts になり、`output_limit` 2000 を超えて
...
**`y_axis` は 2026-09-03 に実機で詰めて `sync_kp: 8.0` が入っている**（移動中のずれ
最大が 0.275mm → 0.130mm）。この軸は左右で静止摩擦が大きく違い、遊びのせいで片方が
もう片方を連れ回さないので、`kp` / `ki` / `kd` と合わせて「両方を能動的に、揃えて
動かす」形にしてある。**検証したのは振幅 1.5mm までで、実運用ストロークでは未検証。**
