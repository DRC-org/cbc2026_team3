...
| `docs/web/` | **操縦 UI（`web/`）の現況の仕様。4 枚。** `screens.md`（画面・レイアウト・部品カタログ）/ `design.md`（配色・ラベル・アイコン・キーボード・確認の取り方・EMG STOP の配置）/ `data_flow.md`（WS 契約・受信境界・context 分割・判定の単一情報源）/ `pitfalls.md`（踏みやすい罠と、それを守っているテスト）。入口は `docs/web/README.md`。**UI を触るときはここから読む** |
...
| `config/<robot>_positions.yaml` | 論理軸の単位換算・機構位置の定数・手動操縦の可動範囲 (`manual`)・半自動のままでも手動で操作できる軸 (`manual_always`) |
...
**全員が位置制御中でない周期**と**左右が同じ軸位置を目標にしていない周期**の 3 つで、
判断はグループ単位（モータ単位にすると、ホーミングの押し当てで片方だけモードが
変わった瞬間にもう 1 台へだけ補正が乗る）。3 つ目は零点確定の整列段のためで、
**左右を意図的にずらす段では補正が整列を打ち消す**（実測で 1mm のずれに対し片側
−440counts・相方 +440counts）。判定は `SyncGroup.targets_share_axis_value()`
（許容幅 1e-6。丸め誤差だけを吸収する値で、`tolerance` を流用してはならない）で、
**状態から毎周期導出する** —— suspend/resume にすると再開の取りこぼしがその
グループの補正を試合中ずっと殺し、画面にもログにも出ない。**止めるのは補正だけで、
...
（`AxisSpec.to_commands_each()` がキーの過不足を `KeyError` で拒否するので載せ忘れも
塞がる）。モータ 1 台だけを指令する API は作らない。
...
- **ゲートも排他も全ロボットに掛かる。** どちらかが手動モード / シーケンス実行中なら起動を
  ...
  **周期タスクは 1 つも止めない**（理由は後述）。**排他は起動の可否だけを見るので、
  それだけでは既に走っている確認を守れない** —— 確認は `main_conveyor` と `sub_valves` を
  `manual_always` の 7 軸で駆動し、どちらも操縦者が目視・打音で確かめるステップである。
  守られていたのは「起動には全ロボットが sequence モードであることが要る → sequence
  モードでは手動が全部拒否される」という連鎖に乗っていたからで、**軸単位で緩めた瞬間に
  この連鎖が切れる**（弁の作動音を聴いている最中に別の弁を開けられる）。逆向きの塞ぎは
  手動側が持ち（`RobotServer._allow_manual_in_sequence` が実行中を拒否）、
  `_motor_check_environment_deny()` と対になる
...
機構を押し込んでから気付くのでは遅い）③`align_distance` の上限（下記の整列段を持つ軸のみ）
④緊急停止（`AxisHandle` が既に通る）。
...
`homing` が見るセンサ（`HomingSpec.sensor_names`。単数 `sensor` と複数 `sensors` を
吸収する唯一の口）が `sensors:` に登録されていることは `tests/test_robot_sequences.py` が
固定する（居ないと「センサが応答していません」で必ず失敗し、配線不良と区別が付かない）。

**左右にスイッチが 1 本ずつ付く軸（`y_axis`）は「両方が押された姿勢」を原点にする。**
`homing.sensors`（**モータ名 → センサ名**の対応表。単数の `sensor` とは排他）を書いた軸では、
探索で**いずれか 1 本**が当たった後に**整列段**が走り、まだ押されていない側のモータだけを
探索方向へ進める。片方が当たった瞬間の姿勢を原点にすると**機構の遊びぶんが原点のずれとして
焼き付き**、遊びの向きは前回の移動方向で変わるので再現しない。対応表のキーが軸の `motors:` と
過不足なく一致していることは起動時に強制する（1 つ書き忘れるとそのモータだけが静かに
対象外になる）。**既に当たった側は「検出時点の実測位置」で固定保持し、毎歩実測へ再アンカー
しない** —— 遊びが無い機構では進めた側が保持側を引きずり、再アンカーすると左右のずれが
増えないので**歯止めが 1 つも発火しないまま軸ごと機構端まで走る**（固定保持なら停滞判定が
拾う）。**`align_distance` は `search_distance` と同格の無人の歯止め**で、`sensors` を書いた
軸には必須・書いていない軸には書けない。**`sync_tolerance` より真に小さいことを起動時に
強制する** —— 届くと零点確定の最中に 3 層の保護が発報し、症状は「動作確認の最初のステップで
いつも緊急停止する」だけになる。`search_distance` を流用してはならない（`y_axis` のそれは
650mm で、片側を 650mm 進めるのは軸をねじり切ることに等しい）。
...
ドライバ種別を書き写さない）。**残っている穴は `sub_y_axis` / `sub_lift`（DM3520）だけで、
手段そのものが無い** —— スイッチを付けても `HomingError` になる（`SET_ZERO` の安全な順序が
`disable` を要求し、`sub_lift` は disable すると自重で落ちるため宣言しない）。
**有効なのは `rotate`（EDULITE）と `y_axis`（M3508）の 2 軸。** ただし `y_axis` は
**2026-09-08 に有効化しただけで実機ではまだ 1 度も走らせておらず**、`direction` /
左右 2 本の極性 / `align_distance` / `step` はどれも未実測である（初回は人が付くこと）。
**「スイッチが付いた」と「零点確定が有効になった」を混同しないこと** —— スイッチは
7 本繋がっているが、`homing:` から参照しているのは 3 本（`rotate` に 1 本、`y_axis` に
左右 1 本ずつ）で、残る 4 本は `sensors:` に登録して途絶を見ているだけである。
...
**軸単位の例外が 1 つだけある（`manual_always`）。モードが 2 つ立つわけではない。**
位置定数 yaml の `axes.<軸>.manual_always: true` を書いた軸は、`OperationMode.SEQUENCE`
のままでも手動指令を受け付ける（現在はメインハンドの `conveyor` とサブハンドの
`valve_1`〜`valve_6` の 7 軸）。コンベアを止めるだけ・弁を 1 個開けるだけのために
シーケンス全体の制御権を奪う（モード切替は走っているシーケンスを止める）のを避けるためで、
立っているモードは今までどおり 1 つ、その軸への指令だけがモードを跨いで通る。
**書けるのは到達判定を持たない単独軸（`duty` / `on_off`）だけ**で、
`AxisSpec._check_manual_always` が起動時に強制する。位置指令の軸で許すと、シーケンスが
`move_to` で書いた目標を手動が上書きし、`wait_reached` が動かない位置を見続けて必ず
`SequenceTimeoutError` になる（`blocked_during_reenergize` が塞いでいる害と同型）。
`duty` / `on_off` は `settle_s` の固定待ちへ落ちるのでこの害が無く、単独軸なので
「同一フレームで同時指令」の不変条件にも触れない。通るのは `manual_move`（位置名による
プリセット）だけで、対象軸に `manual:` は書けないので `manual_set` / `manual_jog` は
従来どおり拒否される。**緩めたのは `_manual_target` のモード判定 1 枚
（`RobotServer._allow_manual_in_sequence`）だけ** — 緊急停止ゲートも再励磁ゲートも
`CommandSpec` も `_apply_operation_mode` も無変更である。経緯は `docs/impl_plan.md` の
「シーケンス制御中でも手動で操作できる軸」。
...
`_manual_target` 側の判定（手動指令がシーケンスモード中に来た場合を拒否。上記
`manual_always` の 7 軸だけが例外）とは逆で、片方だけでは片方向しか守れない。
正は `CommandSpec.blocked_during_manual`（`lib/commands.py`）に置く —
`_cmd_sequence_start` に `if` を足す形にすると、
...
**`manual_always` は逆に、この塞ぎを片方向だけ「緩めた」ものである。** 緩めたのは
「シーケンス制御中に届く `manual_move`」の側だけで、**逆方向 —— シーケンスがその軸へ
書きに来ること —— は塞がない**。手動で止めたコンベアは、次にそれを回すステップが来れば
再び回る。上書きを許容する（除外もラッチも作らない）のは、シーケンスが手順の正であり
続けるため —— 塞ぐと「操縦者が触った軸だけシーケンスが飛ばす」経路ができ、どの手順が
外れているかは画面からもログからも読めない。**代わりに UI が「シーケンスに上書きされる」
ことを 1 行で断る**（断らないと「押したのに戻った」が故障に見える）。
...
**`manual_always` と取り違えないこと。** あちらは「半自動のままでも手動指令を通すか」の
宣言で、書けるのは逆に `duty` / `on_off` の軸だけ（前掲）。連続値の口は開かないので、
`manual_always` の軸に残るのは今までどおり位置名によるプリセット指令だけである。
...
同じ部品でも呼び出し元で既定を変える —— **開くのは操縦者の準備中だけ**（配線確認が
目的のフェーズ）。操縦者の試合中は畳み、手動中だけ開く。**Monitor は準備中も試合中も畳む**
—— かつて Monitor で展開していたが、警告 3 行 + CAN 3 行 + センサ 4 行で領域を使い切り、
開いているのにモータが 1 基も見えなかった。畳んだまま見逃す経路は無い（異常時は自分から
開く）。呼び分けの表は `docs/web/screens.md`。

**主操作は状態によって位置を動かさない。** `ActionPanel` は右の大きい面が常に
「今押すべきボタン」（START / NEXT / RUNNING / DONE）で、左は常に STOP。
状態で入れ替えると押す直前に毎回探し直すことになる。

**サーバーが構造的に拒否する面へボタンを出さない。** `manual_always` の軸を並べる
`AlwaysManualPanel`（常時操作）は**試合中・試合終了の半自動にだけ**出し、対象が 1 本も
無いロボットではパネルごと描かない。**準備中に出さない**のは、そのフェーズではシーケンスが
走っておらずモード切替で足りることと、**動作確認の起動導線が Monitor の準備面にしかない**
ため —— サーバーは動作確認の実行中この経路を拒否するので、出さなければ「打音・目視の最中に、
押せるが必ず拒否されるボタンが画面にある」状態を作らずに済む。**手動モード中にも出さない**
（`ManualPanel` が同じ軸を出すので同じ事実が 2 箇所に並ぶ）。塞ぐ理由は手動指令と同じ `manualBlockedReason` を
使い、新しい判定を作らない（作ると同じ指令の可否が画面の場所で食い違う）。配置と部品の
詳細は `docs/web/screens.md`、罠は `docs/web/pitfalls.md`。
...
**`state` の `manual` の `positions` だけは、素通しでも `MALFORMED` でもない第 3 の扱いを
する**（`parseManual`）—— 読める要素はそのまま通し、**どちらの形でもない要素だけを落とす**。
...
**この「落とす」は `?? []` と同じ穴を持つ。並べて読むこと。** 1 要素だけが読めない配信
（`[{name:"open",…}, {name:"closed", value:{}}]`）では `open` のボタンだけが並び、
`closed` が消えたことは画面のどこにも出ない —— 電磁弁・グリッパはプリセットが唯一の
指令口なので、操縦者には「この弁には開しか無い」ようにしか見えない。それでも落とすのは、
**名前を読めないボタンを押せる状態で出す方が重い**ため（押すと行き先の無い指令が飛ぶ）。
**`MALFORMED` へ倒さないのは、`positions` の欠落が画面を落とす形では現れないから**で、
`safety` のように 1 欄で全画面が白くなる欄と同じ扱いにはしない。**新しい欄を足すときに
この扱いを既定にしてはならない** —— 落としてよいのは「落ちたことが操作の可否として
画面に出る」欄だけである。
**`manual.axes[].manual_always` は素通しのまま、読む側が `=== true` で厳密に見る**
（`AlwaysManualPanel`）—— 欄が落ちた配信では対象が 0 本になってパネルごと消えるので、
**押せるボタンが配信の欠落で増える側へは倒れない**。`?? true` のような向きの既定値を
置いてはならない。`MALFORMED` へ倒さないのは `positions` と同じ切り分けである。
**逆向き（古い UI に新しい配信）は防げない** —— 古い `web/dist` を開いたままの
タブは `RouteErrorBoundary` で route ごと落ちる（`EMG STOP` は境界の外なので生き残り、
画面は再読み込みを案内する）。`scripts/deploy.sh` で更新したら**タブを再読み込みさせること。**
