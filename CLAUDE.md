...
持たないので、`FEEDBACK` の Byte4-6 と bit1 / bit2 は予約にしてあり、**組み立てる側も
...
**基板が報告する緊急停止（`FEEDBACK` bit3）はサーバー全体の緊急停止へ伝播する。**
...
**デバイス ID は「スロット表 + DIP ブロックオフセット」で決まる。刻み幅はスロット数。**
サーボ基板は 5 スロットで `0x01`〜`0x10` の帯、DC 基板は 3ch で `0x11`〜`0x1C` の帯。
刻み幅を「実際に使う台数」にすると、配線で役割を変えた瞬間にブロックの幅が縮んで
隣の基板と重なる。刻み幅 1 でも同じで、DIP を 1 段上げた基板のブロックが隣へ食い込む
（1 通の `SET_TARGET` で 2 台が動き、片方は永久に STALE）。**帯からはみ出したブロックは
未設定へ倒す** — 倒さないとサーボ基板の DIP を上げただけで DC 基板の帯を踏む。
...
bit5（ID 未設定）と bit6（センサ入力）だけで、**bit3 / bit4 は立てない**（駆動されない
...
触れさせるたびに機体が FAULT になりシーケンスが止まる。**センサは PC 側 yaml の
`motors:` にも登録する** — 登録しないと受信ループがフレームを誰にも配らない。
目標値を持たないので `SET_TARGET` は飛ばず、`motor_check.magnitude: 0` で動作確認から
外れるが、途絶は STALE として検出される（`config/checklist.yaml` の
`origin_sensor_react` で反応も目視確認する）。
