...
- `lib/` — 共通ライブラリ（両ロボットで共有）
  ...
  - `control/periodic.py` — 周期タスクの土台（`PeriodicTask` / `PausablePeriodicTask` / `LogThrottle`）。
    4 つの周期タスクはこれを継承し、起動・停止・周期の作法を共有する
  ...
  - `control/limit_guard.py` — リミットスイッチ保護（`LimitGuard` / `CombinedLimitGuard`）。
    50Hz 常駐だが**軸ローカルで方向つき**（触れた向きへの指令だけを止める）。判定 `clamp()` は
    指令の入口（`AxisHandle`）と共有する単一情報源
...
たびに数える（`GenericDriver.sensor_contact_count`。立ち上がり（OFF→ON）を数える
**単調増加のカウンタで、読んでも減らない** —— 零点確定とリミットスイッチ保護が同じ
センサを読むので、「読むと消える」ラッチだと先に読んだ側が相手のぶんまで消し、
上の壊れ方がそのまま再発する。診断ツリーが描く「今どうなっているか」は
`sensor_active` のまま）。読み手は自分で基準値を控えてその差だけを見る。
**探索を始める直前に基準値を取り直す** —— 手動操縦や前回の零点確定で数えたぶんまで
見ていると、1 歩目でいきなり到達と読む。
...
左右 1 本ずつ）で、残る 4 本（サブハンド）は零点確定には使わない。
**ただし「報告のみ」のスイッチはもう 1 本も無い** —— 7 本とも下記のリミットスイッチ
保護（`limits:`）から参照している。
**手段が無い軸は探索を始める前に落ち、起動ログにも `ERROR` で出る。**
現況は `docs/checks_and_health.md` の
「零点確定（ホーミング）」節が正で、直したときは両方を同時に更新すること。

**リミットスイッチはもう零点確定専用ではない。触れたらその軸をその向きへだけ止める。**
7 本のスイッチは全て `can_generic` のサーボ基板に載る一方、止めたいモータは全て別バス
（`y_axis`=can_m3508 / `rotate`=can_edulite / `sub_*`=can_dm3520）なので、**ファーム側には
結線できず PC 側の 50Hz 常駐（`lib/control/limit_guard.py` の `LimitGuard`）にしか
ならない。** 守る不変条件は 6 つ:

- **方向つき・軸ローカルであること。** 止めるのは「そのスイッチのある側へ進む指令」だけで、
  逆方向は必ず通す。無条件に止めると端に触れた軸を戻す操作ごと塞がれ、**二度と動かせない軸**
  ができる。**全体緊急停止へ倒すのも同じ理由で禁止** —— 緊急停止中は `manual_*` が通らないので
  端から退避する操作そのものが消える。`SyncMonitor` が全体停止でよいのは「ずれた軸はどちらへ
  動かしても壊れる」からで、リミットは片方向だけが破壊。性質が違う
- **止める = 目標値を実測位置へ引き戻すこと。電流 0 にも無励磁にもしない**
  （`clear_target` を使わない）—— `sub_lift` は保持ブレーキが無く、消磁するとワークごと
  自重で落ちる。**書き戻しは立ち上がりで 1 度だけ**で、毎周期測り直すと負荷で下がったぶんへ
  目標が追従して誰も操作していないのに軸がクリープする
- **判定は `LimitGuard.clamp()` 1 箇所。** 50Hz の常駐（層①）と指令の入口（`AxisHandle` へ
  差し込む層②）が同じ関数を呼ぶ。**層①だけだと押し戻されるまで 20ms 進み、層②だけだと
  走行中に触れても止まらない。** 判定を 2 箇所に書くと片方だけ符号を落としてももう片方が
  拾い、壊れた層が壊れたまま残る（`SyncGroup.violation()` を 3 層が共有するのと同じ理由）。
  禁止方向は**拒否ではなくクランプ**する（拒否だと端で操作そのものが効かなくなる）
- **零点確定の最中に外すのは `homing.sensor_names` の全センサだけ**（`suspend_sensors`）。
  **軸まるごとではない** —— 両端にスイッチのある軸で反対端の保護まで消える。逆に 1 本でも
  外し忘れると、`y_axis` の整列段は「既に押された側」の保護に引き戻されて必ず失敗する
- **保護が介入した移動は `move_to` を失敗させる。** 層①が目標を実測へ書き戻すと
  `is_reached` は必ず成立するので、放置すると軸は途中に居るのにシーケンスだけが先へ進む。
  `LimitGuard.intervention()` が「要求を曲げた回数」を単調に数え、`move_to` が前後で比べる
  （終了時のラッチを見る方式では、移動中に触れて離れた接点のバウンドを取りこぼす）
- **接触は読んでも消えない単調カウンタで受ける**（`GenericDriver.sensor_contact_count`）。
  consume 方式は読み手が 1 人に限られ、保護が読んだ瞬間に零点確定が接触を取りこぼす。
  ラッチ条件は「前回から増えた **または** 今 `sensor_active`」、解除は「今 OFF **かつ**
  この周期で増えていない」（前者だけでは触れっぱなしを、後者だけでは観測周期より狭い
  ON 区間を落とす）

**センサ途絶では発動させない。代わりに「効いていない」ことを配信する**
（`limit_blind_sensors`。`firmware_unconfirmed_motors` と同じ「壊れているのではなく検出が
働いていない」の報告なので FAULT へ倒さない）。途絶で止めると、スイッチ 1 本の不調で試合中に
軸が動かなくなる。**一度ラッチした後は途絶でも落とさない**（押し当てたまま保護が消えるほうが
危険）。止まっている軸は `limit_latched`（軸名 → センサ名）として配る —— 保護は軸ローカルで
他の軸は平常どおり動くので、配らないと操縦者には「その向きへ指令しても動かない」以外の
手掛かりが 1 つも無い。

**守れるのは低速接近と「原点がずれたまま走り込む」異常であって、全速での突入ではない。**
`y_axis` は検出まで最悪 35ms（センサ 100Hz + 監視 50Hz + 送信）で巡航 200mm/s なら 7.0mm、
減速に `v²/(2a)` = 200²/(2×1200) = 16.7mm、合わせて**約 24mm 行き過ぎる**。
**スイッチは機構端から 25mm 相当以上手前に置く前提**である。
`limits` の `direction` は、同じセンサを指す `homing.direction` と一致していなければならない
（食い違えば**その位置定数ファイルが丸ごと読めなくなる**。起動そのものは止まらない ——
`main._load_position_table_file` が読み込み失敗を「定数なしで起動」へ倒すため）。
**`homing` を持たないサブハンドの 4 本には突き合わせる相手が無い** ので、指差喚呼
`limit_direction_sub` が向きの取り違えを検出できる唯一の経路になる。
