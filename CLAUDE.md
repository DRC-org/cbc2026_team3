...
**`p_max` も `CTRL_MODE` もフラッシュへ保存されず電源断で出荷値へ戻るので、
`initialization_steps()` が起動のたびに config の値を書く**（レジスタへ焼いて済ませると、
書いたことが config から読めず、基板を差し替えた 1 台だけが古い値のまま残る）。
書き損なうと PC の復号レンジとドライバの送信レンジが食い違い、実機では 12.5 の
ドライバを 1000 で復号して **80 倍の位置**を読み、機構端まで押し込んでいるように見えた。
