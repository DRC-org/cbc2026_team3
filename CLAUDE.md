...
**1 歩ごとの追従待ち（`_wait_step`）の早期リターン閾値に `spec.tolerance`（到達許容差）を
使ってはならない。** `commanded = observed + direction*step` なので、機構が 1mm も
動いていなくても `|observed - commanded|` は常にちょうど `step` になる。
`tolerance >= step` の軸（本番の `y_axis`: `tolerance: 1.0` / `homing.step: 0.5`）では
「動いた」と「動いていない」を一切区別できず、初回の `settle_s` 1 回で必ず抜けてしまう
（追従待ちが `_FOLLOW_ATTEMPTS`（5 回）の設計から実質 1 回へ縮退し、上の停滞判定が
実移動を観測できる時間まで削られる）。閾値は必ず `step` より小さい値にする。
`step/2.0` にしたのは、停滞判定が「進んだ」を見る基準（`abs(observed-previous) >= step/2`）と
揃えるため —— 別の基準にすると、`_wait_step` では「進んだ」と判定されたのに停滞判定側では
「停滞」と判定される、といった食い違いが起きる。
