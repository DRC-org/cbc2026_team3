...
WS 接続先はヘッダーの接続表示（と切断バナー）から変更でき、`?ws=drc:8080` でも
一時上書きできる。
タブは URL パス（`/monitor` `/main-hand` `/sub-hand`）。旧ハッシュ形式の
...
- `lib/` — 共通ライブラリ（両ロボットで共有）
  ...
  - `tuning/` — ステップ応答の指標算出（`metrics.py` の純関数だけ）。**呼び出し元は実機
    チューニング CLI（`scripts/tune_y_axis.py`）ただ 1 つで、制御経路からは呼ばれない**
...
- `web/` — Vite + React + TypeScript + Tailwind v4 / daisyUI 5 の操作 UI
  ...
  - `src/components/` は**誰が描くか**で分ける。`shell/`（RootLayout が全画面へ出す外枠）/
    ...
    `ui/` の 6 つで、直下には何も置かない。barrel（`index.ts`）は作らず常に実ファイルまで指す（oxlint の
...
**これは「モータ 1 台の `can_id` に `0xFF` を書いてはならない」という話**で、
サーバーが解除時に全バスへ流す意図的なブロードキャスト解除
（`RobotServer._send_e_stop_clear_broadcast`。次項）とは別物である —— 前者は
...
**受信は 1 通ごとにエグゼキュータへ往復してはならない。** `run_in_executor(bus.recv)` は
スレッドの起床とイベントループへの復帰を伴い、実測 168us/通。C620 は 1 台 1kHz を流すので
M3508 2 台 + `can_generic` で 4200 通/秒になり、**受信だけで 1 コアが埋まる**（実測 83.5%）。
追いつかない分はカーネルがソケットバッファ溢れとして捨て、実機では `can_m3508` の
受信 369 万通に対し **77 万通（17%）が `rx_dropped`** に積まれていた。捨てられた窓は
M3508 の累積角に化ける（後述）。`fileno()` を持つバスは fd を `loop.add_reader` に載せて
起こしてもらい、起きたら滞留を `recv(0)` で出し切る（`_ReadableFd`。1 通 47.5us /
CPU 34.8%）。`fileno()` を持たないバス（`--dry-run` の virtual バス）だけが従来の
エグゼキュータ経由へ落ちる。**復帰待ちのあいだは `remove_reader` で監視を外す** ——
down した socket は「読める」と報告され続けるので、載せたまま待つとイベントループが
毎周期起こされ、素の `continue` と同じく位置制御ループの周期を巻き添えにする。
**取り込み中に失敗しても、既に引き取った分は捨てない**（カーネルのバッファから出した
フレームはもうどこにも残っておらず、捨てればその窓は永久に失われる）。
...
- **`_last_rx_at` はデコード成功後にのみ更新する。** 解釈できていないフレームを
  ...
  **`_rx_seq`（受信カウンタ）も同じ 1 箇所で一緒に進める** —— こちらは
  「待機開始より後に届いたか」の判定専用で、`_last_rx_at` は表示と経過時間のための
  壁時計に留める。判定を壁時計に乗せると、NTP が時刻を後ろへ補正しただけで
  **届き続けているのに新規と認められず** `_wait_fresh_feedback` がタイムアウトする
  （症状は「フィードバックを受信できないため有効化を見送りました」だけで、配線不良と
  区別が付かない）。片方だけ更新する経路を作らないこと
...
二重ループになって必ず干渉する。config の `pid: null` が「ドライバ側で制御していて
PC 側 PID を持たない」の単一の表現で、実際に位置制御ループへ載せるかどうかは
`main._build_position_loops` がドライバ種別（M3508 だけ）で決める。
...
**`p_max` は「動ける範囲」ではなく「読める範囲」である。** 位置速度モードの指令は生の
float なので `p_max` を通らず、16bit のフィードバックだけが `[-p_max, +p_max]` へ
線形マップされる。ストロークが `p_max` を超える軸は、指令どおり動いた先で位置が
読めなくなる（サブハンド前後は実測 500〜520mm = 533〜555rad なので、出荷値 12.5 では
±11.7mm しか読めなかった。2026-09-09 に 2 台とも 1000.0 へ書き換え。**機構側の想定
750mm とは食い違うので突き合わせること**）。
**3 つとも `CTRL_MODE` と同じくフラッシュへ保存されず電源断で出荷値へ戻るが、
起動時に書き直してはならない**（一度入れて `a167d7e` で revert 済み。理由は
`lib/drivers/dm3520.py` の docstring）—— 書き終わるまでの窓でレンジが食い違い、
12.5 で送られたフィードバックを 1000 で復号した **80 倍の位置**が
`activation_steps()` の保持目標にそのまま書かれて、機構がリミットスイッチを踏み越えて
端まで走った（2026-09-09 に実機で発生）。**正しい向きは「書いて直す」ではなく
「読み返して食い違いを検出し、励磁を拒否する」**（`Dm3520Driver.activation_block_reason`。
レジスタを直すのは人間の手順に残す）。守る形は 3 つ ——
①**3 レジスタとも読み返す**（`t_max` のずれは `guard.stall_torque` の実効値を同じ形で
ずらすので `p_max` と同型の事故。`v_max` は表示だけだが、現実の壊れ方は「電源断で
出荷値へ戻る」で 3 つ同時にずれるので重さで区別しない）
②**読めなかったときも通さない** —— 「未確認なら止めない」にすると**応答 1 通の
取りこぼしでゲートが丸ごと消える**。「1 通落ちただけで動かせない」は再試行で解く問題で、
`Dm3520Driver.configuration_probe_messages()`（まだ読めていないぶんだけを返す）を
`CANManager._confirm_configuration` が空になるまで送り直し、判断はその後で下す。
読み返しを `initialization_steps()` の静的な list へ並べないのは、あれが「応答が届いたら
やめる」を表現できないため
③**拒否した理由を画面へ出す** —— `Dm3520Driver.health_detail()` が
`MotorHealthInfo.detail` に「固定小数点レンジ 未確認 / 食い違い」を載せる
（`MotorHealth.WARNING`。**`is_fault()` へは入れない** —— ドライバの異常報告ではなく
PC 側の安全判断であり、励磁拒否は FAULT ではない）。文面は「応答が無い」（電源・配線）と
「食い違う」（config か実機のレジスタ）で分ける —— 手当てが逆になる。
④**再励磁のたびに読み返し直す** —— **物理非常停止（DC 基板の `REF`）は DM3520 の電源を
数秒落とす。** 復帰した個体はモードもレンジも出荷値なのに、起動時に読んだ値が残っていると
ゲートは「一致している」と答え、**80 倍の経路が緊急停止を踏むたびに復活する**。
`CANManager.activate_motors`（緊急停止解除と単発再励磁の唯一の入口）が励磁の前に
`reinitialization_steps()` を送り、`CTRL_MODE` を書き直して控えてあるレンジを捨てる
（`initialization_steps()` を丸ごと再送してはならない —— **`set_zero_on_start` の
`SET_ZERO` が載っているので、零点確定で合わせた原点をその場の姿勢へ書き換える**。
だから「起動時だけの手順」と「電源断で消える設定」を 2 つのメソッドに分け、後者を
前者から呼ぶ形にしてある）。**励磁中だと分かっているモータへは送らない** ——
先頭が `disable` なので、直結ペアの健全な相方へ届けば保持トルクをその場で失う。
判断は `CANManager._is_known_energized` が `is_energized()` の三値で 1 箇所だけ持ち、
鮮度確認の問い合わせ（`_may_probe_for_feedback`）も同じそれを読む。所要時間は
DM3520 1 台あたり +0.1 秒。**`reinitialize` の指定はサーバーではなく `activate_motors`
自身が持つ** —— 「電源断を跨いだか」を判断する材料（何が揮発するか）はドライバ側に
しか無いので、呼び出し側に選ばせない。**MIT モードのまま励磁した場合の症状は
`SequenceTimeoutError` か零点確定の停滞だけで、`is_energized()` も `is_fault()` も
掛からない**（機構の引っかかりと区別が付かない）。

**位置の単位はロータ角（減速前）で、マニュアルの記述と食い違う。実機が正。**
DM-S3519 のマニュアル §Note 2 は「位置は減速後の出力軸の rad」と書いているが、実測では
...
こちらはマニュアルどおり——同じフレームの中で位置と速度の単位系が違う。掛け忘れの症状は
「指令 113mm でラックが 6mm しか動かない」だけで、19.2 倍という比が見えるまで機械的な
滑りと区別が付かなかった。
...
**窓の長さはフレーム自身のタイムスタンプで測る。処理時刻（`time.monotonic()`）で
測ってはならない。** カーネルがフレームを捨てているとき、残りは滞留から詰めて処理
されるので**処理間隔は 1ms 程度にしか見えない** —— つまり両者が食い違うのは、
この判定が要る場面だけである。処理時刻で測っていた頃は、巡航 200mm/s（モータ軸
1834rpm）でわずか **16ms 分**が捨てられるだけで折り返しを逆向きに読み、
**累積角へ 360deg = 6.54mm が入りながら再アンカーの記録も残らなかった**（実機で
発生。症状は「何回か動かすと走行中に軸が荒れて同期ずれで緊急停止」だけ）。
時刻を持たないフレーム（`--dry-run` の virtual バス）と時刻が逆走した場合
（NTP 補正）だけ処理時刻へ落とす。経緯は `docs/impl_plan.md` の
「受信の取りこぼしと累積角への 360deg 注入」。
...
移動は 130〜530mm）はその外側なので**原理的に行き過ぎる**（最小の移動でも 56 倍）。
`kd` を上げても直らない（飽和中は P+I+D の合計がクランプされ、D 項も出力に現れない）。
下げる側も塞がっていて、当時の想定ストローク L=15mm でも行き過ぎない `kp` は 4.85 まで
落ちる一方、実測では `kp`=16 で左が 1mm も
...
その制動をちょうど打ち消せる。**どちらも実行中に変更できず UI にも配信されない** ——
`kd` を含め PID ゲインを実行中に差し替える経路は無いので、**config に書いた値がそのまま
動いている値である**。対が崩れるのは 2 つの yaml の片方だけを書き換えたときに限られ、
症状は「飽和率だけ上がって速くならない」だけ。突き合わせるときは
`config/<robot>_positions.yaml` の `motion.velocity_ff` と `config/<robot>.yaml` の
`pid.kd` を並べて読むこと（起動ログに出るのは `velocity_ff` の側だけである）。
...
（シーケンスの移動は 130〜530mm なので、短距離が出るのは手動ジョグの刻み 1〜100mm の側）。
机上ベンチ（`config/bench/`）は直結していない 2 台なので **0.0 のまま**にする ——
揃うべき前提が無いまま別々の負荷で回り続けるため（手順は
`docs/mechanism_handoff.md` §3-1）。

**ペア軸に片側だけ効く操作を作らない。** 原点確定（`set_origin_here`）も
PID ゲインの差し替え（`scripts/tune_y_axis.py` がプロセス内で行う上書き）も
グループ全員へ展開する。左右を別々の時刻に原点確定すると、その間に
...
**実周期の乱れも `PeriodicTask` が測る**（超過回数と最悪値だけを O(1) で積む）。
**集計 1 行を journal へ出すのは試合終了（`match_finish`）で、`match_start` は前縁リセットだけ**
—— `match_start` だけをリセット点にすると窓が「試合 + その後の準備時間」になり、しかも
その日の最後の試合が 1 行も残らない。**超過 0 件でも必ず 1 行出す** —— 出さないと、公称の
1.4 倍で常時走っている（＝停止距離も偏差監視の 40ms 予算も既に崩れている）機体で journal が
無音になり、「警告 0 件 = 乱れていない」と読まれる。**画面にも配信にも出していない** ——
しきい値が実機未検証で、常時点灯すると安全機構のパネルごと読まれなくなるため。先に実機で測る。
...
**停止と解除は対称でなければならない。** 停止（`_send_e_stop_frames`）はブロードキャスト
`0x0FF` を全バスへ流すので、**バス上の全基板・全チャンネル**がラッチする。解除が
`activation_steps()` の個別送信だけだと **yaml に登録されたモータにしか届かず**、PC の
管轄外（ベンチ設定で一部だけ動かす / 増設基板が yaml に無い / 片方のロボットだけ起動）は
永久にラッチされたまま残る。基板の LED は 1 チャンネルでもラッチがあれば橙になるので、
**全基板が橙のまま戻らず操縦者からは復帰不能に見える**（実機で発生）。そのため解除は
①全バスへブロードキャスト解除 →②管轄内モータへ個別のラッチ解除 →③励磁 の 3 段で、
①と②の重複は意図的である。**ラッチ解除は中断しない（中断すべきは励磁だけ）** ——
ロボットを順に処理するので、`should_abort` をラッチ解除にも効かせると 1 台目の最中に
停止が再発動しただけで 2 台目へ 1 通も飛ばず、外れないラッチが緊急停止ビットを報告し続けて
`_detect_board_e_stop` が停止を再発動する（解除操作のたびに同じロボットが取り残される）。
ラッチ解除で機体が動かないのは、停止時に目標値が捨てられていること（DC は duty 0 /
サーボは現在角保持 / 電磁弁は OFF）と、ファーム側の `everFed_` ゲート（`SET_TARGET` を
1 通も受けるまで出力しない）による。**`_board_e_stop_ignore_before` は①②を送り終えた後に
置く** — 先に置くと、まだ解除の届いていない基板の報告を「解除後の報告」と信じて自分で
止め直す。
...
順序を 1 本に固定すれば、いつ何が動くかがシーケンスの並びから読める。設計はこの 8 つ
（**項を足したらこの数も直すこと。** 食い違うと、読み手は全部を読んだかを数えられない）:
...
- **ゲートも排他も全ロボットに掛かる。** どちらかが手動モード / シーケンス実行中なら起動を
  拒否する。片方だけ見ていると、確認中にもう一方が手動で動かされて干渉する。
  **排他は起動の可否だけで、周期タスクは 1 つも止めない**（理由は後述）
...
- **構成に無い軸のステップは除外するが、除外したことを必ず画面とログに出す。**
  機構が未装着のハンドを外して実機を動かす構成（`config/bench/main_hand`）では
  サブハンドの軸が 1 本も無い。**黙って落とすと、本番構成で 1 軸が config から漏れて
  いてもそのステップごと消えて全ステップ PASSED になる**（症状は「動作確認は通ったのに
  試合でその軸だけ動かない」で、確認そのものが意味を失う）。必要な軸は
  `@step(axes={…})` としてステップの隣で宣言し、判定は `Sequence.restrict_to_axes()` の
  1 箇所だけが持つ（表を離れた場所に置くと、ステップが軸を増やしたときに判定だけが
  古いまま残る）。**軸を宣言しないステップ（零点確定）は構成に依らず必ず登録し**、
  一部だけ存在するステップは残して指令先だけを絞る（口は
  `MotorCheckSequence.move_to` ただ 1 つ。特に最後の `restore_home` をステップごと
  落とすと「必ず初期姿勢で終わる」性質が消える）。指令できる軸が 1 本も残らなければ
  従来どおり登録しない。除外は `excluded_steps` として**欠けている軸まで**同じ 1 通に
  載せ、UI は `MALFORMED` へ倒す経路を持つ（`?? []` で埋めると、除外が起きているのに
  画面は平常を描く）
- **周期タスクを 1 つも止めない（`RobotServer._motor_check_pausables` は空を返す）。**
  動作確認は `move_to` でしか軸を動かさず、その指令はシーケンスと同じ `MotorHandle` を
  通る。周期タスクはどれもその目標を実現する側なので、止めると仕事ごと消える。
  **M3508 の位置制御ループ**を止めると電流が 1 通も出ないまま到達待ちがタイムアウトし、
  復帰した瞬間に残った目標へ機体が走る（操縦者は失敗表示の直後に動く機体を見る）。
  **目標値再送**を止めると、問い合わせ駆動の EDULITE 05 / DM3520 は
  `move_to` の指令 1 通ぶんしか応答を返さず、`AxisHandle.wait_reached` は
  再送しない polling なので**動き出す前の位置を見続けて到達を観測できない**
  （`rotate` の 0→180deg は実際に回りきっても失敗する）。自作モタドラは
  `command_timeout_ms` 500ms で出力が切れるので、`settle_s` 0.5s の
  コンベア・ポンプは目視・聴音の最中に止まり、サーボスロットは現在角で凍結して
  `reached` が永久に立たない。かつて「C620 の `0x200` を奪い合う」として止めていたのは
  **モータ単位で直接 CAN を送っていた旧実装（`MotorCheckRunner`）の理由**で、
  `move_to` へ移った時点で失効している。再送が確認用の指令を上書きすることも無い
  （同じ `MotorHandle` の目標を書き直すだけで、ラッチ値が出るのは目標を 1 つも
  持たないときの「今の姿勢を保て」に限られる）。`pause()` の骨組みだけは残す
  （`safety.*.paused` が WS 契約に載っている）
...
機構を押し込んでから気付くのでは遅い）③**1 歩ごとの鮮度の再確認** — 事前確認だけでは
**探索を始めた後の途絶**に気付けない。センサの読み口は途絶しても最後に届いたフラグを
返し続けるので、探索中にサーボ基板が再起動したり `can_generic` が瞬断したりしても
症状は「いつまでも当たらない」だけになり、止めるのは①か停滞判定だけになる
（ドライバ内蔵の位置ループが端を押し続けている状態で、さらに 3 歩ぶん押し込む）。
**降りる前にその場の実測位置を目標へ送り直す** — 最後に送った「実測 + step」が
生きたままだと押し続けるため（接触を検出したときと同じ作法。判定は探索・離脱の
どちらも `_seek` の 1 本を通る）④緊急停止（`AxisHandle` が既に通る）。
**失敗したら原点を確定せずシーケンスを止める** — 原点がずれたまま走るより、
動かないまま止まって操縦者に知らせるほうが安全。**既にセンサに触れているなら、
一度区間の外まで離れてから寄せ直す**（触れたその場を原点にすると「ON 区間のどこで
始めたか」がそのまま原点のばらつきになる。離脱は探索と逆向きなので、機構端で
始まったときに押し込まない性質は保たれる）。**離脱の上限（`homing.release_distance`）は
`step` の倍数に頼らないこと** — 本来はスイッチの ON 区間の広さで決まる値で刻み幅とは
無関係なので、既定のままだと**精度のために `step` を詰めた瞬間に離脱の許容も一緒に縮み、
精度を上げるほどスイッチから離れられなくなる**（実機で踏んだ: `step` 0.5→0.1 で許容が
10mm→2mm へ落ち、ON 区間 2mm をあと 0.1mm 抜けきれずに失敗）。
原点確定はグループ単位でしか行わない（`set_group_origin_here`。左右を別々の時刻に
確定すると、その間に片方が動いたぶんだけ消えないオフセットが残る）。
**最後の再励磁は緊急停止を見る。** 付け替えの窓（`disable` 0.05 秒 + `SET_ZERO`
0.2 秒 + 鮮度待ち + 0.15 秒 ≒ 0.5 秒）で停止が入ると、`_send_e_stop_frames` の
`disable` の**後**に enable が届き、**停止中に励磁されたまま残る**（ログにも
ヘルスにも出ない）。`main._make_origin_resolver` が `is_estop_active` を
`capture_origin_via_set_zero` へ渡す（**既定値を置かない** —— 渡し忘れが
「緊急停止を見ない零点確定」として黙って通るため）。**中断してよいのは励磁だけで、
...
**事前確認は 5 つで、どれも 1 歩も動かさずに落ちる** —— 位置指令の軸か / 原点を確定する
手段があるか / センサの鮮度 / **対象軸モータの鮮度** / **励磁**。4 つ目が欠けると、未受信の
`0.0` を現在位置と信じて全ストロークぶんの指令を 1 回で出す。5 つ目は停滞判定でも拾えるが、
その文言は「引っかかり・探索方向・励磁」の 3 択なので原因を絞れない —— **`False`（無励磁だと
分かっている）のときだけ降りる。`is_energized()` の `None` は「励磁状態を報告しない
ドライバ」であって無励磁ではない**（倒すと常に `None` の M3508 が一切零点確定できなくなる）。

**センサの読み口は三値で、`None`（読めていない）を `False` へ丸めてはならない。**
可動端インターロックと同じ `main._make_sensor_reader` を零点確定も使う（同じ問いに
答える口が 2 つあると、片方だけが丸めた状態が作れる）。丸めると離脱段で
「途絶したセンサ = 離脱できた」に化け、触れたままの位置で原点を確定する。
**接触のラッチ（`consume_sensor_latch`）を持たないドライバでは現在値へ落とさず `None` を
返し、探索を始める前に降りる** — 現在値へ落とす実装は「ON 区間が `step` より狭いと
指令 1 回ぶんの通過を取りこぼす」経路そのもので、取りこぼした探索が向かう先は機構の破損側。

**離脱で戻った距離は探索距離を消費しない。** `search_distance` が縛るのは「開始位置から
どれだけ押し込むか」で、離脱は探索と逆向きなので、そのぶんを寄せ直しがもう一度走っても
到達しうる最も深い点は離脱前の位置より深くならない。数えないと、粗探索が上限をほぼ
使い切った軸で寄せ直しの上限が 0 以下になり、1 歩目から「−0.5mm 動かしても到達しません」
という読めない文言で落ちる（上限の判定は指令の**前**にしかないので、最後の 1 歩は上限を
最大 `step` ぶん超えうる。超過は 1 歩ぶんに限られるので許容している）。

**探索の到達判定はセンサのラッチで見る。「今 ON か」では取りこぼす。**
リミットスイッチの ON 区間が `homing.step` より狭いと、指令 1 回でその区間を跨いでしまい、
`settle_s`（50ms）後の観測時にはもう抜けている（`rotate` は step 2.0deg を約 18ms で
通過する）。実機で**スイッチに当たっているのに探索が止まらず、可動範囲の端を越えて
回り続けた**（その後 `SyncMonitor` の偏差超過で自動緊急停止。再実行では成功したので
タイミング依存として現れる）。センサの `FEEDBACK` は 100Hz で届いているので、受信の
たびにラッチする（`GenericDriver.consume_sensor_latch`。**読むと消えるので読み手は
`HomingRunner` 1 つに限る** —— 診断ツリーが描く「今どうなっているか」は
`sensor_active` のまま）。**探索を始める直前に 1 度捨てる** —— 手動操縦や前回の
零点確定で溜まったぶんが残っていると、1 歩目でいきなり到達と読む。
**離脱（センサから離れるまで動かす段）は現在値のまま。対称に見えて非対称である。**
ラッチを持ち込むと「一度でも OFF になった」で抜けることになり、接点のチャタリングで
まだ ON 区間の中にいるのに離脱完了と読む（離脱そのものの目的が消える）。取りこぼしの
向きも非対称で、探索の取りこぼしは**機構の破損側**へ進み続けるのに対し、離脱の
取りこぼしは「余計に離れる」だけで次の探索が寄せ直す。**検出したらその場の実測位置を
目標に送り直す** —— 最後に送った「実測 + step」が生きたままだと、原点確定
（`SET_ZERO` の `disable`）が届くまでスイッチを越えた先へ向かい続ける。

**原点を確定する手段は「PC 側位置制御ループ（M3508）」と「ドライバへの `SET_ZERO`」の
2 つで、可否はドライバ自身の `supports_origin_capture()` が答える**（`main.py` に
ドライバ種別を書き写さない）。**4 種のうち手段を持たないのは自作サーボと M3508 単独の
場合だけで、残りは「有効化したか」の差である**: 有効なのは `rotate`（EDULITE）と
サブハンド直動 2 軸（DM3520。`sub_y_axis` は前端スイッチ `0x49`、`sub_lift` は
**下端**スイッチ `0x4C`）の 3 本。`y_axis`（M3508）だけが
**手段もスイッチもあるが有効化していない** —— ファームも `sensors:` も揃っていて、
`homing:` だけを外してある（極性が未実測のため）。
**「スイッチが付いた」と「零点確定が有効になった」を混同しないこと** —— スイッチは
7 本繋がっているが、`homing:` から参照しているのは 3 本だけで、残りは `sensors:` に
登録して途絶を見ているだけである。

**ストロークが `step` に対して長い軸は二段探索にする（`homing.coarse_step`）。**
サブハンド直動 2 軸は、スイッチの移動許容が 0.3mm しかないので `step` を 0.1mm まで
詰めてある。その刻みで前後 500mm を探索すると 5000 歩 ≒ 5 分かかって点検に入らない。
**粗い刻みで当てる → 離脱 → `step` で寄せ直す**の 2 段にすれば、原点の粒度を
`step` に保ったまま実用時間で寄せられる（実測: 前後 28mm/1.1 秒、上下 120mm/19 秒、
どちらも再現性 0.1mm）。**`coarse_step` は ON 区間の実測より狭く取ること** ——
広いと粗い 1 歩で区間を跨ぎ切り、離脱も寄せ直しもできない位置で止まる。
**`coarse_step` を書くなら `release_distance` も必須**（離脱は粗い側の刻みで動くのに、
既定の許容は `step` から作られるので必ず桁が合わない）。
**手段が無い軸は探索を始める前に落ち、起動ログにも `ERROR` で出る。**
現況は `docs/checks_and_health.md` の
「零点確定（ホーミング）」節が正で、直したときは両方を同時に更新すること。
...
効かなくなる）。**ただしジョグの丸めだけは `clamp` ではなく `clamp_from`（範囲を起点まで
広げてから丸める）を通す** —— 素の `clamp` は起点が範囲の外に居ると 1 歩目だけを境界まで
飛ばす（実機で実測 +9.96mm・`max` 2.0mm の軸へ −1.0 のジョグを送り約 8mm 動いた）。
**零点確定がまだの軸は原点が電源投入位置なので、範囲の外に居るのは異常ではなく普通である。**
範囲の外からは寄る向きにだけ動け、外へ広げる向きは従来どおり境界で止まる。
`positions` に書いた値が範囲の外にあったら**起動を拒否**する
...
**PID ゲインを実行中に差し替える経路は持たない。** ゲインは `config/<robot>.yaml` の
`pid:` だけが持ち、起動時に `main._build_position_pid` が読んで以後動かない。実機で詰め直す
のは机上の CLI（`scripts/tune_y_axis.py` + `config/bench/y_axis_tuning/`）で、そちらは
ゲインの上書きをプロセス内に閉じるので config は書き換わらない。この形にしておくと
**「config に書いてある値 = 今動いている値」が構造的に保証される** —— 実行中に差し替える
口があると、良い値が決まる前の途中の値のまま試合に入れてしまい、しかもそれを画面から
読めない（かつて 0 で初期化した入力欄がそのまま送られ、`y_axis` の位置制御が無効になった。
検証は負値だけを弾くので `kp=0` は正常値として通る）。

**モータ名はロボット横断に一意。** ヘルス表示・アクチュエータ動作確認・指差喚呼
（`config/checklist.yaml`）はどれもモータ名だけで突き合わせるため、同名が両機に居ると
片方の状態がもう片方の欄に現れる。症状は「値が出ているのに機体と合っていない」だけで、
拒否もログも出ないので画面から原因が分からない。can_id と同じく
...
`loop()` が止まると、ウォッチドッグ満了の反映も出力の更新も止まる）③**`INFO` は 1 反復 1 通に
割り、落ちた ch は添字を進めず次の反復で送り直す**（再送キューは持たない）④**`FEEDBACK` は
落ちたら諦める**（次の周期が 10ms 後に来るので PC 側の STALE 判定 500ms には遠く届かない）。
**PC 側は `INFO` の未受信を照合しない**（それが正しい）ので、この壊れ方は FAULT にならない。
ただし §3.4 の焼き忘れ検出（`GenericDriver.info_mismatch`）はその間だけ黙って働かなくなり、
かつてはそれ自体が画面のどこにも出なかった。今は起動から `_FIRMWARE_INFO_GRACE_S`（3 秒）を
過ぎても未受信のモータを `RobotServer._firmware_unconfirmed_motors` が拾い、
`safety.firmware_unconfirmed_motors` として配信する（診断ツリーに青チップで出る。
`INFO` を送らないドライバ = M3508 / EDULITE 05 / DM3520 は対象外）。**これは「壊れている」の
報告ではなく「焼き忘れ検出が働いていない」の報告なので、FAULT にも `evaluateHealth` の
異常判定にも倒さない。** **拾うのは `FEEDBACK` が鮮度内で届いているモータだけ**（判定は
`feedback_timeout_ms`。同じ概念に別名を付けない）—— 基板が丸ごと落ちていれば全チャンネルが
STALE になって `evaluateHealth` が既に warning で主張し診断ツリーを強制展開するので、
ここでも言えば同じ事実を 2 度描くことになる。加えて**手当てが逆になる** —— この報告が
残したいのは「`FEEDBACK` は 10ms で届き続けているのに `INFO` だけが出ない」ケースで、
そこで電源・CAN 配線を疑っても必ず何も見つからない（配線が正常だから `FEEDBACK` が来て
いる）。UI の文面もファームの焼き直しと `candump` を案内すること。 **native テストでは守れない層**なので、送信まわりを触ったら実機に
書き込んで `candump` で確かめること。詳細と実測値は
`docs/impl_plan.md` の「送信バッファの本数は 3 枚で違う」。
...
**位置を測れないモータの POS 欄にだけ、代わりに PC の指令値（`command`）を出す**（4 値とも
`—` では画面がそのモータについて何も言えない）。**これは実出力ではない** —— ファームの
`max_duty` クランプ・`everFed_` ゲート・ウォッチドッグ満了・緊急停止ラッチのどれでも基板は
出していないので、`→` と `title` で断る。**M3508 位置制御ループの軌道の中間目標
（`M3508PositionLoop.target()`）をこの欄へ相乗りさせてはならない** —— あちらは配信して
いない値で、畳むと「今どこを狙っているか」と「何を指令したか」が同じ欄の中で入れ替わる。
**実測値があるモータには出さない** —— M3508 は緊急停止しても `command` が残るため、
この規則を外すと停止中の POS に指令値が出る（守っているのは `MotorStatus.test.tsx` の
1 件だけ）。詳細は `docs/impl_plan.md` の「測れないフィードバックは `null` で配る」。
...
入れるのは静的初期化子ではなく `setup()` が呼ぶ `ServoChannel::begin()`。
...
焼き忘れ検出そのものが黙って無効になる。その状態は
`RobotServer._firmware_unconfirmed_motors` が `safety.firmware_unconfirmed_motors`
として拾い、診断ツリーに「版番号 未確認」の青チップで出す。
...
反応も目視確認する）。**`sensors:` にも `expected_firmware` を書く**（`SensorConfig` が
持つ。`expected_angle_range_deg` は書けない —— センサはサーボの型を持たない）。
**駆動するモータが 1 台も無い基板では、これが焼き忘れを拾う唯一の手段である** ——
`motors:` 側の照合は `expected_firmware` を書いた対象としか突き合わせないので、
全スロットがセンサの基板は書かなければ照合対象がゼロになり、旧ファームのまま焼き忘れても
**「スイッチを押してもセンサ入力ビットが立たない」という配線不良と区別の付かない形**
でしか現れない。同じ基板にモータが載っている場合も省かない —— 「隣のモータが見ているから
...
**機体が動いているあいだ画面を覆ってはならない。** モーダルにしてよいのは、押す前に
出して押した瞬間に閉じる確認だけである。**動作確認の進捗と結果（`MotorCheckPanel`）は
モーダルではなくその場のインライン展開**で、`MatchPrep` の動作確認区分に置く。かつては
モーダルで、しかも起動と同時に自動で開いていたため、**両ハンドの全アクチュエータが順に
駆動されているあいだずっとヘッダーの EMG STOP がオーバーレイに覆われて押せなかった**
（クリックは背景として吸われ、パネルが閉じるだけ。`ModalProvider` の `openCount` は
ホットキーも封じる）。止める手段だけが画面から消える壊れ方で、操縦者には「押したら
パネルが閉じた」としか見えない。**平常時は畳み、実行中と失敗時は操縦者の開閉操作を
上書きして開く**（`SubsystemStatus` と同じ）。**中断は折りたたみの外側に置く。**

**EMG STOP の周囲に押下可能な要素を置かない。誤爆の向きは「隣のボタンを押そうとして
EMG STOP を踏む」である。** EMG STOP は赤地・大面積で常に同じ位置にあるので、狙って外すのは
隣の小さいボタンの側で、外した先が緊急停止になる（試合中に踏むと機体は止まりシーケンスは
降りる）。ヘッダー直下に来る帯の操作ボタンは**帯の先頭**に固定し、右端には押せないもの
（塞がれている理由・時計・状態チップ）だけを置く。ヘッダー自身の並び
（`[タブ帯] … [接続][時計][フェーズ][コート]`）も同じ理由で決まっていて、押せるものを
EMG STOP から遠い左側へ寄せ、**EMG STOP の手前の余白は装飾ではなく緩衝なので詰めてはならない**。
`ModeSwitch` の切替ボタンを現在モードのチップの隣ではなく先頭に置くのは、チップの文言長が
モードで変わるとボタンの横位置が動くため。**適用範囲は「ヘッダー直下の最上段」と
「画面全幅の要素の右端」に限る** —— `ActionPanel` の「左が STOP・右が主操作」は EMG STOP から
縦に遠いので従来どおりである。**ヘッダーの EMG STOP は点滅させない**（`alert-blink` は
`EStopOverlay` / `ConnectionBanner` のような**異常時にだけ画面へ出る要素に限る**。
平常時に点滅している要素が 1 つでもあると「平常時に静かで、異常時に自分から主張する」が
成立しない）。
...
**ショートカットの凡例は「そのキーが効く場所」にしか置かない。** 数字キーはタブ自身が、
`Space` は START / NEXT ボタン自身が `<Kbd>` として持つ。離れた場所に一覧を作ると、
①同じ事実を 2 度描くことになり ②割当を変えたときに一覧だけが古くなる（画面と食い違って
いることは実機を触るまで分からない）。実際にフッター（`StatusBar`）の凡例がタブ帯と
ボタン内の `Kbd` と完全に重複しており、キーが効く場所から最も遠い画面下端に居た。
**フッターは廃止した** — 接続表示と時計はヘッダー右端へ移し、常設帯はヘッダー 1 本にしてある。

**同じ記号に 2 つの意味を持たせない。** アイコンは文字と違って読まずに形で引くので、
1 つの記号が 2 つの状態を指すと、区別が付いた気になったまま取り違える。`Hand` は
「許可待ち / ここで停止」専用（`ActionPanel` / `SequenceStepList` / `RobotStatusRow`）で、
手動操縦は `SlidersHorizontal`（`ModeSwitch`）。かつては手動操縦にも `Hand` を使っており、
試合中に手のアイコンを見てもトリガー待ちなのか手動操縦なのか判別できなかった。
**同じ位置・同じ大きさに出る 2 つのボタンも同様** — START (`Play`) と NEXT (`ArrowRight`) は
`ActionPanel` の同じ面へ交互に出るので、記号まで同じだと色と文字でしか見分けられない。
アイコンを足すときは、既に使われている意味と衝突しないか確かめること。

**ラベルは英大文字と日本語で役割を分ける。** 英大文字の短語を使うのは、試合中に頻度高く
押す主操作と、その枠を占める状態表示に限る（`START` / `NEXT` / `STOP` / `RUNNING` /
`DONE` / `RESET` / `EMG STOP`）。それ以外の操作・状態・説明はすべて日本語で書く
（「先頭から再開」「シーケンス未取得」「試合開始前」「セッティングへ戻る」）。理由は、
周辺視野では文字を読まず形で拾うため、**大文字であること自体が「これは今押すボタンだ」の
印**になること。装飾として大文字を増やすとその印が薄まるので、逸脱を足すときはこの理由に
照らすこと。**英字でも大文字でなければこの印にはならない**ので、タブ名（`Monitor` /
`Main Hand` / `Sub Hand`）・接続表示（`Connected` / `Disconnected`）・緊急停止オーバーレイの
`Reset` は今までどおりで構わない（前 2 つは操作ではなく名前、最後は全画面を覆う唯一の
ボタンなので探す必要がない）。

**モード帯（`ModeSwitch`）とヘッダーのタブ帯は見た目を分ける。** 同じ `tabs tabs-box` を
上下 2 段に並べると、「見る場所を変える操作」と「機体の制御権を奪う操作」が同じ形になる。
モード帯は現在モードを状態チップで示し、切替は行き先を書いたボタン 1 つに絞る。
...
**これは grid の話であって、flex-col の子へ書き写してはならない。** `align-self` が効くのは
クロス軸で、grid（行方向に並ぶ）では縦だが **flex-col では横**になる。付けると幅が
`stretch` から `fit-content` へ落ち、内容しだいで列より縮むか、min-content のぶん溢れる。
**flex-col の子は主軸（縦）にはそもそも伸びないので `self-start` は要らない。**
実際に試合中の操縦者画面の右カラム（幅 358px）でこれを取り違えており、試合時間パネルが
157px へ縮んで `0:00` が枠からはみ出し、機体状態パネルは 424px へ膨らんで画面外へ出ていた。

**縦が足りないときに縮む側は明示する。** flex の既定（`flex-shrink: 1`）は列の全パネルを
一律に縮めるので、削れる余地の無いパネル（`MatchTimer`。高さが中身で決まり切っている）には
`shrink-0` を与え、**内部スクロールを持つパネル（`SubsystemStatus` のモータ一覧）に溢れを
引き受けさせる**。落とすと、機体状態が異常時に自分から展開した瞬間に試合時間の caption が
数字に重なって読めなくなる —— 試合中に最も参照する値が、最も要るときに消える。
