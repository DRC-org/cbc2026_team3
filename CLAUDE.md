...
**左右直結ペアは 4 組（`y_axis` / `rotate` / `sub_rotate` / `sub_pitch`）で、位置がずれるとその場で
壊れる。** 位置定数 yaml の `motors:` で 1 論理軸に複数モータを束ね、逆回転は `scale` の符号で表す。
保護は ①同一フレームでの同時指令 ②偏差監視（シーケンス停止 / ループ内 200Hz で電流 0 /
`SyncMonitor` 50Hz で全体緊急停止）③フィードバック途絶をペア単位で判定
（片方だけ止めると残った側が押し続ける）。
**①は 4 組すべてに効くが、3 重が揃うのは `y_axis` だけ。** ②③の入口はどちらも `sync_tolerance` で、
③はさらに `M3508PositionLoop` 内の `SyncGuard` にしかない（`main._attach_sync_groups`）ため、
`rotate` は②のシーケンス停止と `SyncMonitor` の 2 段だけ、サーボの 2 組は①だけになる。
**サーボ 2 組に `sync_tolerance` を書いてはならない。書き忘れではない** —— `FEEDBACK` がクランプ後の
指令角のエコーで偏差監視が原理的に働かず、効かない保護を書くと次に読む人がそこを保護と数える
（検出は `config/checklist.yaml` の `sub_servo_pair_sync` の目視だけ）。
**判定と単位換算は `lib/axis_sync.py` に一本化してあり、②の 3 段とも `SyncGroup.violation()` を呼ぶ。**
段ごとに違うのは頻度と超過後の扱い（debounce・ラッチ・効果）だけで、境界そのものがずれてはならない。
逆換算を段ごとに書き写すと、符号を 1 箇所落としただけで「ずれていないのに止まる」か
「ずれているのに止まらない」のどちらかになり、しかも片方の段だけが壊れるので気付けない。
`lib/axis_sync.py` は最下位層なので上位モジュールを import してはならず、段ごとの差とその理由は
...
ドライバ種別を書き写さない）。**残っている穴は 2 つで、性質が違う**: `sub_y_axis` /
`sub_lift`（DM3520）は**手段そのものが無い** —— スイッチを付けても `HomingError` になる
（`SET_ZERO` の安全な順序が `disable` を要求し、`sub_lift` は disable すると自重で落ちる
ため宣言しない）。`y_axis`（M3508）は**手段もスイッチもあるが有効化していない** ——
ファームも `sensors:` も揃っていて、`homing:` だけを外してある（極性と探索パラメータが
未実測のため）。有効なのは `rotate`（EDULITE）だけ。**「スイッチが付いた」と「零点確定が
有効になった」を混同しないこと** —— スイッチは 7 本繋がっているが、`homing:` から
参照しているのは 1 本だけで、残りは `sensors:` に登録して途絶を見ているだけである。
