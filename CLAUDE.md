...
- **ゲートも排他も全ロボットに掛かる。** どちらかが手動モード / シーケンス実行中なら起動を
  拒否し、実行中は両機の**目標値再送**を止める。片方だけ見ていると、確認中にもう一方が
  手動で動かされて干渉する。**M3508 位置制御ループは止めない** —— M3508 への指令は動作確認の
  ものもそのループを通る（`MotorHandle` の target_sink → `M3508PositionLoop.set_target` が
  唯一の経路）ので、止めると目標だけが入って電流が 1 通も出ず、`y_axis` を 1mm も動かせない。
  一度これで動作確認が零点確定の停滞判定（「指令しても動きません」）で必ず落ちる状態にあった
