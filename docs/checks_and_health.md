...
4. **停滞判定** — 再アンカー方式では「進まない機構」が探索距離を消費しないので、
   ...
   **この判定が実移動を観測できる時間そのものは `HomingRunner._wait_step` が持つ**
   （1 歩ごとに `settle_s` を最大 `_FOLLOW_ATTEMPTS`（5）回待つ）。早期リターンの
   閾値は必ず `step` より小さい値（`step/2`）にすること —— `spec.tolerance`
   （到達許容差）を使うと、`commanded = observed + step` なので 1mm も動いていない
   機構でも `|observed - commanded|` は常に `step` になり、`tolerance >= step` の軸
   （本番の `y_axis`: `tolerance: 1.0` / `step: 0.5`）では「動いた」と区別できず、
   初回の `settle_s` 1 回で必ず抜けてしまう（追従待ちが設計の 1/5 に縮退し、
   上の停滞判定が実移動を観測する時間まで削られる）
