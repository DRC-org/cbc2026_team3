...
1. **`search_distance` の上限** — 配線が抜けたセンサは「いつまでも当たらない」形でしか
   ...
   **`manual` の全幅以上であることを起動時に検証する**（`_check_homing_range`）——
   短いと可動範囲の端から始めた探索がセンサへ届く前に打ち切られ、症状は「機構もセンサも
   正常なのに零点確定だけが失敗する」。原点の近くから始めたときは成功するので、
   配線や探索方向を疑うことになる（実際に `manual` を 22mm 幅から 650mm 幅へ広げた
   ときに 22.0 のまま取り残された）。長い側は拒否しない —— 探索はセンサに当たった
   時点で止まるので、実害は失敗の検出が遅れることに留まる
