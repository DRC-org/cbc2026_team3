...
その行を `Unused` として宣言し（`kServoBoards`）、`config/main_hand.yaml` の
...
SV1〜SV4 = `0x49`〜`0x4C`、およびサーボ基板 #2 の SV0〜SV4 = `0x50`〜`0x54`。
どちらも現在は `Unused`）をファームの `kServoBoards` でその行を
`TouchSensor` にしてから登録すること。**スロット設定一式（役割・ピン・初期角・可動範囲・
パルス仕様・センサ極性）をファームが基板番号ごと・スロットごとに持つ**ので
（`kServoBoards[]`）、配線を差し替えるだけでは足りない。基板 #0 の SV3 を
