...
連続する tick 開始時刻の差 = 実周期を測り、公称周期の 1.5 倍（`JITTER_OVERRUN_FACTOR`）を
超えたら「乱れ」として超過回数と最悪値だけを O(1) で積む（サンプル列は持たない。
起床が遅れた分と tick 自身の処理が長すぎた分は区別しない — 時間予算にとってはどちらも
同じ「次の周期までに終わらなかった」であるため）。しきい値は `interval_s` からの相対値
...
（2026-09-05 に外した）。** しきい値 (`JITTER_OVERRUN_FACTOR = 0.5`) は理屈で
導いた値で、実機で検証していない。本番構成（CANable 4 本・毎秒 4200 通）で
200Hz の位置制御ループがどれだけ揺れるかは誰も測っていない —— 親が試した
のは WSL の virtual バスを 20 秒動かしただけで、警告 0 件だったが本番とは
条件が違いすぎる。もし実機で常時点灯すると、`SubsystemStatus` は同期ずれ
ラッチや緊急停止も出す安全機構のパネルなので、操縦者がそこ全体を見なく
なり、本物の警告まで巻き添えで信用されなくなる。加えて、仮に正しく
警告できても**試合中の操縦者にできる行動が無い**（PC の負荷は下げられ
ない）—— bus-off 復旧ウォッチドッグの節で触れた CAN の警告が「ワークが
落ちたかも → 掴み直す」という行動に繋がるのとは性質が違う、エンジニアが
後から読む情報である。**順番が逆になっていた**（先に実機で測って線を
決めてから画面に出すべきところを、理屈だけの線を先に画面へ出していた）
ので、実機計測が先に来るよう画面と配信だけを外した。

**乱れの記録は試合開始（`match_start` が成立した瞬間）にリセットする**
（`PeriodicTask.reset_jitter_stats()`、呼び出しは `lib/server.py` の
`_handle_match_start`）。**これは画面のためではなく、試合ごとに 1 行の
記録を journal へ残すため** —— 画面が無い今、累積のまま置くと「この試合で
何回・最悪何 ms 出たか」を journal から都度引き算しないと読めない。
リセットの直前に超過回数と最悪値を INFO で 1 行残す（超過 0 件のタスクは
出さない）ことで、その 1 行がそのまま「直前の試合の集計」になり、将来
実機で計測してしきい値を決めるときにそのまま使える単位になる。
**リセット点は `match_reset` ではなく `match_start`。** `match_reset` は次の
準備フェーズへ戻る操作なので、そこでリセットすると `finished`（結果確認中）
のあいだに直前の試合の記録が journal 上でも「どの試合の分か」を辿りにくく
なる。**回数と最悪値は必ず両方一緒に落とす**（片方だけ残すと「回数 0 なのに
最悪値が残る」不整合になる）。

**次にやるべきこと: 実機（本番の CAN 構成）で実際に動かして計測する。**
そのうえでしきい値を検証し、操縦者が対処できる行動が定義できるなら、
画面と WS 配信へ戻すかどうかを判断する。それまでは計測・WARNING・
試合単位のリセット・journal ログだけが有効な状態。
