...
| `rotate` | EDULITE 05 ×2 | 無効（config でコメントアウト） | 原点確定の経路が無い（スロットとセンサ登録は済んでいる。下記） |
...
**`rotate` のスロットは確保済みで、残る欠落は `SET_ZERO` の経路だけである。**
原点センサはサーボ基板 #0 の SV3（`0x43`）に載り、`config/main_hand.yaml` の
`sensors:` に `rotate_origin_sensor` として登録してある。そこに載っていた
`sub_gripper` はサーボ基板 #1 の SV0（`0x48`）へ移した —— **スロット設定一式（役割・
ピン・初期角・可動範囲・パルス仕様・センサ極性）をファームが基板番号ごと・スロットごとに
持つ**ので（`kSlotsByBoard[基板][スロット]`）、こう分けないと基板 #0 の SV3 を
スイッチにした瞬間に基板 #1 の SV3 も道連れになる。**配線を差し替えるだけでは足りず、
ファーム側の表がその基板番号のその行で対応していなければならない。**
