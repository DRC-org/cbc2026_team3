...
360 ステップ × 0.05s = 最短 18 秒で、**セッティングタイムに毎回これが入る**。
`step` は所要時間と原点の粒度のトレードオフなので、実機でスイッチ位置を測ってから
詰めること（`search_distance` は歯止めなので、速さのために縮めてはならない）。
...
`step`（0.5mm）ではなく**区間幅**（数 mm）で、症状は「原点合わせをしたのに位置がずれる」
だけ。始めた位置は毎回違うので再現もしない。**原点付近から動作確認を始めるほど踏みやすい。**
