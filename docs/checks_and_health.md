...
`sub_gripper` はサーボ基板 #1 の SV0（`0x48`）へ移した —— **スロットの役割はファームが
基板番号ごとに持つ**ので（`kSlotRoleByBoard`）、こう分けないと基板 #0 の SV3 を
スイッチにした瞬間に基板 #1 の SV3 も道連れになる。**配線を差し替えるだけでは足りず、
ファーム側の役割表がその基板番号の行で対応していなければならない。**
...
SV1〜SV4 = `0x49`〜`0x4C`。現在は `Unused`）をファームの役割表で `TouchSensor` に
してから登録すること。
