...
**止めるのは目標値再送だけ。M3508 位置制御ループは止めない。** M3508 は電流指令しか
受け付けず位置制御は PC 側のループが担うので、**動作確認が M3508 へ出す指令もそのループを
通る**（`MotorHandle` の target_sink → `M3508PositionLoop.set_target` が唯一の経路）。
止めると目標だけが入って電流フレームが 1 通も出ず、`y_axis` を 1mm も動かせない ——
症状は零点確定の停滞判定（「指令しても動きません」）か到達タイムアウトで、機構もモータも
正常なので配線を疑うことになる。0x200 のスロットを奪い合う相手も居ない（同一バスの
M3508 は必ず 1 つのループが束ねる）。
