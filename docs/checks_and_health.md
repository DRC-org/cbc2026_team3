...
homing:
  ...
  step: 2.0                      # 1 回の移動量。下限は静止摩擦、上限は tolerance
  settle_s: 0.05
```

**探索時間は `search_distance / step` × `settle_s` を下回らない。** 上の値では
180.0 / 2.0 = 90 ステップ × 0.05s = 最短 4.5 秒で、**セッティングタイムに毎回これが入る**。
縮めてよいのは `step` だけで、`search_distance` は唯一の無人の歯止めなので速さのために
縮めてはならない。**その `step` も「小さいほど良い」値ではない** —— 下限は静止摩擦が、
...
もう抜けている —— `rotate` は step 2.0deg / `limit_speed` 2.0rad/s なので **2deg を
約 18ms で通過**する。実機ではこれで**スイッチに当たっているのに探索が止まらず、
...
1. **検出の遅れ** —— ラッチを読むのは `settle_s` ごとなので、スイッチに触れてから
   ...
   その 1 周期に進める距離も **`step` 以下**（`rotate` で 2.0deg）
2. **残っている指令** —— 検出位置からさらに `step` 先まで向かう。**送り直しが消すのはここ**

つまり `rotate` の行き過ぎは**最大 2step ≒ 4deg から最大 1step ≒ 2deg へ縮む**。
**スイッチは可動範囲の端（0deg）にあり、2deg 程度の行き過ぎには耐える強度がある**という
...
`step`（`y_axis` の 0.5mm。`rotate` は deg 単位で 2.0）ではなく**区間幅**（数 mm）で、
...
| `step` **2.0deg** | **実機で決めた。** 0.5deg では `position_kp` 30.0 が生むトルクが減速機の保持力と静止摩擦を超えられず、離脱段で 3 歩連続して進まず `HomingError` になった。手動操縦では 1.0deg で動いたが、ホーミングは 1 歩ごとに静止するので**毎歩で静止摩擦を超え直す**必要がある。閾値ぎりぎりだと停滞判定（`step/2` 未満が 3 歩連続）へ落ちるので余裕を見た値 |
...
現在値は**許容範囲の上限ちょうど**で、動作の安定を確認したうえで 1.0deg へ下げて精度を
上げる余地はある —— 下げる日には「数歩ごとに停滞判定へ落ちないか」を実機で確かめること。
