...
> **2026-09-09 に実測済み。結果は
> [`history/incidents.md`](history/incidents.md) の 2026-09-09 が持つ。**
> 零点は電源断を跨いで残り、代わりに**位置の報告値が [0, 360) へ畳まれる**。以下の
> 手順と分岐表は「そう決まる前の見立て」を含むので、結論は上のエントリを読むこと。
>
> **設計はその後 PC 側オフセットへ移した。** 起動・励磁・零点確定のどの自動経路からも
> `SET_ZERO` は 1 通も出ない（`docs/invariants.md` §3）。このツールの `--set-zero` は
> 調査用に残してあるが、**使うと PC 側の原点と二重定義になる**ので、試合前の機体へは
> 打たないこと。
...
- **サーバー（`main.py`）を止めること。** 同じバスへ 2 つのプロセスが問い合わせると、
  どちらの読みも相手の送ったフレームへの応答と混ざる。ツールは起動時に 0.3 秒バスを聞き、
  自分が 1 通も送っていないのにフィードバックが流れていたら「他プロセスが使っている」と
  して実行を拒否する
