...
**故障ではない。手順が 1 段抜けている。** サブハンドの前後 `sub_y_axis` は、昇降 `sub_lift` が
移動高さ（`top`）に居るときしか動かせない。零点確定は下端から 10mm 離脱して終わるので、
**昇降を確定しただけでは `top` に居ない。**

手当ては 2 手:

1. 昇降 `sub_lift` の零点をまだ確定していないなら先に確定する
2. **手動操縦で `sub_lift` を `top`（-140mm 付近）へ寄せてから**、`sub_y_axis` を選び直す

拒否の 1 行にこの手当てが入っているので、そのとおりに動かせばよい。**統合動作確認（②）から
走らせる場合は自動で寄せるので、この拒否は出ない。** 作動点測定でも同じ条件が掛かる。
