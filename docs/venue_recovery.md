...
**止まるのは 3 本のセンサでだけである。** 零点確定が見るのは `rotate_origin_sensor`
（**サーボ基板 #0 SV3 = `0x43`**）と `y_axis` 左右の 2 本（右 = #0 SV4 = `0x44` /
左 = **#1** SV0 = `0x48`）で、この症状で見に行くのはその 3 本だけでよい。配線・基板の
電源・ファームの焼き込みを順に見る。
...
**サブハンドの 4 本（`0x49`〜`0x4C`）はこの症状を出さない。** 零点確定からは参照して
いないので、途絶してもシーケンスは止まらない。**ただし「何にも使っていない」わけでは
なく**、リミットスイッチ保護の入力なので、途絶している間はその軸の保護が消える
（診断ツリーに `STALE` と「リミット保護 無効」の両方が出る → 3-4）。
**動作確認が止まった原因としてそちらを見に行っても時間を捨てるだけである。**

### 3-4. 軸が片方向にだけ動かない（逆へは動く）

**リミットスイッチ保護がラッチしている。** 機体状態（`SubsystemStatus`）が自分から開いて
**「リミット到達」**のチップと軸名・センサ名を出す。ここが出ていれば故障ではない ——
スイッチに触れたので、その向きへの指令だけを止めている。

1. 出ているセンサ名と反対の向きへ手動操縦で少し戻す。**離れれば自動で解除される**
   （Reset も再起動も要らない）
2. 触れていないのに止まる／逆向きへも動かないなら、`limits` の `direction` の取り違えを
   疑う（指差喚呼 `limit_direction_sub` が見ているのはこれ。**サブハンドの 4 本は
   未実測**）
3. **「リミット保護 無効」**の青チップが出ていたら逆で、そのスイッチは応答が途絶えていて
   保護が効いていない。触れても止まらないので、その軸は手動で慎重に動かす（センサの
   途絶そのものは STALE として別に出る → 3-2）
...
### 3-6. 指令しても動かない（無励磁のまま） → **緊急停止を押さずに「再励磁」**
...
### 3-7. journal の読み方（2026-09-08 でログの体裁が変わった）
