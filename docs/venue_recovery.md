...
| 軸 | 手で動かしてよいか | 戻ったとき |
|---|---|---|
| `rotate`（メインハンド） | **よい。** 零点確定が済んでいれば、可動域（`travel` 0〜200deg）から回転数が決まるので、どこまで回してあっても正しく読む | そのまま復帰してよい。**ただし可動域の外まで回すと journal に「論理位置が可動域の外にあります」が 1 回出る** —— 出たら機構を可動域の中へ戻してから解除する |
| `rotate`（**零点確定がまだ**） | **動かさない。** 暫定原点しか無いあいだは可動域が使えず、電文 2 通の差分だけで読む。**ちょうど 180deg 動かすと符号を取り違え、次の指令で逆向きに 180deg 回って可動端に当たる**（2026-09-10 実機） | 動かしてしまったら**零点合わせをやり直す** |
