...
**故障ではない。手順が抜けている。** サブハンドの前後 `sub_y_axis` は、昇降 `sub_lift` が
移動高さ（`top`）に居るときしか動かせない。零点確定は下端から 10mm 離脱して終わるので、
**昇降を確定しただけでは `top` に居ない。** パネルからまとめて回すと、`sub_lift` は成功・
`sub_y_axis` は失敗という結果で**毎回**終わる。

**急いでいるなら②の統合動作確認から回す。** そちらは自動で寄せるのでこの拒否は出ない。

パネルから前後だけ確定したいときは 4 段:

1. 零点合わせで **昇降 `sub_lift` だけ**確定する（前後は失敗するが、それでよい）
2. **手動操縦モード**へ切り替えて `sub_lift` を `top`（-140mm 付近）へ寄せる
3. **手動操縦モードを抜ける** ← 忘れると「手動操縦モードのため実行できません」で次が拒否される
4. 零点合わせで `sub_y_axis` を選ぶ

①を先にやるのは、原点が未確定のうちは `top` という座標そのものが意味を持たないため。
作動点測定（`switch_measure`）でも同じ条件が掛かる。
