...
原点センサ（`rotate_origin_sensor`、**サーボ基板 #0 SV3 = `0x43`**）が FEEDBACK を
返していない。配線・基板の電源・ファームの焼き込みを順に見る。**センサの配線不良と、
そもそも存在しないスロットを登録している場合とで症状が同じ**なので、
`config/main_hand.yaml` の `sensors:` の `can_id` が実在するスロットかを先に確かめる
（サーボ基板 1 枚は SV0〜SV4 の 5 本しか無い）。

**`0x44`（SV4）と間違えないこと。** そちらは `y_axis` の原点スイッチ用に予約した
スロットで、**スイッチが未装着なのでファームは `Unused`・`sensors:` にも未登録**である
（登録されているセンサは `0x43` の 1 つだけ）。`0x44` を見に行っても配線そのものが無い。
