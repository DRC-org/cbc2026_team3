...
### 3-3b. 作動点測定が「'sub_lift' が top に居るあいだしか動かせません」で拒否される

**故障ではない。前後 `sub_y_axis` は昇降 `sub_lift` が移動高さ（`top`）に居るときしか
動かせず、作動点測定は寄せない。** 操縦者画面の零点合わせは昇降と前後をまとめて送るので
この拒否は出ない（中で「昇降を確定 → `top` へ寄せる → 前後を確定」の順に回す）。

**手当て**: 手動操縦で `sub_lift` を `top`（-140mm 付近）へ寄せてから、前後の作動点測定を
押す。操縦者画面のボタンは押すと全機を半自動へ戻してから送るので、モードを自分で抜かなくてよい。
...
| `sub_y_axis` / `sub_lift`（サブハンド） | **必要** | DM3520 は電源投入時に位置が 0.0rad へ固定され、**モータ内部の原点がその瞬間の姿勢へ作り直される**。再励磁を跨ぐと零点確定済みが落ち、`sub_y_axis` の指令が「先に 'sub_lift' の零点確定を」と拒否される（ソフトの緊急停止を解除した後も同じ） |

やり直しは操縦者画面の**零点合わせ**（担当機だけを単独実行。Monitor の動作確認は両機が動く）。
**終わった昇降は下端スイッチの 10mm 上に居る**（離脱したぶん）ので、前後を動かすなら
先に `top` へ寄せること。
