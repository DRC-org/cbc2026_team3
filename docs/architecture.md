...
| 粗探索（`homing.coarse_step` を書いた軸のみ、探索の前） | 到達を待たず、目標を実測の `coarse_step` 先へ毎周期（`settle_s`）出し直して止まらずに流す（`_glide`）。接触の判定は探索と同じ累計。当てたら離脱してから `step` で寄せ直す |
| 停滞判定 | `step/2` 未満が 3 歩連続で `HomingError`。粗探索は `_FOLLOW_ATTEMPTS` 周期で `coarse_step/2` 進まなければ |
