...
| 起動 | `_start_all` | `CANManager.run()` → 位置制御ループ → 同期監視 → リミット保護 → 目標値再送 → サーバー |
| 畳む | `_shutdown_all` | リミット保護 → 位置制御ループ → 目標値再送 → 同期監視 → CAN → サーバーの順（順序は不変条件） |
...
**リミット保護だけは全ロボットの配線が済んでから組む**（`_build_sensor_view` →
`_wire_limit_guards`）—— センサ読み取り口はセンサ名がロボット横断に一意であることに依って
いて、全機の `CANManager` が揃うまで組めない。そのためサーバーへの登録も
`_wire_one_robot` の中ではなく `_register_robots` がまとめて行う。

生成した部品は `server.add_robot(robot_name, seq, can_manager, position_loops=…,
sync_monitors=…, target_refreshers=…, limit_guard=…, manual=…)` でサーバーへも渡す（サーバーは
これを①動作確認との排他 ②緊急停止解除でのラッチ解除 ③緊急停止時の保持目標の破棄
④安全ループの生死配信 ⑤リミット保護の状態配信 に使う）。
...
周期タスク    control/position_loop.py / sync_monitor.py / target_refresh.py /
              limit_guard.py / trajectory.py ─ periodic.py / feedback.py /
...
| `control/limit_guard.py` | リミットスイッチ保護（`LimitGuard` 50Hz / `CombinedLimitGuard`）。判定 `clamp()` は指令の入口と共有する単一情報源 |
...
### リミットスイッチ保護（`lib/control/limit_guard.py`）

対象は `config/<robot>_positions.yaml` の `axes.<軸>.limits`（`sensor` と `direction` の対。
どちらも省略不可。§6）を書いた軸で、現状は `y_axis` / `rotate` / `sub_y_axis` / `sub_lift` の
4 本、**スイッチ 7 本が全部ここから参照されている**。零点確定と同じスイッチを別の役割で使う軸も
あるが、あちらは「当たるまで動かして原点を決める」手順、こちらは「当たったらその向きへは
進めない」常駐保護である。

**2 層あって、判定（`LimitGuard.clamp(axis, value, observed)`）だけを共有する。**

| 層 | 頻度 | 何をするか |
|---|---|---|
| ① 常駐（`LimitGuard` 自身） | 50Hz | ラッチの立ち上がりで、その軸の目標を実測位置へ**1 度だけ**書き戻す（`AxisHandle.set_target_value` 経由） |
| ② 指令の入口（`AxisHandle`） | 指令のたび | シーケンス・手動・零点確定の全指令を端でクランプする |

`LimitGuard` は `CANManager` も `PositionTable` も掴まない。センサ読み取り（`sensor_active` /
`sensor_contact_count` / `sensor_is_stale`）と軸ハンドルの取得（`axis_handle`）は注入で受ける
ので、CAN を 1 本も立てずに全経路を検証できる（`SyncMonitor` / `HomingRunner` と同じ作法）。

| API | 何を出しているか |
|---|---|
| `clamp(axis, value, observed)` | **判定の単一情報源。** 禁止方向へ `observed` より進む指令を `observed` で頭打ちにし、曲げたら `intervention` を進める |
| `latched` | 軸名 → ラッチ中のセンサ名（読み取り専用の写し。生の集合は渡さない） |
| `blind_sensors` | 途絶で判定できていないセンサ名。一時停止中のセンサは載せない |
| `intervention(axis)` | `LimitIntervention(count, sensors)`。**単調増加で、`reset()` でも落とさない** |
| `suspend_sensors(names)` | contextmanager。零点確定（`HomingRunner.home`）だけが通る |
| `watches(sensor)` / `axis_names` / `sensor_names` | 配線と零点確定が「外すべき保護」を選ぶために使う |

`CombinedLimitGuard` は複数ロボットの保護を 1 つの `clamp` 口へ束ねる（統合動作確認は両ハンドの
モータを 1 つの `MotorGroup` へ混ぜるので、結べる保護は 1 つしか無い。軸名はロボット横断に一意
なので、どちらが答えるかは軸名だけで決まる）。**判定は 1 行も持たず配るだけである。**

配線は `main.py` の `_build_sensor_view`（センサ読み取りの注入元）→ `_build_limit_guard`
（ロボット 1 台ぶん）→ `_wire_limit_guards`（全ロボットを組み、`MotorGroup.bind_limit_guard` で
層②を結ぶ）→ `_suspend_limit_guards`（零点確定へ渡す `suspend_sensors`）の順。
**後始末では最初に止める**（新しい目標値を書きうる唯一の常駐タスクなので、後回しにすると他を
止めた後の 1 周期が「止めたはずの軸」へ目標を 1 通書く）。設計の根拠は
[invariants.md](invariants.md) §3 のリミットスイッチ保護の各節。
...
| `LimitGuard` | 50Hz | `limits` を持つ全軸 | ラッチの立ち上がりで、その軸の目標を実測位置へ 1 度だけ書き戻す |
...
| 探索 | 毎ステップ `AxisHandle.observed_value()` を読み直して `commanded = observed + direction*step`。到達判定は**接触回数の増加**（`GenericDriver.sensor_contact_count`。単調増加なので基準値を探索開始直前に取り直す） |
| 停滞判定 | `step/2` 未満が 3 歩連続で `HomingError` |
| 検出後 | その場の実測位置を目標に送り直してから原点確定 |
| 原点確定 | `set_group_origin_here`（グループ単位でしか行わない） |

`home()` 全体は `LimitGuard.suspend_sensors(homing.sensor_names)` の中で走る（`finally` で必ず
戻す）。**外すのは見るセンサだけで、軸まるごとではない**（理由は
[invariants.md](invariants.md)「零点確定の最中に外すのは `homing.sensor_names` のセンサだけ」）。
...
| リミット保護の介入 | 指令の**前**に `AxisHandle.limit_intervention` を控え、到達待ちの後で比べる。増えていたら到達していても失敗（`SequenceTimeoutError` にセンサ名を添える）—— 層①が目標を実測へ書き戻すと到達判定は必ず成立してしまうため |
...
| メソッド | 責務 |
|---|---|
| `set_target_value(commands)` | `{モータ名: 指令値}` を `command_mode` で**同時に**送る（`asyncio.gather`）。**唯一の指令経路。** リミット保護のクランプ（層②）はここで掛かり、**実際に送った指令値を返す**（手動操縦はこの戻り値をジョグの起点に控える） |
| `has_target` / `limit_intervention` | リミット保護が「止めるものがあるか」を問う口 / その軸で保護が要求を曲げた回数（`LimitIntervention`。単調増加） |
...
axes:                      # 換算: command = value * scale + offset
  ...
  y_axis:                  # 複数モータで駆動する論理軸。軸名はモータ名でなくてよい
    ...
    limits:                # リミットスイッチ保護（§4）。書かない軸に保護は無い
      - { sensor: y_axis_r_origin_sensor, direction: -1 }   # この向きへ進む指令を止める端
      - { sensor: y_axis_l_origin_sensor, direction: -1 }   # sensor / direction とも省略不可
...
| `command_mode: position` 以外の軸に `manual:` / `motion:` / `homing:` / `limits:` | 可動範囲・軌道・原点・端という概念が無い（`duty` / `on_off` は現在位置も到達も観測できない） |
| `limits` の `sensor` が `sensors:` に居ない / 同じ軸で同じ `sensor` を 2 度書く / `direction` が ±1 以外 | 「書いたのに効かない保護」ができる（`sensors:` への登録漏れは `tests/test_robot_sequences.py` が静的に固定する） |
| `limits.direction` が同じセンサを指す `homing.direction` と食い違う | 逆向きの保護は保護が無いより悪い（[invariants.md](invariants.md)「`limits.direction` は…一致させる」） |
...
```jsonc
{
  ...
  "safety": {
    ...
    "limit_latched": { "y_axis": ["y_axis_r_origin_sensor"] },  // リミット保護が止めている軸 → センサ名
    "limit_blind_sensors": [],           // 途絶で保護が効いていないセンサ（介入回数は配らない）
...
| ホーミングは `rotate` だけ実機検証済み | `search_distance` はまだ効く経路を通っていない。`step` 1.0deg での通し確認も未取得。**`y_axis` は 2026-09-08 に有効化しただけで実機ではまだ 1 度も走らせておらず**、`direction` / 左右 2 本の極性 / `align_distance` / `step` はどれも未実測（初回は人が付くこと） |
| 零点確定が有効なのは `rotate` / `y_axis` の 2 軸 | `sub_y_axis` / `sub_lift`（DM3520）はドライバが `supports_origin_capture()` を宣言しない（`SET_ZERO` の安全な順序が `disable` を要求し、`sub_lift` は自重で落ちる）。原点が確定できない軸は電源投入位置がそのまま原点で、ずれは指差喚呼が人の目で埋める |
| リミットスイッチ 7 本の `direction` と、スイッチから機構端までの余裕が未実測 | メインハンド 3 本は同じセンサを指す `homing.direction` と一致することを読み込み時に突き合わせるが、**サブハンド 4 本には突き合わせる相手が無く指差喚呼 `limit_direction_sub` が唯一の検出経路**。余裕は `y_axis` の実測パラメータで**約 24mm 必要**（[`mechanism_handoff.md`](mechanism_handoff.md) §1 / §2） |
