...
| 解除の再励磁中・原点の付け替え中 | `hold_target_refresh` で止め、抜けるときに目標を捨てる | 同左（ラッチも捨て、励磁後の実測で取り直す） |
| DM3520 のレンジが未確認 | — | **送らない**（ラッチも取らない。`activation_block_reason()` を読む） |
| 動作確認中 | 止めない | 止めない |
...
| 粗探索（`homing.coarse_step` を書いた軸のみ、探索の前） | 到達を待たず、目標を実測の `coarse_step` 先へ毎周期（`_GLIDE_PERIOD_S` 10ms。`settle_s` は使わない）出し直して止まらずに流す（`_glide`）。接触の判定は探索と同じ累計。当てたら離脱してから `step` で寄せ直す |
| 停滞判定 | `step/2` 未満が 3 歩連続で `HomingError`。粗探索は `_GLIDE_STALL_S`（0.25 秒）で `coarse_step/2` 進まなければ |
