...
#### 複数センサのラッチは 1 回の観測でまとめて読む

`GenericDriver.consume_sensor_latch` は**読むと消える**ので、センサごとに別々の
タイミングで読むと「A を読んだ瞬間に B の接触が捨てられる」。どちらが先に当たったかは
探索から整列段へ持ち越す必要があるため、`_SensorLatches` が 1 回の観測で全センサを
読み、走行中は OR で溜め込む。
...
### シーケンス制御中でも手動で操作できる軸（2026-09-09）

コンベアを止めるだけ・弁を 1 個開けるだけのために、シーケンス全体の制御権を奪う
（`set_operation_mode` が `_stop_sequence` を呼ぶ）必要があった。試合中には実質使えない
操作なので、**軸単位で** `OperationMode.SEQUENCE` のままの手動指令を許すようにした。
宣言は位置定数 yaml の `axes.<軸>.manual_always` だけが持ち、対象は
`main_hand` の `conveyor` と `sub_hand` の `valve_1`〜`valve_6` の 7 軸。

**この 7 軸だけ開けてよい根拠は 3 つで、どれか 1 つでも欠けたら開けてはならない。**

1. どちらも**単独モータ**（DC 基板 1ch / 電磁弁基板 1ch）なので、左右直結ペアの
   「同一フレームで同時指令」の不変条件に一切触れない
2. どちらも**到達判定を持たない軸**（`duty` / `on_off`）で、シーケンスは `settle_s` の
   固定待ちへ落ちる。手動が割り込んでも `AxisHandle.wait_reached` を汚さない。
   **位置指令の軸で同じことをすると必ずタイムアウトする** —— シーケンスが `move_to` で
   書いた目標を手動が上書きし、`wait_reached` が動かない位置を見続ける
   （`blocked_during_reenergize` が塞いでいる害と同型）。この条件は
   `AxisSpec._check_manual_always` が起動時に強制する（`manual:` が位置指令の軸にしか
   書けないのと対）
3. `duty` / `on_off` 軸に `manual:` は書けないので、送れるのは `manual_move`
   （位置名によるプリセット）だけ。「定義した状態以外を送れない」という通常運用の
   保証がそのまま残る（`manual_set` / `manual_jog` は対象軸でも
   `ManualController._require_manual` が従来どおり拒否する）

**シーケンスによる上書きは許容する。** 手動でコンベアを止めても、次にコンベアを回す
ステップが来れば再び回る。シーケンスが手順の正であり続けるので、除外・ラッチの仕組みは
作らない。

**緩めたのは `_manual_target` のモード判定 1 枚だけである。** 緊急停止ゲート
（`manual_move` は `allowed_during_e_stop=False` のまま）も再励磁の in-flight 判定も
1 つも触っていない。判定順を「軸の解決 → モード判定」へ入れ替えたのは、どちらの軸かが
分かるまでモードの可否を決められないため。副次的に、**軸名の打ち間違いが
「手動操縦モードではありません」で覆い隠されなくなった**（覆い隠すと、切り替えても
直らない拒否を操縦者が延々と踏む）。

**この緩和経路にだけ、動作確認の実行中を塞ぐ判定を足した**
（`RobotServer._allow_manual_in_sequence`）。`sequences/motor_check.py` は
`main_conveyor`（conveyor を run→stop）と `sub_valves`（弁 6 個を 1 個ずつ開閉）を
**まさにこの 7 軸**で駆動し、どちらも操縦者が目視・打音で確かめるステップである。
それが守られていたのは「動作確認は全ロボットが sequence モードでないと起動できない →
sequence モードでは手動が全部拒否される」という連鎖に乗っていただけで、**軸単位で
緩めた瞬間にこの連鎖が切れる**（弁の作動音を聴いている最中に別の弁を開けられる）。
`motor_check_start` 側の排他（`_motor_check_environment_deny`）と対になる、逆向きの塞ぎ。

軸名はサーバーにも UI にも書かない。サーバーが問う口は
`ManualController.is_always_manual()` ただ 1 つで、一覧は
`PositionTable.manual_always_axes()` が答える。配信は `manual.axes[].manual_always`
（**config と同じ語**。配信側だけ別名にすると、UI が読む語と config に書く語が
食い違う）。
