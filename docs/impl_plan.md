...
| 台形速度プロファイルの値が実機未検証 | `config/main_hand_positions.yaml` の `axes.y_axis.motion`（`max_velocity` 50.0 / `max_acceleration` 300.0 / `velocity_ff` 1.0）は**机上の計算値**。15mm 移動 0.467s・追従誤差 0.284mm 程度・`output_limit` の 75% が余裕、という見積もりだけが根拠 | 保守側に振ってあるので「遅い」「届かない」向きには倒れるが、**速い向きに間違っている確証も無い**。実機で上げていく手順は `docs/mechanism_handoff.md` §3-2。危険の向きは `max_acceleration`（機構強度とワーク保持力）と、`velocity_ff` を `kd` と揃え忘れたときの**巡航中の逆向き飽和** |
