...
**この規約は `lib/config_schema.py` の `_check_dm3520_master_id_collisions` が
`load_robot_config` の中で機械的に検査し、違反していれば起動を拒否する。**
出荷値のまま（ESC_ID == MST_ID）残った個体や、2 台の ESC_ID/MST_ID を交差させて
書いてしまった個体を検出する（回帰テストは `tests/test_config_schema.py` の
`test_dm3520_master_id_*`）。バス単位（同じ物理バスに繋がったノード同士）で見るのは
`can_id` の重複検査と同じ理由で、`can_dm3520` が現状 1 ロボットの config でしか
使われないため 1 ファイル内で閉じた検査にしてある（ロボット横断の `can_id` 重複が
`tests/test_robot_sequences.py` の静的テストに寄っているのとは対称的な判断）。
...
| リミットスイッチ保護 | ①50Hz 常駐の引き戻し（`LimitGuard`）②指令の入口のクランプ（`AxisHandle`）③介入回数による `move_to` の失敗 | 層ごとに別テストファイルを持つ（`test_limit_guard.py` / `test_limit_clamp.py` / `test_limit_intervention.py`）。①だけを見るには**保護を結んでいない `AxisHandle`** を、②だけを見るには **50Hz を 1 度も回さない**条件を作る |
...
| リミット保護は触れた向きだけを止め、逆方向は必ず通す | `LimitGuard.clamp()` の向き判定 `(value - observed) * direction > 0` から符号を落とす（**端に触れた軸が二度と動かせなくなる**）/ `_stop_axis` を `clear_target` へ変える（**`sub_lift` が消磁して自重で落ちる**）/ 立ち上がり判定を落として毎周期書き直す（**軸がクリープし、退避の指令も上書きされる**） | `test_limit_guard.py::TestClamp` / `::TestStopsOnContact` / `::TestHoldIsWrittenOnce` |
| リミット保護は 50Hz と指令の入口の 2 層で、境界を共有する | `AxisHandle` から `limit_guard.clamp` の呼び出しを外す（**層①だけになり、走行中に触れても押し戻されるまで 20ms 進む**）/ `LimitGuard._stop_axis` の書き戻しを落とす（**層②だけになり、走行中に触れても止まらない**）。**2 つは別々のファイルが受け持つ**（統合経路では 1 枚壊しても他が拾う） | `test_limit_clamp.py::TestBlockedDirection` / `test_limit_guard.py::TestStopsOnContact` |
| 保護が介入した移動を「到達」と読ませない | `move_to` の介入カウンタの前後比較を落とす（**目標を実測へ書き戻すので `is_reached` は必ず成立し、軸は途中に居るのにシーケンスだけが進む**）/ 比較を「終了時のラッチ」へ変える（**移動中に触れて離れた接点のバウンドを 1 件も拾えない**） | `test_limit_intervention.py::TestTouchedWhileMoving`（`test_移動の途中で触れた移動は失敗する` / `test_触れて離れた移動も失敗する`） |
| 零点確定はセンサ単位で保護を外す（軸まるごとではない） | `LimitGuard.suspend_sensors` を素通しにする（**`limits:` を書いた軸でホーミングが必ず失敗する**）/ 一時停止を軸まるごとへ広げる（**両端スイッチの軸で反対端の保護まで消える**）/ 入るときのラッチ破棄を落とす（**1 歩目だけが古いラッチで止まる**） | `test_limit_guard.py::TestSuspendSensors` / `test_homing.py::TestSuspendsTheLimitGuard` |
| センサ途絶では保護を発動させず、効いていないことを配る | `_is_latched` の `sensor_is_stale` 分岐を落とす（**スイッチ 1 本の不調で試合中に軸が動かなくなる**）/ 途絶時にラッチも落とす（**押し当てたまま保護が消える**）/ `blind_sensors` へ載せない（**黙って無効になり「守っているつもり」の機体になる**） | `test_limit_guard.py::TestStaleSensor` / `test_server_limit_state.py::TestBlindSensorsAreBroadcast` |
| センサの接触は読んでも消えない | `GenericDriver.sensor_contact_count` を consume 方式のラッチへ戻す（**保護が読んだ瞬間に零点確定が接触を取りこぼす**）/ ラッチ条件から現在値の OR を落とす（触れっぱなしを拾えない）/ 回数の増加を落とす（観測周期より狭い ON 区間を落とす） | `tests/drivers/test_generic.py::TestSensorInput` / `test_limit_guard.py::TestLatchCondition` / `test_homing.py` |
...
│           │   ├── AppHeader.tsx        # 画面唯一の常設帯。フェーズ / コート / タブ /
│           │   │                        #   接続（WsSettings への入口）/ 時刻 / EMG STOP を 1 段に収める
│           │   ├── TabBar.tsx           # タブ定義は lib/tabs.ts。AppHeader の中に畳む
│           │   ├── Clock.tsx            # ヘッダーの時刻。毎秒 setState するので必ず切り出す
...
**実使用は `positions.conveyor.run`（メインハンドの搬送方向）1 箇所だけ**で、他はすべて
スカラーである（「コート別位置定数の初の実使用」節）。
...
### 指令してから待ち始めるまでの窓で消えた目標も「中断」である（2026-09-08）

`move_to` は全軸へ `set_target_value` を送り終えてから `wait_reached` をまとめて作る
（到達待ちコルーチンを溜めないため。前節）。**その間に緊急停止
（`TargetRefresher.clear_targets()`）が挟まると、`wait_reached` は最初から目標が無い
状態で待ち始める。**

`MotorHandle.wait_reached` の中断検出は「待ち始めた時点で目標を持っていたか」の
スナップショットに拠っていたので、この窓で消えた目標は「一度も指令していない軸」と
区別が付かない —— `is_reached()` の「目標が無ければ到達済み」に吸われて **True** を
返し、**中断された動作がステップ成功として記録されて次のステップへ進んでいた。**
`wait_reached` の docstring は「既知の限界」としてこれを正しく書いていたが、
限界として残したままだった。

窓は数 ms だが、そこへ入るのは緊急停止そのものである。しかも症状は「E-STOP を
踏んだのにシーケンスの進捗だけが 1 つ先へ進む」で、解除後の再開位置が姿勢と食い違う。

**判断は呼び出し側へ移した。** `wait_reached(expect_target=True)` は「呼ぶ直前に
指令を送った」と呼び出し側が宣言する印で、立っていれば待ち始めた時点で目標が
無いことも中断として扱う。`has_target` を見て推測する形にはできない ——
まさに消えた場合と一度も指令していない場合を、その値だけでは区別できないためである
（後者は「待つ必要が無い」として True を返す既存の意図的な設計で、変えていない）。
本番の呼び出し元は `move_to` 1 つだけで、そこは必ず指令の直後である。

**この不具合は「実時間で狙うテスト」が隠していた。** 中断のテストは
`asyncio.sleep(0.01)` で緊急停止を狙っており、狙いが外れた回は別の経路
（指令前に消える → タイムアウト / 指令後・待機前に消える → この窓）へ落ちる。
CI で不定期に `assert ['move', 'after'] == ['move']` として現れ、再実行すると
緑になるので**フレークとして片付く形**をしていた。テストは到達判定のフックで
「待機中に消えた」を毎回同じ順序で起こす形へ張り替え、窓そのものを再現する
ケースを別に足した。**中断検出の 3 層（待機中・窓・run() の文言）はそれぞれ
単独で変異させて落ちることを確認してある。**

### PID ゲインの値も起動時に拒否する（2026-09-08）

`pid` はキー名だけを起動時に突き合わせ、**値の検査を `main._load_pid_config` の
「数値でなければ警告して既定値」に預けていた**。これは 2 通りに破れていた ——
yaml の `true` は `float()` を通って 1.0 として静かに効き、`.inf` / `.nan` も
`float()` が例外を投げないので**警告 0 件でそのまま採用される**（`kp: .inf` が
有効なゲインとして起動する）。

**PID ゲインを実行中に差し替える経路は持たない**という方針（CLAUDE.md「config に
書いた値がそのまま動いている値である」）を採っている以上、起動時のここが唯一の関門で、
通した値を後段で止める層はどこにも無い。`_parse_pid` から `_number()` を呼ぶ形にして、
bool・非数値型・NaN・無限大をまとめて弾く。**判定を書き写さず `_number` に任せる**のは、
しきい値側（`_parse_health`）だけを直したときにこちらが古いまま残るのを防ぐため。

`main._load_pid_config` 側の「警告して既定値へ倒す」手当ては削除した。**拒否する層と
黙って既定値へ倒す層が二重にあると、後から片方を外しても症状が出ない。**
`null`（書きかけの yaml）を許す性質だけは残す —— 機構調整中に 1 行コメントアウトした
だけで実機が起動しなくなると、動作確認そのものができない。

### 畳み込みの上限を確かめるテストが分岐へ到達していなかった（2026-09-08）

`_settle_pending_reenergize` / `_settle_pending_reactivation` は「キャンセルしてから
有界に待ち、待ちきれなくても先へ進む」形で前回の再励磁タスクを畳む。その上限を
確かめているつもりのテストは、**`asyncio.wait` の `timeout` を `None` にしても通って
いた**（実測 0.24s。分岐に一度も到達していない）。

根拠にしていた前提が事実と違った。docstring は「エグゼキュータのスレッドへ入った
`bus.send` は `Task.cancel()` では止まらず、完了するまで `CancelledError` が
投げ込まれない」と書いていたが、**裏のスレッドは走り続けるものの、待っている側の
Task は `cancel()` した瞬間に `CancelledError` を受け取る**（実測 0.000s）。
上限が実際に効くのは、畳もうとしているコルーチンが自ら `CancelledError` を握り潰して
`await` へ戻り続ける場合だけである。`lib/can_manager.py` の励磁・再励磁の鎖に
その箇所は無いので、**この上限は現状どこからも発火しない保険**である。保険だと
分かった上で残す —— 外すと「握り潰す箇所を 1 つ足した瞬間に解除が永久に返らなくなる」
形に戻る。

テストは `contextlib.suppress(asyncio.CancelledError)` で居座るコルーチンへ張り替えた。
**単発側と解除側は層が別なので 1 枚ずつ確かめる** —— 上限は同じ定数を共有するが、
片方だけ外してももう一方のテストは緑のままである。定数は
`_REENERGIZE_CANCEL_TIMEOUT_S` から `_PENDING_TASK_CANCEL_TIMEOUT_S` へ改名した
（2 系統で共有しているのに、単発の再励磁コマンド専用に見える名前だった）。
...
- コート差異は**位置の値だけ**を `{red:, blue:}` に切り替えられる形にした。
  ...
  **この方式の初の実使用が下の `conveyor.run`** で、実際に書き換えたのは 1 行だけだった。

### コート別位置定数の初の実使用: `conveyor` の搬送方向（2026-09-06）

**赤コートと青コートでコンベアの搬送方向が逆になる**という機構要件が判明した。duty は
`docs/motor_driver_can_protocol.md` §4 のとおり −1.0〜+1.0 で**符号がそのまま回転方向**
なので、コート別位置定数はこの 1 行で足りる:

```yaml
positions:
  conveyor:
    stop: 0.0                       # 停止 [duty]。0 に符号は無いのでコート非依存
    run: { red: 0.3, blue: -0.3 }   # 搬送 [duty]。符号が回転方向
```

**Python も TypeScript も `sequences/*.py` も 1 行も変えていない。** 値の供給経路は
シーケンス（`lib/sequence/engine.py` の `move_to`）/ 手動プリセット（`lib/manual.py`）/
動作確認（`lib/server_motor_check.py`）の 3 つだが、どれも既に `set_court` でコートを
受け取っており、解決は `PositionTable.raw(axis, name, court=)` が行う（両コートが揃って
いることの検証は `lib/sequence/positions.py` の `_parse_value()`）。**仕組みだけがあって
一度も使われていなかったものが、初めて実使用に入った。**

**なぜ位置定数で表すのか。** 採らなかった案は 2 つある。

- **`axes.conveyor.scale` の符号をコート別にする案。** `scale` は「人間の単位 → モータ
  指令値」の換算係数、つまり**機構が決める定数**であってコートには依存しない。ここへコートを
  持ち込むと、回転方向の表現が `axes`（換算）と `positions`（値）の 2 箇所に分かれる。
  単位換算と回転方向を 2 箇所に分けないという判断は逆回転ペア軸（`y_axis` / `rotate`）で
  既に置いてあり（回転の向きは `scale` の符号**だけ**が持つ）、コートを `scale` へ入れると
  その原則と正面から衝突する。分かれた瞬間に「片方だけ直したときに気付けない」状態が
  できる —— duty 軸は到達判定を持たない（DC 基板はフィードバックを一切持たない）ので、
  符号が 2 箇所で打ち消し合っても実機は黙って違う向きへ回るだけになる。
- **`sequences/main_hand.py` でコートを見て分岐する案。** 「`sequences/*.py` に数値を
  書いてはならない」に反する。機構が変わったら yaml の数値だけを差し替えられるという
  性質が消え、搬送 duty を変えるのに Python を編集することになる。

**`stop: 0.0` はコート別にしない。** 0 に符号は無いので、コート別に書いても 2 値とも
0.0 になる。`_parse_value()` は片方だけの記述を拒否する（両コート必須）ので、意味の無い
2 値を並べることはできてしまうが、そうすると **config を読んだ人が「コートで変わる値」と
「変わらない値」を区別できなくなる**。辞書で書いてあること自体が「ここはコートで変わる」の
表示になっている状態を保つ。

**コート依存は `conveyor` の搬送方向だけである。** `y_axis`（前後）と `rotate`（回転）の
位置はコートに依らず不変 —— これは 2026-09-06 時点で**機構側が結論として与えたもの**であり、
**検討した結果そう決めた**のであって未検討ではない。**根拠は機構の構成が決めるものなので、
この文書は結論だけを記録している**（同梱 config でコート別に書かれた位置も
`positions.conveyor.run` の 1 つだけである）。ここを書いておかないと、機構が変わるたびに
同じ問いが蒸し返され、しかも「まだ検討していない」のか「検討して不変と決めた」のかが
コードからは読めない。**機構が変わったらこの結論は再確認が要る。コート依存を足すときは、
この段落にその軸を追記すること。**

**指差喚呼 `conveyor_run` の文言を直した。** 動作確認シーケンス
（`sequences/motor_check.py` の `main_conveyor`）は `run` → `stop` の 1 往復しか回さない
ので、**確認できるのは選択中のコートの向きだけ**である。「正転・逆転の目視確認」のままだと、
1 度の動作確認で両方向を見たつもりになる。現在の label は「コンベアが選択中のコートの向きへ
回ること確認（逆コートは切替後に再確認）」で、両向きを見るならコートを切り替えて動作確認を
もう一度回す運用になる（`set_court` は `PHASES_OUTSIDE_MATCH` なので、これは準備中に
しかできない）。

**赤 = +0.3 / 青 = −0.3 は仮置きである。** どちらの向きが正転かはモータの配線と機構が
決めるもので、対になるファーム側の `firmware/dc_motor/include/config.h` の
`kDirForwardIsLow`（方向ピンの論理）も `TODO(実機で確認)` のまま。**逆だったら `red` と
`blue` の値を入れ替えるだけでよい。** 誤ったままでも機構は壊れず、両コートともワークを
反対向きへ送るという形でしか現れないので、**試合当日にしか症状が出ない**。棚卸しは
`docs/mechanism_handoff.md` §1 の `positions.conveyor.run` の行が持つ。

固定は `tests/test_robot_sequences.py::TestShippedPositionYaml::test_conveyor_run_is_reversed_between_courts`
で、**符号の関係（赤と青が異符号で、どちらも 0 でない）だけ**を見る。大きさ 0.3 は仮値
なので固定しない。このテストが要るのは、`run: 0.3` というスカラーへ戻す変更も両コートを
同符号にする変更も**他のどのテストも落とさずに通ってしまう**ため —— duty 軸は到達判定を
持たず（DC 基板はフィードバックを一切持たない）、シーケンスも位置定数の読み込みも値の
符号を見ない。
...
- **ただしこれは grid の話で、flex-col の子へ書き写すと幅が壊れる。** `align-self` が効くのは
  クロス軸で、行方向に並ぶ grid では縦だが、**flex-col では横**になる。flex-col の子に
  `self-start` を付けると幅が `stretch`（＝列の幅）から `fit-content` へ変わり、内容が幅を
  決めるようになる —— `fit-content` は `min(max-content, max(min-content, 使える幅))` なので、
  中身が短ければ列より縮み、min-content が列より広ければ列を越えて膨らむ。**flex-col の子は
  主軸（縦）にはそもそも伸びないので、高さを留める目的の `self-start` は 1 つも要らない。**
  実際に試合中の操縦者画面（`pages/RobotControl.tsx`）の右カラム（`flex min-h-0 flex-col`、
  幅 358px）でこの取り違えが起きていた。`MatchTimer` のパネルは中身が `0:00` の 1 行しか
  無いので **157px** まで縮み、font-size 3.4em の数字がパネル枠からはみ出した。逆に
  機体状態のパネル（`SubsystemStatus`）は min-content が列より広く **424px** へ膨らんで、
  右端がビューポートを 58px 越え、診断ツリーの TMP 列とヘルスチップが読めなくなっていた
  （症状が縮む側と溢れる側の両方に出るので、片方だけ見ても原因にたどり着きにくい）。
  再発は `pages/RobotControl.test.tsx` の「試合中の右カラム」が固定する ——
  右カラムの直下の子が `self-*` を 1 つも持たないことを見る。
- **幅を直したら、次は「縦が足りないときどちらが縮むか」を決める。** flex の既定
  （`flex-shrink: 1`）は列に入っているパネルを一律に縮めるので、右カラムでは機体状態が
  強制展開した瞬間に**試合時間パネルまで縮み、caption「残り時間」が font-size 3.4em の
  数字の下半分に重なって読めなくなる**（dry-run は「無励磁のまま rotate_l / rotate_r」を
  常に主張するので、この状態が既定になる）。削れる余地が無い側（試合時間。高さは中身で
  決まり切っている）に `shrink-0` を与え、**内部スクロールを持つ側（機体状態のモータ一覧）に
  溢れを引き受けさせる**。縮む側に必要な `min-h-0` は `Panel` が基底で持っているので
  呼び出し側では書かない。**`flex-1` は付けない** —— 中身が数行しかない平常時に全高の
  白い箱になり、上の「grid の子は…」で禁じている形そのものになる。
  幅の不変条件とは別のテスト（同 describe の「縦が足りないときは機体状態だけが縮む」）で
  固定してあり、`self-start` が戻る変異と `shrink-0` が消える変異は別々に落ちる。
- **`self-start` を外したら、列幅で縮めない要素が残っていないかを確かめる。** 幅を内容に
  合わせていた間は隠れていた溢れが、列幅へ固定した瞬間に見える。診断ツリーはほとんどが
  `min-w-0` + `truncate` なので収まるが、**`HealthIndicator` の CAN バス表だけは縮まない** ——
  `table-fixed` でない表の列幅は内容の min-content で決まるので `truncate` が効かず、しかも
  異常時のバッジ補足（`bus_off rx_down tx_err … rx_err …`）が `whitespace-nowrap` + `shrink-0`
  で、幅 21rem の診断カラムでは表だけが列を越える。表を `overflow-x-auto` のコンテナへ入れて
  そこでスクロールさせる（スクロールコンテナの min-content 寄与は 0 になるので、外側の
  レイアウトは押し広げられない）。**画面本体を横スクロールさせない**という約束の側で受ける。
...
#### 外周の整理と重複表示の削除（2026-09-07）

「レイアウト再設計（2026-08-26）」で立てた 4 原則のうち、**②同じ事実を 2 度描かない**が
守れていない箇所が実描画（1920×1080 / dry-run）で複数見つかった。加えて、画面の縦を
常時消費している帯（ヘッダー・モード帯・フッター）が、消費している縦のわりに何も
言っていなかった。原則は据え置きのまま、外周と重複表示だけを直す。

##### フッター（`StatusBar`）を廃止した

フッター 25px が持っていたのは 3 つ —— ショートカット凡例・接続表示（`WsSettings` への
入口）・時計。このうち**凡例は完全な重複だった**。`1 2 3` はタブ帯の各タブが `<Kbd>` で
既に描いており、`Space` は START ボタンと NEXT ボタンの**中に**入っている。つまり凡例は、
キーが効く場所から最も遠い画面下端で同じことをもう一度言っていた。
`StatusBar` は当時から凡例を `TABS[].hotkey` から描いており「割当の正は 1 箇所」は
守れていたが、**割当が 1 箇所でも表示が 2 箇所ある**ことは変わらない。

凡例を削り、残る接続表示と時計をヘッダー右端・EMG STOP の手前へ移して帯ごと畳んだ。
一般則としては **「ショートカットの凡例はそのキーが効く場所にしか置かない」** に落とす
（`CLAUDE.md` / 上記「キーボード操作」節）。離れた場所の一覧は、同じ事実を 2 度描くうえに、
割当を変えたときに一覧だけが古くなる。

**時計は `components/shell/Clock.tsx` として独立したコンポーネントのまま移した。**
これは整理ではなく必要条件で、毎秒 `setState` するものをヘッダー本体へインライン展開すると
**1 秒ごとにヘッダー全体（タブ帯を含む）が再描画される** —— context を購読頻度で 3 つに
分けた目的（`RobotContext`）と同じものを、コンポーネント境界の側で守っている。

##### ヘッダーはフェーズとコートを隣に置く

> **【並び順は翌日に変わった】** 下の 3 群の**順序**は現行ではない。押せるものを EMG STOP から
> 遠ざけるため `[タブ帯] … [接続][時計][フェーズ][コート]` へ組み替えた（下の
> 「EMG STOP の誤爆を防ぐ配置（2026-09-08）」節）。**「フェーズとコートは対」「接続と時刻は対」
> という群そのものと、詰まったときに削るのはタブ帯だけという性質は現行である。**

`AppHeader` はフェーズチップを左端、コートチップを右端（EMG STOP の手前）に置いており、
**間の 900px が空白**だった。どちらも「今どの試合設定・状態か」を表す対なのに、操縦者は
帯の両端を 2 回に分けて読むことになる。並びを「今どういう設定・状態か（フェーズ・コート）」
→「どこを見るか（タブ）」→「機体と繋がっているか（接続・時刻）」の 3 群に組み直し、
空いた中央を接続表示と時計の移設先に充てた。**帯の高さは 48px のまま**で、
フッターぶんの 25px が操作領域へ戻る。

同時に `flex-wrap` を外した。狭い画面で 2 段に折れると、タブを別の帯に分けない
（＝縦を 2 段消費しない）という判断そのものが無効になる。詰まったときに削ってよいのは
タブ帯だけなので、そこだけが縮み、他は `shrink-0`。

##### モード帯（`ModeSwitch`）を細帯へ

> **【切替ボタンの位置は翌日に変わった】** 下にある「右端に切替ボタン 1 つを置く」は現行では
> ない。右端は EMG STOP の真下なので、ボタンは帯の**先頭**へ移した（下の「EMG STOP の誤爆を
> 防ぐ配置（2026-09-08）」節）。**細帯にしたこと・`tabs tabs-box` をやめたこと・高さをモードで
> 変えないことは現行である。**

`ModeSwitch` は `Panel` の card 枠に `tabs tabs-box` を入れており、**52px を使って
「半自動 / 手動操縦」の 2 択と「シーケンス制御中」の 1 行しか出しておらず、右の 1340px は
空白**だった。枠と本文余白ぶんの縦を、1 行の事実に払う理由が無い。罫線 1 本と左端の
アクセントバーだけの帯（30px 前後）に改め、左に現在モードの状態チップ、右端に切替ボタン
1 つを置く。手動中に帯そのものを警告色にする方針は維持する（Monitor 側にも同じチップが
出るという対も維持）。

**`tabs tabs-box` をやめたのが本題である。** ヘッダーのタブ帯とまったく同じ見た目の
タブ群が上下 2 段に並んでいて、**「見る場所を変える操作」と「機体の制御権を奪う操作」が
同じ形をしていた**。現在モードは状態チップで示し、切替は行き先を書いたボタン 1 つに絞る。
これは `CLAUDE.md` の規則（モード帯とヘッダーのタブ帯は見た目を分ける）として残す。

- 「シーケンス制御中」の文字は削除した。選択中を示す状態チップと同義で、同じ帯の中で
  2 度言っていた。
- 準備フェーズにあった「シーケンス」パネル（シーケンス名 + 総ステップ数の 1 行）を
  廃し、この帯へ畳んだ。**総ステップ数は準備フェーズだけ**出す —— 試合中は
  `ActionPanel` が `1/22` の形で同じ数を持っているため。
- 高さはモードで変えない。変えると下の主操作（`ActionPanel`）が上下にずれ、
  「主操作は状態によって位置を動かさない」が帯の側から破られる。

##### `Hand` アイコンの二義を解消した

`Hand` が「許可待ち / ここで停止」（`ActionPanel` / `SequenceStepList` /
`RobotStatusRow`）と「手動操縦」（`ModeSwitch`）の 2 つの意味で使われており、**試合中に
手のアイコンを見てもトリガー待ちなのか手動操縦なのか区別できなかった**。アイコンは文字と
違って読まずに形で引くので、1 つの記号が 2 つの状態を指すと、区別が付いた気になったまま
取り違える。`Hand` は前者専用に固定し、手動操縦は `SlidersHorizontal` にした。

同じ理由で NEXT のアイコンを `Play` から `ArrowRight` へ変えた。START と NEXT は
`ActionPanel` の**同じ位置に同じ大きさで交互に出る**ので、記号まで同じだと色と文字で
しか見分けられない。START は先頭から走り直す操作で、押し間違えると中断姿勢のまま
全工程が流れる。

##### 同じ事実の二重描画を削った

| 消したもの | 同じ事実を既に描いていた場所 |
|---|---|
| `ActionPanel` ヘッダー行の `4/22` | すぐ下の巨大表示（現在番号）。総数はそちらへ小さく添えた |
| 「NEXT で走る範囲」の最大 4 行リスト | すぐ下のステップ一覧。同じ「次に走るステップ群」を並べていた |
| `MatchTimer` 進行中の caption「残り時間」 | legend「試合時間」 |
| `MatchStrip` の「MATCH {コート名}」 | ヘッダーのフェーズチップとコートチップ（常時表示） |
| ステップ一覧の「クリックで再開」 | 押せば分かること（塞がれている理由だけを残す） |

**削ったのは重複だけで、別の事実は残す。**

- 「NEXT で走る範囲」は 1 行へ圧縮したが、**「N ステップ走って「X」で停止」と
  「残り N ステップを最後まで走り切ります (途中で止まりません)」を同じ文言にしては
  ならない**。後者は「押したら止まらない」という別の事実で、操縦者が身構える範囲が
  変わる。この不変条件は圧縮の前後で変わっていない。
- `MatchTimer` の caption は進行中だけ消した。停止中の「開始前」「試合終了時点の残り」は
  legend からは読めない —— **同じ `0:30` でも「まだ始まっていない」のか「その残りを
  抱えて終わった」のかで意味が正反対**になるので、必ず出す。
- `Dashboard` の legend「機体状態 — どのバス・どのモータか」は「機体状態」へ。
  同じ画面の他の legend が名詞だけなので、1 つだけ説明文が付いていた。

##### 実測（1920×1080 / dry-run）

| | 前 | 後 |
|---|---|---|
| フッター | 25px | 廃止 |
| ヘッダー | 48px（フェーズとコートの間が 900px の空白） | 48px（接続・時刻を吸収） |
| モード帯 | 52px（右 1340px が空白） | 30px 前後 |

右カラムの溢れ（試合時間 157px / 機体状態 424px / ビューポート 58px 超過）は同じ日の
別件で、原因と手当ては「レイアウト再設計」の「実装上の注意」に追記してある。

#### EMG STOP の誤爆を防ぐ配置（2026-09-08）

前節でフッターを畳んで接続表示と時計をヘッダー右端へ寄せた結果、**画面右上に押せるものが
集まった**。加えて EMG STOP のアイコンは `alert-blink` で常時点滅していた。操縦者からの
指摘は 2 点 ——「点滅させないでほしい」「EMG STOP と手動操縦の切替ボタンが近すぎて誤操作を
誘発する」。

**誤爆の向きは「隣のボタンを押そうとして EMG STOP を踏む」である。** 逆ではない。
EMG STOP は赤地・大面積で常に同じ位置にあり、狙って外すのはむしろ隣の小さいボタンの側で、
外した先が緊急停止になっている。試合中にこれを踏むと、機体は止まりシーケンスは降りる。

##### 全画面を洗った結果

| 場所 | 押せる要素 | EMG STOP からの隔たり |
|---|---|---|
| `ModeSwitch` | 手動操縦へ / 半自動へ戻る | **`-mx-2 -mt-2` でヘッダーへ密着。縦 2.4px・横ほぼ 0** |
| `MatchStrip`（Monitor 試合中） | 試合終了 / セッティングへ戻る | Page の `p-2` + 帯の `px-2` で右 16px・下 12px |
| `StartGate`（Monitor 準備中） | START | Page の `p-2` + Panel の `p-3` で右 20px・下 20px |
| `AppHeader` | 接続表示（`WsSettings` の入口） | 同じ帯の内側。押せない時計が挟まる |

`MatchPrep` の RESET と DEV 全チェックは左カラムのパネル見出し行、`ActionPanel` の主操作は
左カラムのパネル最下段で、どちらも画面右上には来ない。**`ModeSwitch` だけが桁違いに近い。**

##### 直したこと

**1. ヘッダーの EMG STOP は点滅させない。** 赤地・大面積・固定位置・常時表示なので、
点滅を外しても発見性は落ちない。`alert-blink` は**異常時にだけ画面へ出る要素に限る**規約に
した（残る利用者は `EStopOverlay` = 緊急停止発動中と `ConnectionBanner` = 切断中で、
どちらも平常時には描かれない）。平常時に点滅している要素は画面でここだけで、
「平常時に静かで、異常時に自分から主張する」に正面から反していた。

**2. EMG STOP の周囲に押下可能な要素を置かない。** ヘッダー直下に来る帯の操作ボタンは
左寄せに固定し、右端には押せないもの（塞がれている理由・時計・状態チップ）だけを置く。
**適用範囲は「ヘッダー直下の最上段」と「画面全幅の要素の右端」に限る** —— 画面上の
あらゆるボタンを左へ寄せる話ではない。`ActionPanel` の「左が STOP・右が主操作」と
「主操作は状態によって位置を動かさない」は EMG STOP から縦に遠いので従来どおりである。

**3. ヘッダーの並びを組み替えた。** `[フェーズ][コート][タブ帯] … [接続][時計]` から
`[タブ帯] … [接続][時計][フェーズ][コート]` へ。押せるもの（タブ帯・接続表示）を EMG STOP
から遠い側へ、押せないもの（フェーズ・コート・時計）を EMG STOP 側へ寄せる。
タブ帯を最左に置くのは、**画面の隅がポインタで最も当てやすい位置**であり、かつ EMG STOP から
最も遠いため。前節で立てた「フェーズとコートは対」「接続と時刻は対」はどちらも保っており、
「帯が詰まったときに削ってよいのはタブ帯だけ」という性質も変えていない。

**4. EMG STOP の手前に明示的な余白を置く。** 装飾ではなく緩衝なので**詰めてはならない**。

**5. `ModeSwitch` と `MatchStrip` のボタンを帯の先頭へ移した。** `ModeSwitch` で
現在モードのチップの隣ではなく先頭に固定するのは、**チップの文言長がモードで変わる**
（「手動操縦中 — シーケンスは停止しています」と「半自動」）ためで、隣に置くとボタンの
横位置がモードで動き「主操作は状態によって位置を動かさない」が破れる。右端に残すのは
塞がれている理由のテキストだけで、押せないので EMG STOP の真下でよい。
`MatchStrip` は二度押し武装中の説明文をボタンの右へ移した —— 左に残したまま左寄せにすると、
**武装のたびにボタンが横へずれる**（説明を左に置いていたのは、右寄せのときに
「ボタンの左端を動かさない」ためだったので、寄せる向きが変われば説明の側も入れ替わる）。

**6. `StartGate` は変更していない。** 20px 離れており、かつ二度押し武装（`useArmedPress`）を
持つため、ここまでの理由がそのままは当てはまらない。

##### 採らなかった案

- **EMG STOP の真下だけを空ける**（ヘッダーの EMG STOP に固定幅を与え、下の帯へ同じ幅の
  右パディングを入れる）。「右端はボタンの定位置」という当時の判断を保てるが、`AppHeader` と
  各帯にまたがる**幅の対**ができる。片方だけ動かすと緩衝が黙って消え、症状は
  「いつのまにかまた近い」だけになる。
- **切替ボタンを二度押し武装にする。** 誤爆は防げるが**近すぎること自体は直らない**。
  二度押しは「計時の開始点」と「残り時間との勝負の最後」の 2 つに限る方針（`CLAUDE.md`）
  からの逸脱でもある。
...
#### 動作確認の進捗パネルをモーダルからインライン展開へ

**症状: アクチュエータ動作確認の実行中に緊急停止が押せない。** `MotorCheckPanel` は
`Modal` で描かれており、しかも起動確認ダイアログの「開始」がそのまま
`onPanelOpen()` を呼んで自動で開いていた。`.modal` は全画面の fixed オーバーレイなので、
**両ハンドの全アクチュエータが順に駆動されているあいだずっと、ヘッダーの EMG STOP が
オーバーレイに覆われてクリックできない**（クリックは背景として吸われ、パネルが閉じる
だけ）。加えて `ModalProvider` の `openCount > 0` がホットキーも封じる。起動確認
ダイアログに書いてある「実行中も緊急停止 (EMG STOP) は即時優先で動作します」は、
そのあいだ嘘になっていた。

**止める手段だけが画面から消える**という壊れ方で、しかも操縦者から見ると「押しても
効かない」ではなく「押したらパネルが閉じた」なので、緊急停止そのものが壊れていると
読める。

変えたのは配置だけで、判定も配信も触っていない。

| | 変更前 | 変更後 |
|---|---|---|
| 進捗・結果 | `Modal`（Dashboard が `checkPanelOpen` で開閉、起動と同時に自動で開く） | `MatchPrep` の「アクチュエータ動作確認」区分にインライン展開 |
| 起動前の確認 | `Modal`（`MotorCheckButton`） | **そのまま**。機体が動く前に閉じるので駆動と重ならない |
| 開閉 | 「進捗を表示」ボタンでモーダルを開く | 区分内の開閉（`手順と結果`）。実行中・失敗時は操縦者の操作より優先して開く |
| 中断 | モーダルのフッタ | 開閉の外側（畳んだ状態でも同じ位置に出る） |
| 起動 / 再実行 | モーダルのフッタにも「実行 / もう一度実行」 | `MotorCheckButton` の 1 つだけ（インライン化で同じ区分に並ぶため、置くと同じ操作が 2 つ並ぶ） |
| 進み具合・失敗理由 | `MotorCheckSummary`（区分見出し・truncate）とモーダルの両方 | パネルだけ。`MotorCheckSummary` は状態チップ 1 語へ縮めた |

強制開示の反転は `setManualOpen(!open)` と書く。`(v) => !v` にすると、強制開示中に
押したぶんが内部状態だけ「開く」へ倒れ、**実行が終わった後もステップ 15 行が指差喚呼の
上に居座り続ける**（`SubsystemStatus` と同じ罠）。

変異テストで確認した層:

| 壊した 1 行 | 落ちるテスト |
|---|---|
| 根の `div` に `modal` クラスを足す（モーダルへの逆戻り） | `MotorCheckPanel.test.tsx`「画面を覆わない (モーダルではない)」 |
| `forcedOpen` を `false` に固定 | 同「実行中は畳んでいても自分から開き…」「中断・失敗の理由は畳んでいても…」 |
| 開閉の反転を `(v) => !v` へ | 同「実行が終わったら畳んだ状態へ戻る」 |
| `MatchPrep` から `<MotorCheckPanel />` を削除 | `MatchPrep.test.tsx`「動作確認の進捗と結果もその場で開く」 |
| `MotorCheckSummary` に失敗理由を戻す | `MotorCheckSummary.test.tsx`「途中で降りたら未完了と出す」 |
...
1. **探索の到達判定をラッチにした**（当時は `GenericDriver.consume_sensor_latch`）。
   `update_state` が受信のたびに OR で積み、`HomingRunner` が読んで消す。
   **既存の `sensor_active`（今の状態）はそのまま残す** —— 診断ツリーが描いているのは
   「今どうなっているか」で、そちらはこれが正しい。
   **【現行と違う】読み手が 2 人になった 2026-09-09 に、この「読むと消える」ラッチは
   単調増加のカウンタ（`sensor_contact_count`）へ作り替えた**（下の「リミットスイッチ
   保護」節）。判定の中身 —— 「今 ON か」ではなく通過を拾う —— はそのままで、
   読み手が自分で基準値を控えてその差を見る形に変わっただけである
...
> **【この 1 段落は 2026-09-09 に覆った】** 「読み手を 1 つに限る」で通せたのは、
> 接触を読むのが零点確定だけだった間である。リミットスイッチ保護（`LimitGuard`）が
> 2 人目の読み手になった時点で、**先に読んだ側が相手のぶんまで消す**という上の性質
> そのものが、この節が直したはずの壊れ方（当たっているのに止まらない）を再発させる
> 側に回った。**読み手を制限するのではなくラッチをやめ**、立ち上がり（OFF→ON）を数える
> 単調増加のカウンタ（`GenericDriver.sensor_contact_count`。読んでも減らない）へ
> 作り替えて、読み手はそれぞれ自分で基準値を控えその差だけを見る形にした
> （`consume_sensor_latch` は削除済みで、復活させてはならない）。
> **「探索を始める直前に捨てる」は「直前に基準値を取り直す」へ読み替える** ——
> 目的も効果も同じである。詳細は「リミットスイッチ保護」節。
...
- **見ているのは `motors` だけ。** ファームはセンサスロットも `INFO` を送る
  （仕様書 §5.2）が、この時点では `main.py` がセンサを `expected_firmware` なしに
  生成するので照合対象そのものが無い。`sensors:` に `expected_firmware` を書けるように
  する日には `ctx.can_manager.sensors` も見ること（`_firmware_unconfirmed_motors`
  の docstring に同じ断りを置いてある）
  → **2026-09-08 に実施済み**（後述「サーボ基板 3 枚 15 スロットの割当を確定させた」）。
  駆動するモータが 1 台も無い基板ができたため、この予告のまま置ける状態でなくなった
...
### サーボ基板 3 枚目（UNO R4 Minima）（2026-09-07）

**サーボ基板を 1 枚増やしたが、3 枚目だけ基板仕様が違う。** #0/#1 は Arduino Nano
（ATmega328P + MCP2515 の SPI 外付け）だが、**#2 は UNO R4 Minima（Renesas RA4M1）で
CAN は R4 内蔵ペリフェラル**（`D4`=TX / `D5`=RX 固定、`Arduino_CAN`）。MCP2515 は
載っていない。サーボ 5 本のピンも別物で **SV0=D9 / SV1=D11 / SV2=D10 / SV3=D6 /
SV4=D3**（#0/#1 は SV0〜SV4 = D4〜D8）、RGB LED は **D8**（#0/#1 は D9。#2 では D9 が
サーボに要る）。DIP は 3 枚とも A0〜A3 の 4bit で、番号（14〜17）まで一致する。

基板番号が 2 なのでデバイス ID は `0x50`〜`0x54`。**現時点で各スロットの用途は未定**
なので 5 スロットとも `SlotRole::Unused` で、PC 側 `config/*.yaml` にはモータもセンサも
1 つも登録していない。したがって**この基板は `FEEDBACK` も `INFO` も 1 通も送らず、
PC からは繋がっていないのと区別が付かない** —— 運用（`docs/venue_recovery.md` /
`docs/checks_and_health.md`）の側は何も変わらない。

#### 別プロジェクトを作らず、`firmware/servo/` に env を足した（1 プロジェクト 2 env）

素直な形は `firmware/servo_r4/` の新設だが、**採ると `config.h` が 2 つになる。**
...
`firmware/servo/platformio.ini` に env を 2 つ（`nano` / `uno_r4_minima`）持たせれば、
`config.h` は 1 つのまま・`kFirmwareVersion` も 1 つのままで済む。
**`default_envs` は両方を並べてある** —— 片方だけをビルドして満足すると、もう一方が
壊れたことに気付けない（ピンの表も静的検査も MCU ごとに違う分岐を通る）。
env ごとの `build_src_filter` が担当しない CAN バックエンドの翻訳単位を外し
（Nano ビルドから `can_r4.cpp`、R4 ビルドから `can_mcp2515.cpp`）、`lib_deps` も
分けてある —— **R4 側に `mcp_can` を残さない**のは、載っていない MCP2515 の構成でも
動くように見える誤解を残さないため。

#### 「同じファームを全基板へ焼く」は「MCU ごとに 1 種類のバイナリ」へ緩めた

「サーボ基板のスロット設定を基板番号ごとに持たせる（2026-09-05）」の節が置いていた
前提 —— **全基板へ同じファームを焼く** —— は、この改修で緩んでいる。

その前提が守っていたのは「基板ごとに違うバイナリを許すと、どれが正しいのか照合できない」
だった。**MCU が違うバイナリはその心配をしない** —— Nano 用（`platform = atmelavr` /
`board = nanoatmega328`）を R4 へ物理的に焼けないので、焼き間違いという事故そのものが
起きない。**焼き分けの判断が人ではなくハードウェアの側にある**のが、`config.h` を 2 つに
割る案との決定的な違いである（あちらは同じ MCU に 2 種類のバイナリを許す形だった）。

代償は**実機ビルドが 1 つ増えたこと**。サーボ基板は
`pio run -e nano -d firmware/servo` と `pio run -e uno_r4_minima -d firmware/servo` の
**2 つとも通す**必要がある（`main.cpp` を共有しているので、片方だけ通しても壊れているのは
常にもう片方）。native テスト（`pio test -e native`）は `MotorCan` が Arduino 非依存
なので従来どおりどちらか一方で足りる。

#### スロット表は「行の添字 = 基板番号」から「基板番号を明示して持つ並び」へ

`kSlotsByBoard[基板][スロット]` を `kServoBoards[]`（`ServoBoardConfig` = 基板番号 +
スロット 5 個）へ変えた。**各バイナリは自分が担当する基板の要素だけを持つ**
（Nano 用は #0/#1、R4 用は #2）。

**理由はピンの静的検査である。** 1 つの表に 3 枚ぶんを並べると、R4 ビルドの
`static_assert` は Nano 行の `D4`/`D5` を**内蔵 CAN ピンとの衝突**と読み、Nano ビルドは
R4 行の `D9` を**RGB LED との衝突**と読む。どちらも実在しない衝突なので、通すには
検査そのものを緩めるしかない —— 「`config.h` の想定と実基板の配線が食い違ってもビルドが
通った」という過去（`DIS` のつもりの `D7` が実機では ch2 の方向ピンだった件）へ戻ることになる。
担当外の基板を表に**載せない**形にすれば、検査は張ったまま分けられる。

**安全側の倒れ方は変えていない。** 「表に無い基板番号は全スロット `Unused`（ID を
名乗らず駆動もしない）」はそのままで、担当外の基板番号もこの規則にそのまま乗る
（DIP を 8 以上へ回したときと同じ扱い）。`SlotRole::Unused` が enum の 0 であること、
ゼロ埋めの `pin = 0` を UART の予約で捕まえることも変わらない —— ただし**固定ピン表は
MCU ごとに中身が違う**（Nano は MCP2515 の CS/INT と SPI、R4 は内蔵 CAN の D4/D5）ので、
検査の網はそのバイナリが担当する基板ぶんしか張られない。

#### `kFirmwareVersion` は 5 のまま上げていない

**版番号が答える問いは「この基板に焼き直しが要るか」だけである。** 基板 #0/#1 の
CAN 上の振る舞いは 1 ビットも変わっていない（プロトコルもピン配置も役割表も同じ）ので、
上げる理由が無い。**新設の R4 バイナリも同じ v5 を名乗る** —— CAN 上の振る舞いが
同一である以上、区別する意味が無い。

上げた場合に起きるのは、**Nano 2 枚の無意味な再書き込みと `config/**/*.yaml` の
`expected_firmware` 26 箇所の更新**である。しかも揃え損ねれば
「正しく焼いたのに全モータ FAULT」になる（§3.4）。実害だけがあって得るものが無く、
これを何度かやると版番号そのものが信用されなくなる。

**次に触る人がこれを「上げ忘れ」と読まないよう、仕様書 §3.4 に基準を書き足した** ——
『上げるのは既に焼いてある基板の CAN 上の振る舞いが変わるときだけ』。
逆に、**基板 #2 の用途が決まってスロットの役割やピンが変わるときは上げること**
（そのときは実際に振る舞いが変わる）。

#### CAN の扱いは `can_backend.h` の薄いインタフェースで分けた

`firmware/servo/src/can_backend.h` が `begin` / `send` / `poll` だけを宣言し、
`can_mcp2515.cpp`（Nano）と `can_r4.cpp`（UNO R4）が実装する。**`main.cpp` は MCU に
依らず 1 コピーのまま**にした —— `main.cpp` を `#if` で割ると、安全機構とスロット処理が
2 通りに分かれて片方だけが古くなり、しかも症状はその MCU の基板にしか出ない。

**R4 の `Arduino_CAN` には受信フィルタの API が無い**ので、#2 にはバス上の全フレームが
上がってくる。宛先判定は `routeFrame` が行い自分宛でないものはそこで捨てられる ——
**DC 基板（同じ R4 内蔵 CAN）で実績のある形**なので、新しい risk を負っていない。
#0/#1 の MCP2515 でフィルタを張っているのは「バスから全部を SPI で読み出して捨てると
受信バッファ 2 段が溢れる」というハード固有の事情への対処であって、R4 には当たらない。

#### 「送信バッファの本数は 3 枚で違う」がサーボ基板の内部にも現れた

**この改修の途中で、`servo/src/main.cpp` の `loop()` が `INFO` を 5 スロット分まとめて
同じ反復で送っていることが見つかった。** 既存章「送信バッファの本数は 3 枚で違う」が
DC 基板・電磁弁基板で潰した欠陥と同じもので、当時サーボが**無症状だった理由は
MCP2515 が TX 3 本 + ライブラリが空きを待つから**でしかない。#2 の `Arduino_CAN` は
**標準 ID の mailbox を 1 本しか使わない**（`R7FA4M1_CAN.cpp` の `write()` が
`CAN_MAILBOX_ID_0` 固定）ので 2 通目以降が必ず落ち、`kInfoIntervalMs`(1000) が
`feedback_interval_ms`(10) の整数倍で位相が固定されるため**毎回同じスロットだけが出て
残り 4 本は永久に 1 通も出ない**。落ちた `INFO` は FAULT にならず、そのスロットの
**焼き忘れ検出（§3.4）が黙って無効になる**だけなので、症状から原因へ辿る手段が無い。

**つまり「基板を 1 枚足したら既存の潜在欠陥が顕在化した」のではなく、同じ欠陥が
ずっとそこにあり、ハードの余裕に隠されていた。** 「同じ送信コードが基板ごとに違う
壊れ方をする」という既存章の主張が、**同じプロジェクトの中でも成立した**ことになる。

修正は DC 基板・電磁弁基板と同じ「**1 反復 1 通・落ちたスロットは添字を進めず次の反復で
送り直す**」（`loop()` の `g_infoPendingSlot`）へ揃える形で、**`#if` による MCU 分岐は
入れない** —— 1 反復 1 通は 3 枚に共通の規則で Nano でも成立し、PC から見える振る舞い
（1Hz・内容・CAN ID）は変わらない（5 通が数反復に散るだけで、むしろ `sendMsgBuf` の
空き待ちで `loop()` が伸びる最悪値は縮む）。分岐を入れると**片方の MCU だけ直した状態**が
作れてしまい、それこそがこの規則の戒めているものである。**`kFirmwareVersion` も上げない**
（PC から観測できる CAN 上の振る舞いは変わっていない。上の「版番号を上げていない」節と
同じ基準）。**この層は native テストで守れない**（mailbox の busy は実機の CAN
ペリフェラルでしか再現しない）ので、送信まわりを触ったら実機で `candump` すること。

### 起動・実行時ログの整理（2026-09-08）

**要求は「制御プログラムのログが見にくい。不要なものは削除したい。特に Web サーバーの
ログは要らない」。** 読みたいのは CAN とシーケンスのログなのに、それが定型部と HTTP の
アクセスログに埋もれていた。実測は 4 つ。

- **1 行の定型部だけで 60 桁を超えていた。** `logging.basicConfig` の既定書式では
  `2026-09-08 09:20:45,337 [INFO] lib.control.position_loop:` まで出る。1 回の起動を
  追うのに日付は一度も要らず、ロガーのパッケージパスは「どこにあるか」であって
  「何が言っているか」ではない。そこへ 60 桁払うので、本文は端末の右端へ押し出される
- **アクセスログがリロード 1 回で数十行。** `web/dist` は SPA なので、index.html /
  JS / CSS / 自己ホストしたフォントまでが 1 回の再読み込みで並ぶ。操縦者が試合前に
  画面を開き直すたびに、その直前の CAN・シーケンスのログが画面外へ流れていく
- **ステップのログが `@step` のラベルと 2 箇所に分かれていた。** `sequences/*.py` の
  各ステップ本体に `logger.info("[main_hand] …")` が **44 行**あり、中身は
  デコレータのラベルの書き写しだった。**そして実際にずれていた** ——
  `@step("ハンド閉じる (受け取り)")` の本体は `[sub_hand] ハンド閉じる`、
  `@step("ワーク解放 (配置)")` の本体は `[sub_hand] ワーク解放`。**ラベルは UI に、
  写しは journal にしか出ない**ので、食い違いはログを読む人にしか見えず、その人は
  UI 側のラベルを知らない。書き写しである以上、次に誰かがラベルを直せば同じことが
  また起きる
- **起動ログの最長行が 300 桁を超えていた。** `ロボット登録` が位置制御ループ・
  位置定数軸・同期監視・目標値再送・手動連続操作の名前を Python のリスト repr の
  まま 5 組並べていた。しきい値は dataclass の repr
  （`HealthThresholds(feedback_timeout_ms=500.0, …)`）、指差喚呼は dict の repr
  （`{'pre_match': 27}`）で、**確かめたい数値がフィールド名に埋もれていた**。
  起動ログは試合前点検で目視する対象（「試合時間は当日ルールで変わりうる」）なので、
  値が読めない形で出すのは出していないのとあまり変わらない

#### ① 体裁の単一情報源を `lib/logging_setup.py` に置いた

`configure_logging(level, *, stream)` がルートへハンドラを **1 本だけ**張る（冪等）。
書式は `HH:MM:SS.mmm` + 5 桁固定のレベル略称 + 14 桁固定の短縮ロガー名
（`lib.control.position_loop` → `position_loop`、`__main__` → `main`）+ 本文。
**幅を固定することが目的**で、揃っていれば読み手は左の 25 桁を視線で飛ばして本文だけを
追える（欄幅 14 は実際に出る最長の `target_refresh` に合わせてある。1 つでも溢れると
そこから右が行ごとにずれ、列で読めなくなる）。

**`logging.basicConfig` へ戻してはならない。** あれは既にハンドラが 1 本でもあると
**黙って何もしない**ので、ライブラリが先に張っていると体裁の指定ごと無視される ——
例外も警告も出ないまま「効いているつもりで効いていない」が成立する。
`configure_logging` は既存のハンドラを外してから張るのでその形にならない
（外すだけで `close()` はしない。呼び出し元が渡したストリームを閉じてしまうため)。

`JOURNAL_STREAM` が環境にあれば時刻を出さない（systemd 配下でだけ立つ変数で、
journald は行ごとに自前のタイムスタンプを付けるから、出すと 1 行に時刻が 2 つ並ぶ）。
色は **TTY かつ非 journal かつ `NO_COLOR` 未設定**のときだけ付き、時刻とロガー名を
dim へ沈め、レベルだけを着色し、**本文には一切色を付けない** —— 色は「どこを飛ばして
よいか」の目印であって装飾ではないので、読みたいものへ色を乗せると読む対象がぶれる。
同じ理由で INFO のレベル名も無色にしてある（平常の行が色付きだと、色が付いていること
自体が異常の目印にならなくなる）。

#### ② アクセスログは出さない（二重の歯止め）

`RobotServer.start()` の `web.AppRunner(app, access_log=None)` と、
`configure_logging` 内で `aiohttp.access` を WARNING へ落とす 1 行。
**HTTP が通ったかどうかは UI の接続表示に出る**ので、ログで追う価値がほとんど無い
一方、押し流される側の損害は大きい。二重にしてあるのは、`AppRunner` を別経路で
作られたときにも止めるため（片方だけだと、経路を 1 つ足した瞬間に静かに戻る）。

#### ③ `main.py` に `--log-level`（debug|info|warning|error、既定 info）

順序は **引数解析 → `configure_logging` → 停止シグナル登録**。引数解析を前へ出しても
失われる行は無い（引数解析自体はログを出さない）ので、1 行目からレベルが効く。
DEBUG で増えるのは主に下の ⑤ で落とした名前の一覧で、「config を読むより速い」以上の
役割は持たせていない。

#### ④ シーケンスのステップログはエンジンが 1 箇所で出す

`sequences/*.py` の 44 行を全部消し、`lib/sequence/engine.py` が
`[main_hand] 2/22 自陣ワーク 3 列目まで前進` の形で 1 行出す。番号と総数を添えるのは、
「今どのステップか」だけでは進み具合が読めないため。**`sequences/*.py` の各ステップへ
`logger.info` を書き戻してはならない** —— 書き戻した時点で `@step` のラベルと同じ
文字列が 2 箇所に増え、上に書いたずれが再発する。

**トリガー待ちを抜けてから出す。** 待つ前に出すと、操縦者の許可を待っている間ずっと
そのステップが実行中に見え、journal の並びが実際の実行順と食い違う（`require_trigger`
のステップは常に止まるので、これは例外的な状況ではなく毎回起きる）。

完走したときだけ `[main_hand] 完走 (22 ステップ)` を足す。**通常停止と例外では出さない**
—— 前者は `lib/server.py` の `sequence_stop` が、後者は直上の `logger.exception` が
既に記録しているので、ここでも書くと同じ事実が 2 度並ぶ。

#### ⑤ 起動ログから repr を追放した

**要点は「1 行に 1 つの事実」で、名前の一覧は要約と DEBUG に分けたこと。**

| 変えた行 | 変えた内容 |
|---|---|
| しきい値 | dataclass の repr → `しきい値: 途絶 500ms / 温度 警告 65℃ 異常 80℃ / TX エラー 96` |
| 指差喚呼 | dict の repr → `指差喚呼: 27 項目`。ロールが複数あるときだけロール名と件数を並べる（ロールが 1 つでも辞書のまま運ぶ理由は「指差喚呼はロール 1 つに統合」の項） |
| `ロボット登録` | INFO は要約（モータ数 / 軸数 / 位置制御ループ / 同期監視）。位置定数軸・目標値再送・手動連続操作の**名前の一覧は DEBUG** へ。**同期監視の対象名だけは INFO に残す** —— 左右ペアの保護が実際に何へ掛かったかは、機構を壊す前に起動時点で確かめたい |
| 台形プロファイル | モータごと → **軸ごと 1 行**（下の「落としてはいけないもの」） |
| `E-STOP 送信試行完了` | ロボットごと 1 行 → 全機ぶんを 1 行へ。**「試行」は落とさない** —— このループは送信失敗を握ったまま先へ進むので、1 通も届いていなくてもこの行は出る。「完了」と書くと緊急停止が実際に効いたと読めてしまう |
| `サーバー起動` | dry-run のときだけ ` (dry-run)` を添える。**dry-run はここでしか名乗らない** —— 擬似値が出るので画面からは判別できず、会場で「繋がるのに機体が動かない」を切り分ける最初の 1 行になる |
| 終了時の `終了` | 削除。`KeyboardInterrupt` を捕まえて出していたが、直前に `後始末完了` が必ず出ており、同じ事実（畳み終わった）が 2 行になる。消すならこちらである（下） |
| 数値 | `%g` で `.0` を落とす（`180.0 秒` → `180 秒`）。config は小数で書けるが実際に入るのはほぼ整数で、`.0` が並ぶと読み手が桁を数え直すことになる |

#### 落としてはいけないもの

ログを減らす作業なので、**減らしてはならない行を先に決めた。** どれも「無くても
平常時は何も困らないが、困ったときにそこにしか無い」種類の行である。

| 残す行 | 落とすとどう見えるか |
|---|---|
| 台形プロファイルの 3 値（`max_velocity` / `max_acceleration` / **`velocity_ff`**）とモータ名 | **実行中に変更できず UI にも配信されないので、起動ログが「今どの値で動いているか」を知る唯一の経路**である（「`motion.velocity_ff` は `pid.kd` と同値に保つ」の項）。特に `velocity_ff` は 2 つの yaml にまたがる対の片側で、崩れたときの症状は「飽和率だけ上がって速くならない」だけ。軸ごと 1 行へ畳んだのは、制限値が軸の持ち物で左右直結ペアでは同じ 3 値がモータの数だけ並ぶからで、**適用先を知る手掛かりであるモータ名は行に残してある** |
| 動作確認の除外ステップ WARNING（`統合動作確認: ステップ '…' を除外 (構成に無い軸: …)`） | 「構成に無い軸のステップを除外する」の項が要求する 2 つの出口のうちの片方。画面を開かずに構成の食い違いへ気付ける唯一の経路で、落とすと本番構成で 1 軸が config から漏れていてもそのステップごと消えて全ステップ PASSED になる |
| 零点確定の手段が無い軸の ERROR（`零点確定: 軸 … は原点を確定する手段がありません`） | 押した瞬間に失敗する構成のまま試合当日を迎えないための行。`sub_y_axis` / `sub_lift` は**スイッチを付けてもここで落ちる**ので、この 1 行が無いと「動作確認だけがいつも失敗する」理由がどこからも読めない |
| `後始末完了` | 後始末が**完走した**証跡。無いと `TimeoutStopSec` 超過の SIGKILL と判別できない（畳めずに殺されたのか、畳み終わったのかは、この行の有無でしか分からない）。終了時の 2 行のうちこちらを残したのはそのため |

#### やらなかったこと

- **WebSocket の接続 / 切断ログ（`WebSocket 接続: <remote>` / `WebSocket 切断: …`）は
  残した。** HTTP のアクセスログと表面上は似ているが価値が違う ——
  **操縦者端末の切断は障害の一次情報**で、しかも切れた側の画面には切断バナーしか
  出ないので、いつ・どの端末が落ちたかは journal にしか残らない。件数も、
  操縦者 2 名 + Monitor の 3 台ぶんが接続・切断のときだけなので押し流さない
- **ファイルへのログ出力とローテーションは足していない。** service で動かす前提で、
  永続化・ローテーション・保持期間はどれも journald が既に持っている。二重に持つと
  「journal には出ているのにファイルが古い」形の食い違いを自分で作ることになる
  （会場での読み口を `journalctl -u cbc-control` 1 つに保つ、という
  `docs/venue_recovery.md` の前提とも噛み合わない）

### サーボ基板 3 枚 15 スロットの割当を確定させた（2026-09-08）

サブハンドの機構が固まり、**繋ぐものが 11 個**出てきた —— 角度を持つ機構が 5 つ
（回転の左右 2・ピッチの左右 2・オフセット 1）と、可動端を知りたい箇所が 4 つ
（`sub_y_axis` の前後端・`sub_lift` の上下端）。加えてメインハンドの `y_axis` に
左右 2 本のスイッチが要る。**15 スロットがちょうど埋まった**ので、割当は
「どこへ載せるか」ではなく「どの組み合わせなら成立するか」の問題になった。

現況（役割・ID・PC 側の名前）は仕様書 §2.2 / §7.1 が正で、ここには**なぜその形に
なったか**だけを書く。

#### 基板 #1 を全スロットセンサにした

スイッチは 6 本要るのに、基板 #0 の空きは SV4 の 1 枠しか無かった。**5 本を 1 枚に
まとめられるのは #1 しか無い**（#2 はサーボ 5 本で埋まる）。

**基板を 1 枚増やして「スイッチ 6 本を 1 枚に」揃える案は採らなかった。** 増えるのは
基板・電源・CAN の分岐と、DIP を回しすぎたときの事故の面積で、得られるのは
「y_axis の左右が同じ基板に載る」という配線上の見た目だけである。**左右のスイッチが
同じ基板に載っていることに機能的な意味は無い** —— 原点確定はグループ単位で行われ、
どちらの `FEEDBACK` も同じ `can_generic` を通って同じ受信ループに入る。

#### 基板 #1 が 2 つのロボット config にまたがる

SV0（`0x48`）だけが `config/main_hand.yaml` の `sensors:` で、SV1〜SV4 は
`config/sub_hand.yaml` にある。**これは異常な状態ではない** —— 基板は「どのロボットの
ものか」ではなく「どのバスに繋がっているか」でしか区切られておらず、`can_generic` は
最初から両ロボットが物理的に共有するバスである。DC 基板 #0 も既に `conveyor`
（メインハンド）と `pump_vac` / `pump_blow`（サブハンド）を分け合っている。

**ただし配線は物理的にメインハンドから基板 #1 まで引く必要がある。** ここだけは CAN の
論理では吸収できない差なので、`config/main_hand.yaml` と `firmware/servo/include/config.h`
の両方にその旨を書いた。引き回しを嫌って #0 へ寄せる逃げ道は無い（#0 は 5 スロットとも
埋まっている）。

#### `SensorConfig` に `expected_firmware` を足した

基板 #1 は**駆動するモータが 1 台も無い**基板になった。§3.4 の版番号照合は
`expected_firmware` を書いた対象としか突き合わせないので、`motors:` 側だけを見る実装の
ままでは**この基板の照合対象がゼロ**になる。旧ファーム（そのスロットを `Unused` として
宣言している版）のまま焼き忘れると、症状は「スイッチを押してもセンサ入力ビットが
立たない」だけで、**配線不良と区別が付かない**。

穴そのものは「`INFO` を一度も受けていない基板があると焼き忘れ検出が黙って無効になる穴を
閉じた（2026-09-06）」の節で既に予告していた（ファームはセンサスロットも `INFO` を送るが、
PC 側に照合対象が無い、と書いてある）。**予告のまま置ける状態ではなくなったので閉じた** ——
それまでは「センサの載る基板には必ずモータも載っている」ことに暗黙に寄りかかっており、
今回その前提が崩れた。

- `expected_angle_range_deg` は**センサには書けない**。サーボの型はセンサに存在しない
  概念で、書けるようにすると「照合が通っているのに意味の無い値」が config に生える
- 配信フィールド名（`safety.firmware_unconfirmed_motors`）は**変えていない**。UI の
  受信条件と WS 契約を動かす理由が無く、並ぶのはどちらも「焼き忘れ検出が働いていない
  デバイス」で意味も同じである
- **同じ基板にモータが載っていても書く**（基板 #0 の 2 本のセンサにも書いた）。
  「隣のモータが見ているから省いてよい」は「そのモータを config から消したら照合も
  黙って消える」という**見えない依存**になる

#### サーボの左右ペアには `sync_tolerance` / `sync_kp` を書かない

`sub_rotate` / `sub_pitch` は `y_axis`（M3508）や `rotate`（EDULITE）と同じ左右直結
ペアだが、**3 重の保護のうち②偏差監視だけが原理的に働かない。**

サーボの `FEEDBACK` が返すのは**クランプ後の指令角のエコー**であって実位置ではない
（仕様書 §7.4 の位置の欄が「補間中の**指令角**。実測ではない」と書いているとおりで、
基板は出力軸の角度を測る手段を持たない。到達フラグが推定でしかないのと同じ根である ——
§7.3）。
偏差監視へ載せても見えるのは「PC が送った指令の差」で、それは PC 自身が同じフレームで
作った値なので**常に 0 に近い** —— 片側が固着してもホーンが外れてもトルク不足で止まって
も、差は動かない。

**効かない保護を書くほうが危険である。** `sync_tolerance` が書いてあれば、次に読む人は
そこを「守られている層」として数える。書かずに理由を残せば、左右のずれに気付ける手段が
**目視だけ**であることが config から読める。①（同じフレームで同時に指令）と
③（フィードバック途絶をペア単位で判定）は `motors:` に束ねるだけで効くので、
そちらは従来どおり効いている。

**サーボ側で偏差を測れるようにする案**（ポテンショメータを足す・電流を読む）は、
どちらも基板が観測手段を持たないので**プロトコルから作り直しになる**（仕様書 §8 の
「速度・電流・温度はどの基板も測る手段を持たない」。`FEEDBACK` の空きは状態フラグの
bit6-7 だけである）。機構が付く前にそこへ投資する判断材料が無い。

#### サーボの逆回転ペアは `scale` の符号だけでは表せない

`rotate`（EDULITE）は連続回転なので `scale: -1.0` だけで逆回転を表せるが、**サーボは
0〜270deg の絶対角**である。符号を反転しただけでは逆回転側の指令が負の角度になり、
ファームが 0deg でクランプして動かない。**症状は「片側だけ動かない」だけで、
`SET_TARGET` には書いた値が載っているので `candump` からも読めない**（そして直結した
機構は片側だけ動かされる = その場で壊れる側の症状である）。

そのため逆回転側は `scale: -1.0` と `offset: 270.0` を対で書く。**`offset` は仮値で、
実際の折り返し点は左右のサーボをどの向きに取り付けるかで決まる**ので機構が付いてから
実測する（`docs/mechanism_handoff.md` §2）。**`sub_rotate` の実測値を `sub_pitch` へ
写してはならない** —— 取り付けの向きは軸ごとに違う。

ファーム側の可動範囲定数をスロットごとに分けてあるのはこの帰結でもある。同じ論理位置
でも左右へ届く角度が違うので、片側の実測で相方のクランプを動かすと**測っていない側が
機構に当たる**。

#### 新しい 3 軸は動作確認にだけ載せ、試合手順には入れない

`sequences/motor_check.py` は運用で使う位置名へ動かす確認なので、軸が増えれば
そのぶん確認も増える。一方 `sequences/sub_hand.py`（試合手順）には入れていない。

**位置定数が仮値（`home` 0.0 / `working` 90.0）だからである。** 試合手順は「どの順で
どこへ動かすか」を決める文書で、姿勢が決まっていない軸をそこへ置くと、**動く順序だけが
先に固まって機構の都合と食い違う**。動作確認は「指令が届いて動くか」しか問わないので、
仮値のままでも意味のある確認になる（むしろ機構が付く前に配線とファームを詰めるための
唯一の経路である）。

試合手順へ組み込むのは、機構が付いて姿勢が決まってからでよい —— そのとき足すのは
`move_to` の行だけで、モータ定義も位置定数の構造も動かない。

#### `sub_gripper` の廃止 —— サブハンドの把持は吸着へ一本化された

基板 #1 の SV0 を空けるために `sub_gripper` を移す先を探したが、**移す必要そのものが
無くなった** —— サブハンドの把持は電磁弁 6 個（`valve_1`〜`valve_6`）とポンプ
（`pump_vac` / `pump_blow`）による吸着で行う設計に固まり、機械的なグリッパが機構から
消えた。モータ定義・位置定数・シーケンス・指差喚呼から一式を削除してある。

**「使わないが定義だけ残す」を選ばなかった。** 残すと ①動作確認がそのステップを毎回
駆動し（存在しないサーボへ指令を送り続ける）②`sensors:` へ回したいスロットが埋まった
ままになり ③次に読む人が「サブハンドには 2 通りの把持手段がある」と読む。
把持が吸着 1 本になったことは、`sub_lift` の自重落下（`sub_lift_holds`）や
弁の `settle_s` が**唯一の把持経路の信頼性**になったという意味でもあり、
そこを曖昧にしたままにはできない。


### `y_axis` の零点確定 —— 左右 2 本のリミットスイッチと整列段（2026-09-08）

サーボ基板 3 枚の割当が固まり、`y_axis` の左右に原点リミットスイッチが 1 本ずつ
付いた（右 = 基板 #0 SV4 `0x44` / 左 = 基板 #1 SV0 `0x48`）。この軸は**左右 2 台の
M3508 が機構的に直結した逆回転ペア**なので、スイッチが 2 本あることは
「予備がある」ではなく**原点の決め方そのものが変わる**ことを意味した。

#### なぜ「両方が押された姿勢」を原点にするのか

機構には遊びがあるので、左右のわずかなずれは物理的に存在する。片方が当たった瞬間の
姿勢をそのまま原点にすると、**その遊びぶんが原点のずれとして焼き付く。** しかも遊びの
向きは前回どちら向きに動かしたかで変わるので、症状は「原点合わせをしたのに毎回少しずつ
違う場所が原点になる」という**再現しない**形になる。これは 1 本のスイッチでは原理的に
消せない —— 2 本目が「もう片方も当たった」を教えてくれて初めて姿勢が一意に決まる。

そこで探索の後に**整列段**を挟んだ。探索は従来どおり軸単位で動かして
**いずれか 1 本**がラッチしたら止まり、整列段が**まだラッチしていないセンサのモータ
だけ**を `direction` へ `step` ずつ進めて、全センサがラッチしたら原点を確定する。

#### `sensor`（単数）を `sensors`（モータ名 → センサ名）にした

**整列段は指令先を選ぶので、センサとモータの結び付きが要る。** センサ名の配列
（`sensors: [a, b]`）では「どちらがどのモータのものか」が書けず、整列段は
**押されている側を進める**ことになる —— 押されていない側のスイッチには永久に届かず、
`align_distance` いっぱいまで動いてから失敗する。config からは正誤が読めないので、
対応表として書かせるほかない。

`sensor`（単数）と `sensors`（複数）は**排他でどちらか一方が必須**にした。両方書けると
「どちらのスイッチが原点を決めるか」が決まらない。読み手が場合分けしなくて済むよう、
両者を吸収する口は `HomingSpec.sensor_names` **1 つだけ**にしてある —— 単数形を直接読む
経路を残すと、複数形の軸だけ事前確認（センサ鮮度）が `None` として素通りする形の穴が
作れる（実際に `tests/test_robot_sequences.py` の登録照合がその形だった）。

キーが軸の `motors:` と**過不足なく一致**していることは `AxisSpec` が起動時に強制する。
1 つ書き忘れるとそのモータだけが整列段の対象から静かに外れ、片側が押されないまま
原点が確定する —— つまり**この機構を入れた意味がそのまま消える**。

#### 「ペア軸に片側だけ効く操作を作らない」を破っていない

整列段は片側だけを進めるので、一見この原則の例外に見える。**例外ではない。**
指令は毎ステップ `AxisHandle.set_target_value` の**1 回**で、進めない側にも必ず
「検出時点の実測位置」を保持値として載せる。左右が別々の時刻に指令を受ける経路は
どこにも作っていない。

載せ忘れも構造的に塞いだ —— モータごとに違う軸位置を指令値へ換算する唯一の口を
`AxisSpec.to_commands_each()` にし、キーが軸のモータと過不足なく一致しなければ
`KeyError` で拒否する。**モータ 1 台だけを指令する API は最後まで作らなかった。**

#### 保持側は固定保持。毎歩実測へ再アンカーしない

探索段は 1 歩ごとに実測へ再アンカーする（指令が実位置を追い越して先行しないため）。
整列段の保持側は**意図的にそうしない。**

理由は**遊びが無い機構**にある。片側駆動が物理的に成立しない機構では、進めた側が
保持側を引きずる。保持側の目標を毎歩実測へ張り直すと、目標が引きずられた先へ追従して
しまうので、**左右のずれが増えない** —— `align_distance` にも停滞判定にも掛からないまま
**軸ごと機構端まで走る。歯止めが 7 つあるのに 1 つも発火しない**という、最も悪い壊れ方に
なる。固定保持なら引きずりは「進めた側が進まない」形で現れ、停滞判定
（`_STALL_LIMIT` = 3 歩）が拾う。**これが「この機構では片側駆動が成立しない」を
検出できる唯一の形である。**

#### 同期補正の前提は 2 つあり、1 つが欠けていた

`sync_kp` の補正が意味を持つ前提は ①全員が位置制御中 ②**全員が同じ軸位置を目標に
している** の 2 つで、実装は①しか見ていなかった。整列段は②を意図的に崩す段なので、
そのままでは補正が整列を打ち消す —— `y_axis` の実測値ではずれ 1mm で片側 −440counts・
相方 +440counts になり、**進めたい側は押し戻され、保持側は前へ引きずられる。**

`SyncGroup.targets_share_axis_value()` と `SyncGuard.skewed_groups()` を足し、
200Hz ループの補正 skip へ union した。許容幅 `_TARGET_ALIGN_EPSILON` は **1e-6** で、
丸め誤差だけを吸収する —— **`tolerance` を流用してはならない**（あちらは「到達したか」の
基準で、そこまで緩めると本物の追従差まで「同じ目標」と読む）。

**採らなかった選択肢が 2 つある。**

- **contextmanager による suspend/resume。** 整列段の前後で明示的に止めて戻す形は
  読みやすいが、**再開の取りこぼしがそのグループの補正を試合中ずっと殺す** ——
  しかも画面にもログにも出ない（補正は「移動中のピーク」にしか現れず、WS 配信の
  20Hz 瞬時値では読めない）。状態から毎周期導出する形なら、取りこぼしという概念が
  そもそも存在しない
- **補正を無視して整列を強行する。** 上の実測値（−440counts/mm）では成立しない。
  `sync_limit` 1250counts に対して 1mm のずれで 3 分の 1 以上を使い切るので、
  整列に必要な数 mm は補正だけで飽和する

**止めるのは補正だけで、保護（偏差ラッチ・途絶判定）は生きたままである。**
整列段は左右を意図的にずらす段なので、ずれの上限を見る側まで止めたら
`align_distance` を置いた意味が無い。

#### `align_distance` を専用キーにし、`< sync_tolerance` を強制した

整列段は「押されていない側をいつまでも進める」形の失敗をするので、片側だけを動かす
操作にも無人の歯止めが要る。**`search_distance` を流用してはならない** —— `y_axis` の
それは 650mm で、**片側を 650mm 進めるのは軸をねじり切ることに等しい。** 左右のずれは
機構の遊びの範囲（数 mm）にしかありえないので、桁の違う別のキーになる。

上限は `sync_tolerance` より**真に小さい**ことを `AxisSpec` が起動時に強制する。
届くと**零点確定の最中に 3 層の保護**（位置制御ループ 200Hz / `SyncMonitor` 50Hz /
`move_to` 完了時）が発報し、機体はラッチしたまま止まる —— 操縦者からは
**「動作確認の最初のステップでいつも緊急停止する」**としか見えず、原因が config から
読めない。だから config の側で先に拒否する。同じ理由で、`sensors` を書いた軸には
`align_distance` を必須にし、書いていない軸には書けなくした（整列段が存在しないので、
書いても効かない設定になる）。

歯止めは `search_distance` / 再アンカー / 事前確認 4 つ / 停滞判定 / 離脱の歩数上限 /
**`align_distance`** / 緊急停止 の **7 つ**になった。

#### 複数センサの接触は 1 回の観測でまとめて読む

当時の `GenericDriver.consume_sensor_latch` は**読むと消える**ので、センサごとに別々の
タイミングで読むと「A を読んだ瞬間に B の接触が捨てられる」。どちらが先に当たったかは
探索から整列段へ持ち越す必要があるため、1 回の観測で全センサを読み、走行中は OR で
溜め込む形にした。

**この「1 回の観測でまとめて読む」は今も現行だが、根拠は変わった。** 2026-09-09 に
ラッチを単調増加のカウンタ（`sensor_contact_count`）へ作り替えたので、**読んで消える
ことによる取りこぼしはもう無い**（現在の実体は `_SensorContacts` で、基準値からの差を
数える）。それでもまとめて読むのは、どちらが先に当たったかを整列段へ持ち越すという
本来の目的が残っているためである。

#### 実機ではまだ 1 度も走らせていない

config は有効化したが、**この軸の零点確定は実機で 1 度も動かしていない。**
`direction` −1（設計意図から置いた）/ 左右 2 本の極性（`sensorActiveLow`）/
`align_distance` 5.0mm（`sync_tolerance` の半分に置いた暫定値）/ `step` 0.5mm
（`rotate` は 0.5deg で 1 歩も動かなかった実績がある）は**どれも未実測**である。
...
**探索時間はセッティングタイムから引かれる。** 650.0 / 0.5 = **1300 ステップ**で、
ストローク全長を走ると最短 65 秒・最悪 325 秒。ただし実際に掛かるのは「そのとき原点から
どれだけ離れているか」で決まる（`search_distance` は上限であって毎回走る距離ではない）
ので、**動作確認の前に手動で原点付近へ寄せておく**運用にする。縮めたいときに効くのは
`step` だけで、`search_distance` は唯一の無人の歯止めなので速さのために縮めてはならない。


### リミットスイッチ保護 —— 触れたらその軸をその向きへだけ止める（2026-09-09）

スイッチが 7 本揃った時点で、それらは零点確定にしか使われていなかった。うち
`homing:` から参照しているのは 3 本（`rotate` に 1 本、`y_axis` に左右 1 本ずつ）で、
サブハンドの 4 本は `sensors:` に登録して途絶を見ているだけ ——「触れた」ことは
100Hz で PC まで届いているのに、**触れても何も止まらなかった。**

守りたかったのは「行き過ぎ」の 3 経路で、どれも機構を壊す:

- **原点がずれたまま走り込む**（電源投入位置を原点にしていた頃の主要な失敗）
- **手動ジョグの押し込み**（操縦者が端へ寄せていく操作そのもの）
- **位置定数の誤り**（`positions` の値が可動域の外にある）

#### ファーム側には置けない

自作モタドラのファームは自分のスロットのセンサを読めるので、一見「基板の中で閉じる」の
が筋に見える。**閉じない。** 7 本のスイッチは全て `can_generic` のサーボ基板に載るのに、
止めたいモータは 1 台もその基板に無い（`y_axis` = `can_m3508` の M3508、`rotate` =
`can_edulite` の EDULITE 05、`sub_*` = `can_dm3520` の DM3520）。**バスもドライバも別物**
なので、センサから指令までの経路は必ず PC を往復する。行き過ぎの見積もりが要るのは
ここが理由で、判定は 50Hz の常駐タスクにしかならない。

#### 捨てた案

| 案 | 捨てた理由 |
|---|---|
| **無条件停止**（触れたらその軸を止める） | 端に触れた軸は**離れる向きへも動かせなくなる**。手で押し戻す以外に戻す手段が無く、試合中なら詰む。方向を持たせれば「機構側だけを塞ぎ、退避は必ず通す」が成立する |
| **全体緊急停止へ倒す**（`SyncMonitor` と同じ扱い） | 緊急停止中は `manual_*` が通らない（`CommandSpec.allowed_during_e_stop`）ので、**端から退避する操作そのものが消える**。`SyncMonitor` が全体停止でよいのは「ずれた軸はどちらへ動かしても壊れる」からで、リミットは片方向だけが破壊。**性質が違うものを同じ扱いにすると、片方の正しさがもう片方の復帰不能になる** |
| **電流 0 で止める**（`M3508PositionLoop.clear_target` を呼ぶ） | `sub_lift`（DM3520）は保持ブレーキを持たず、**消磁するとワークごと自重で落ちる**（`checklist.yaml` の `sub_lift_holds` はこの前提を人が確認する項目である）。「止める = 力を抜く」は 200Hz の偏差ラッチの語彙であって、ここでは使えない。**止める = 目標値を実測位置へ引き戻すこと**にした |
| **軸まるごとの一時停止**（`SyncMonitor.suspend_group` と同型で、零点確定のあいだ軸単位で外す） | 両端にスイッチのある軸（`sub_y_axis` / `sub_lift`）で**反対端の保護まで消える**。探索が空振りしたとき、機構を守るものが 1 つも無くなる。外すのはセンサ単位（`suspend_sensors(homing.sensor_names)`）にした |
| **終了時のラッチで移動の成否を見る**（`move_to` の最後に `latched` を見る） | 移動中に触れて離れた接点のバウンドを 1 件も拾えない。**ラッチは外れるのに、層①が書き戻した目標は残る**ので軸は触れた位置で止まったままになり、元の目標を再送する経路は無い。単調増加の介入カウンタを前後で比べる形にした |
| **接触を consume 方式のラッチのまま読む** | 読み手が 1 人に限られる。保護が読んだ瞬間に零点確定が接触を取りこぼし、**「当たっているのに止まらず端を越えて回り続ける」**（2026-09-05 に実機で起きた壊れ方）が再発する。読んでも減らない単調カウンタ（`GenericDriver.sensor_contact_count`）へ作り替えた |

#### 2 層あって、判定だけを共有する

`lib/control/limit_guard.py` の `LimitGuard.clamp()` が単一情報源で、2 つの層が呼ぶ。

- **層①（50Hz の常駐）** —— ラッチの立ち上がりで、その軸の目標を実測位置へ**1 度だけ**
  書き戻す。**これだけだと、押し戻されるまで 20ms 進む。**
- **層②（指令の入口）** —— `AxisHandle` に差し込み、シーケンス・手動・零点確定の
  すべての指令を端でクランプする。**これだけだと、走行中に触れても止まらない**
  （指令は既に飛んでおり、次の指令が来るまで誰も止めない）。

`lib/axis_sync.py` の `SyncGroup.violation()` を 3 層が共有しているのと同じ構成である。
判定を 2 箇所に書くと、片方だけ符号を落としてももう片方が拾ってしまい、**壊れた層が
壊れたまま残る。**

**書き戻しは立ち上がりで 1 度だけ**にした。毎周期実測を測り直すと、負荷で下がったぶんへ
目標が追従して**誰も操作していないのに軸がクリープする**（`QueryDrivenTargetRefresher` が
`idle_target_value()` をラッチするのと同じ理由）。さらに、逆方向へ退避しようとした指令を
保護が毎周期上書きし続けることになり、**端から出られない軸**という捨てたはずの案へ戻る。

**目標を 1 つも持たない軸には書かない。** 誰も駆動していないなら止めるものが無く、
書けば「操作していないのに保持が始まる」。ラッチ自体は立つので、以後その向きの指令は
層②が止める。

#### ラッチ条件は「増えた or 今 ON」、解除は「今 OFF and この周期で増えていない」

カウンタ化で失われたものが 1 つある —— 旧 `consume_sensor_latch` は現在値を OR していた。
零点確定は基準値を取る時点で必ず OFF なので等価だが、**`LimitGuard` はその前提を持たない**
（触れっぱなしでは立ち上がりが 1 回しか立たない）。したがって両方を見る:

- **回数の増加だけ**では、触れっぱなしを拾えない
- **現在値だけ**では、ON 区間が観測周期（20ms）より狭い接触を取りこぼす ——
  2026-09-05 に実機で起きた「当たっているのに止まらない」壊れ方そのもの

解除で「この周期で増えていない」を要求するのは、その 20ms のあいだに入って出た
（＝また触れた）場合に外さないためである。

#### センサ途絶では発動させない。代わりに「効いていない」ことを配る

途絶で止めると、**スイッチ 1 本の不調で試合中に軸が動かなくなる。** かといって黙って
無効にすれば「守っているつもり」の機体ができる。`firmware_unconfirmed_motors` と同じ
「壊れているのではなく検出が働いていない」の報告として `safety.limit_blind_sensors` を
配り、FAULT にも `evaluateHealth` の異常判定にも倒さない。**一度ラッチした後は途絶でも
落とさない** —— 押し当てたまま保護が消えるほうが危険である。

配信は 2 つだけにした。`limit_latched`（軸名 → センサ名。**センサ名まで配る** ——
どちらの端かはセンサ名にしか無く、両端にスイッチのある軸では軸名だけで退避の向きを
選べない）と `limit_blind_sensors`。**介入回数は配らない** —— 手動で端へ寄せるたびに
増えるので、シーケンス由来と手動由来が混ざった数になり操縦者には読めない。

#### 保護が介入した移動を「到達」と読ませない

層①が目標を実測へ書き戻すと、`MotorHandle.is_reached`（送った目標との比較）は**必ず
成立する。** 放置すると軸は途中に居るのにシーケンスだけが次へ進む —— 症状は「動作確認は
通ったのに、その軸だけ位置定数と違う場所にいる」で、どのログにも出ない。

`LimitGuard.intervention()` が「要求を曲げた回数」と直近のセンサ名を単調に数え、
`move_to` が移動の前後で比べる。**層①と層②のどちらが働いても同じカウンタが進む** ——
どちらも「保護が要求を曲げた」瞬間であり、分けると入口でクランプされた場合しか拾えない。
**書き戻せた周期だけ数える**（緊急停止中で送れなかった周期を数えると、機構が要求どおり
動き切った移動まで失敗する）。失敗の文言にはセンサ名を添える —— 「到達しませんでした」
だけでは、保護と配線不良で手当てが正反対になる。

#### 行き過ぎの見積もり（`y_axis`）

| 内訳 | 値 |
|---|---|
| 検出まで（センサ `FEEDBACK` 10ms + 監視周期 20ms + 送信）最悪 35ms × 巡航 200mm/s | 7.0mm |
| 減速 `v²/(2a)` = 200²/(2×1200) | 16.7mm |
| **合計** | **約 24mm** |

**スイッチは機構端から 25mm 相当以上手前に置く前提**であり、**守れるのは低速接近と
「原点がずれたまま走り込む」異常であって、全速での突入ではない。** この数字を docs に
書き残したのは、機構側の取り付け位置がこの前提を満たしているかを誰も確かめていない
ためである（棚卸しは `docs/mechanism_handoff.md` §1 / §2）。

#### `direction` の取り違えは保護が無いより悪い

逆向きだと「端に触れたのに、そこから離れる向きへ動かせない」かつ「機構側へは自由に
進める」になる。同じセンサを指す `homing.direction` との一致は `AxisSpec` が読み込み時に
突き合わせるが、**`homing` を持たないサブハンドの 4 本には突き合わせる相手が無い。**
機械が確かめられるのはメインハンドの 3 本までで、残りは指差喚呼 `limit_direction_sub` が
唯一の検出経路になる（`limit_direction_main` と 2 項目に分けたのは、触りに行く場所が
機体ごとに離れているため。1 項目にすると片方の機体だけ触って唱えたことに気付けない）。

**この突き合わせは「起動を拒否」しない。** `main._load_position_table_file` は位置定数の
読み込み失敗を `logger.error(… — 定数なしで起動)` に倒して空表で先へ進むので、実際に
起きるのは**そのロボットの位置定数がまるごと消え、軸 0 本で立ち上がる**ことである
（保護もシーケンスも零点確定も同時に無くなる）。この挙動そのものは意図した設計だが
（定数が壊れているときに推測で動かすより、値を引いた時点で明示的に失敗させるほうが
安全）、**「拒否されるから安全」と読むと、黙って保護の無い機体が立ち上がる。**
手掛かりは起動ログの `ERROR` 1 行と、動作確認がそのロボットのステップを「構成に無い軸」
として全部除外することの 2 つしかない。落とし穴として
`docs/checks_and_health.md` の「変更するときの落とし穴」へ残した。

#### 変異テストの対応

「変異テスト」節の表へ 3 行を足した（層ごとに 1 枚ずつ壊す形は下表のとおり）。

| 入れた不具合 | 落ちるテスト |
|---|---|
| 層①の引き戻しを消す | `test_limit_guard.py::TestStopsOnContact::test_触れたら目標が実測位置へ引き戻される` |
| 毎周期書き直す（立ち上がり判定を落とす） | `::TestHoldIsWrittenOnce`（`test_毎周期書き直さない` / `test_実測が動いても目標を追わない` / `test_逆方向へ退避する指令を上書きしない`） |
| `clear_target` で止める | `::TestStopsOnContact::test_電流0にも無励磁にもしない` |
| 層②のクランプを外す | `test_limit_clamp.py::TestBlockedDirection::test_禁止方向の指令は端で頭打ちになる` |
| `clamp` の向き判定の符号を落とす | `::TestBlockedDirection::test_逆方向の指令はそのまま通る` / `test_limit_guard.py::TestClamp::test_逆方向の指令は通る` |
| ラッチ条件から現在値の OR を落とす | `test_limit_guard.py::TestLatchCondition::test_触れっぱなしでもラッチが続く` |
| ラッチ条件から回数の増加を落とす | `::TestLatchCondition::test_観測の隙間を通り抜けた接触でもラッチする` |
| 解除条件から「この周期で増えていない」を落とす | `::TestLatchCondition::test_同じ周期で入って出たら解除しない` |
| 途絶センサでも判定する | `::TestStaleSensor::test_途絶中は止めない` / `test_途絶してもラッチは落とさない` |
| 途絶を `blind_sensors` に載せない | `::TestStaleSensor::test_途絶センサは保護が効いていないものとして見える` |
| `suspend_sensors` を無効化する | `::TestSuspendSensors::test_一時停止中は発動しない` / `test_homing.py::TestSuspendsTheLimitGuard` |
| 一時停止を軸まるごとに広げる | `::TestSuspendSensors::test_止めたセンサ以外の保護は残る` |
| 一時停止に入るときラッチを落とさない | `::TestSuspendSensors::test_入る時点のラッチも落とす` |
| 一時停止中に基準値を進めない | `::TestSuspendSensors::test_止めている間の接触をまとめて拾わない` |
| 介入回数を `move_to` が見ない | `test_limit_intervention.py::TestTouchedWhileMoving::test_移動の途中で触れた移動は失敗する` |
| 終了時のラッチで成否を見る | `::TestTouchedWhileMoving::test_触れて離れた移動も失敗する` |
| 介入していない移動まで失敗させる | `::TestNoIntervention`（`test_保護が介入しない移動は成功する` / `test_触れていても逆方向の移動は成功する`） |
| 配信から `limit_latched` / `limit_blind_sensors` を落とす | `test_server_limit_state.py`（`TestLatchedAxesAreBroadcast` / `TestBlindSensorsAreBroadcast`） |
| 緊急停止の解除でラッチを落とす | `test_server_limit_state.py::TestEStopReleaseDoesNotResetLatch` |
