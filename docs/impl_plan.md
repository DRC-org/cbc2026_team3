...
#### リミットスイッチによる零点確定（2026-09-08 に `sub_y_axis` で開通）

`Dm3520Driver` が `deactivation_steps()` / `origin_capture_steps()` を宣言するように
なったので、`main.py` の `_make_origin_resolver()` が探す 2 つの手段のうち「ドライバへの
`SET_ZERO`」に本機も乗る。**`sub_y_axis` は前端スイッチ（`0x49`）を原点として
`homing:` を有効化済み**で、座標は前端が 0・収納側が負。`rotate`（EDULITE 05 ペア）も
同じ経路。

**長く宣言していなかった理由は消えた。** 「`SET_ZERO` の安全な順序が `disable` を
要求するが、`sub_lift` は disable すると自重で落ちる」と見ていたが、2026-09-08 に
実機で落ちないことが分かった（保持は減速比 19.2 のギヤ頼みで保持ブレーキは無いが、
それで止まる）。**根拠は指差喚呼 `sub_lift_holds` の実機確認なので、あれが通らなく
なったら宣言ごと見直すこと。**

**`sub_lift` はまだ `homing:` を書いていない。** 手段もスイッチ（`0x4B` / `0x4C`）も
あるが、`direction` / `search_distance` / `step` が未実測なだけである。埋まれば
前後軸と同じ経路で動く。それまでこの軸の原点は**電源投入位置**（本機は投入時に位置が
0.0rad へ固定される）で、搬送中に手で動かしたぶんはそのまま座標のずれになる。
...
| 零点確定の手段が無い軸の ERROR（`零点確定: 軸 … は原点を確定する手段がありません`） | 押した瞬間に失敗する構成のまま試合当日を迎えないための行。**手段を持たないドライバの軸に `homing:` を書くとここで落ちる**ので、この 1 行が無いと「動作確認だけがいつも失敗する」理由がどこからも読めない（かつては `sub_y_axis` / `sub_lift` がこれに当たった。2026-09-08 に DM3520 が `SET_ZERO` を宣言して外れている） |
...
> **零点確定が有効なのは `rotate`（EDULITE）と `sub_y_axis`（DM3520）の 2 本**
> （`y_axis` は極性が、`sub_lift` は探索パラメータが未実測。下表の行）は
...
| 零点確定が有効なのは `rotate` と `sub_y_axis` の 2 本 | `main.py` の `_make_origin_resolver` は「PC 側位置制御ループ（M3508）」と「ドライバへの `SET_ZERO`」の 2 つを順に探し、可否はドライバ自身の `supports_origin_capture()` が答える（`deactivation_steps()` と `origin_capture_steps()` の両方を持つドライバだけが宣言できる）。EDULITE 05 は宣言するので `rotate` の `homing:` は**有効**（センサはサーボ基板 #0 の SV3 = `0x43`、実機で接触が読めることを確認済み） | **DM3520 も 2026-09-08 から宣言する** —— それまでは「`SET_ZERO` の安全な順序（`disable → set_zero`）が `sub_lift` の自重落下と両立しない」として宣言していなかったが、実機で落ちないことが分かって前提が消えた（根拠は指差喚呼 `sub_lift_holds`。あれが通らなくなったら宣言ごと見直す）。`sub_y_axis` は**前端スイッチ（サーボ基板 #1 SV1 = `0x49`）を原点として有効化済み**で、座標は前端が 0・収納側が負。**`sub_lift` は手段があるのに `homing:` を書いていない** —— 上下端スイッチ（`0x4B` / `0x4C`）も揃っているが `direction` / `search_distance` / `step` が未実測なだけである。**`y_axis`（M3508）は確定手段があるのにスイッチが未装着**で、ファームの `kServoBoards[]` 基板 #0 SV4 は `Unused`、`sensors:` にも `homing:` にも書いていない（3 箇所は必ず同時に戻す。上の「零点確定を rotate へ移し、y_axis を一時無効化した」節）。原点が確定できない軸は**電源投入位置がそのまま原点**で、ずれは指差喚呼（`main_home_position` / `sub_dm3520_origin`）が人の目で埋める。**ただし `rotate` は放っておくと電源投入位置すら原点にならない** —— EDULITE 05 の原点はフラッシュの機械ゼロで、2 台のゼロが揃っていないと物理的にずれ 0 でも逆換算後に差として現れ（実機で 175.879deg）、起動直後に `SyncMonitor` が全体緊急停止を掛ける。`rotate_r` / `rotate_l` の `set_zero_on_start: true` がこれを消しており、**零点確定が有効になっても `false` へ戻してはならない**（戻すと機体が動かせず、動作確認そのものを開始できない）。**`direction` −1 は 2026-09-05 に実機で確定した。`step` は同日に下限が分かっている**（0.5deg では静止摩擦を超えられず `HomingError`）が、**現在値 1.0deg はその 2 倍で、この刻みでの通し確認はまだ**。**未実測は `search_distance` 180.0deg だけ** —— その日の零点確定はスイッチに触れた状態から始めたので、この歯止めが効く経路を通っていない。探索は現在 180 ステップ・最短 9 秒（最悪 45 秒） |
