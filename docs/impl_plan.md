...
| 受信ループはインタフェース断で降りない | `_receive_loop` の再試行を外して `raise` へ戻す | `test_can_manager.py::TestReceiveLoopSurvivesInterfaceDown::test_インタフェース断で降りず復帰後に受信を再開する` / `::test_受信断は降りずに必ず記録される` |
