...
その `step` は 2026-09-05 に実機で 2.0deg へ上げた（下の「`rotate` の零点確定を実機で
通した」節）ので、**現在は 180.0 / 2.0 = 90 ステップ・最短 4.5 秒**である。
...
消せるのは後者だけ。`rotate` では**最大 2step ≒ 4deg から最大 1step ≒ 2deg へ縮む**。
**2deg 程度の行き過ぎに耐える強度がある**という機構側の性質が前提で、そこを 0 にする
