...
（`invariants.md` §4）。**今より弱くなる箇所は 1 つも無い**が、次は残る。
...
## 10. 昇降の可動範囲 450mm は上端スイッチを実測するまでの暫定

`manual.min` −450 / `homing.search_distance` 470 / `guard.max_step` 500 は、上端スイッチが
未実測で「宣言で先に止まると実際の端が測れない」ため広げた値である。歯止めは上端スイッチと
`stall_torque` だけになり、**DM3520 が電源断で黙って原点を作り直す**（§ 会場カード 3-4b）ズレの
被害範囲も広がる。スイッチ間距離を実測したら 3 つとも実測 + 余裕へ絞り直すこと。

**上端スイッチは配線もスロットも未検証。** 初めて測るときは作動点測定（Monitor 側の
刻みを指定できるパネル）で `coarse_step` を小さくして 1 本だけ当てること —— 距離測定は
下端用に決めた `coarse_step` を両向きに使い回すので、上端の ON 区間がそれより狭いと跨ぎ切る。

## 11. 距離測定に残る、いまは踏めない 2 つ

どちらも現行の `config/sub_hand.yaml` / `sub_hand_positions.yaml` では露出しないが、
設定を変えると再燃する。

- **`measure()` は `home()` と違って整列段 (`_align`) を通さない。** 左右 2 本センサ持ちの軸
  (`homing.sensors`) を距離測定に載せると、接触は「どちらか一方が当たった時点」で止まるのに
  `engage` は軸全体の平均位置で記録され、**まだ届いていないモータを巻き込んだ値が黙って
  作動点として報告される**。サブハンドの 2 軸は `homing.sensor` 単数なので今は発火しない
- **`_probe_sensors` は反対側の端が未宣言だと元のセンサへ黙って落ちる。** 片側しか
  `guard.limits` を書いていない軸で反対向きを測ると、違う場所のセンサを見たまま走る。
  サブハンドの 2 軸は両側とも宣言済みなので今は踏めない
