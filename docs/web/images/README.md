...
| `*-manual-subhand.png` | サブハンド・手動操縦 | 電磁弁 6 + ポンプ 2 が画面外 → 8 個ともスクロールなしで押せる / 可動範囲バーにプリセットの刻み / `目標 1.00 on_off` → `目標 ON` |

**After 側は最新の実装で撮り直すこと。** Before は `main` に対して固定だが、After はこの PR が
進むたびに古くなる。**今どうなっているか**の一覧は `screens/`（全 15 画面）にあり、同じ画面は
そちらと一致していなければならない。
