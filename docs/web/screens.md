...
**`AlwaysManualPanel`（常時操作）は試合中・試合終了の半自動にだけ出る。** 半自動の制御権を
握ったまま操作できる軸（config が `manual_always: true` を宣言した `duty` / `on_off` の軸 ——
メインハンドの `conveyor`、サブハンドの `valve_1`〜`valve_6`）を並べる面で、対象が 1 本も無い
ロボットでは**パネルごと描かない**。

- **準備中には出さない。** そのフェーズではシーケンスが走っていない（`sequence_start` は
  試合中のみ）ので、従来どおりモード切替で手動へ入れば済む。加えて**動作確認の起動導線は
  Monitor の準備面（`MatchPrep`）にしかなく**、サーバーは動作確認の実行中この経路を拒否する
  —— 出さなければ「打音・目視の最中に、押せるが必ず拒否されるボタンが画面にある」状態を
  作らずに済む（`motor_check_start` のフェーズゲート自体は `finished` も許す。準備中に
  限っているのは導線のほうである）
- **手動モード中にも出さない。** `ManualPanel` が同じ軸を含む全軸を出すので、同じ事実が
  2 箇所に並ぶ
- 塞ぐ理由は手動指令と同じ `manualBlockedReason`（切断中 / 緊急停止中）。新しい判定は増やさない
...
| `AlwaysManualPanel` | 半自動のまま操作できる軸（`manual_always`）だけを並べる面。**試合中・試合終了の半自動にだけ出し**、対象が 0 本なら描かない。行は `ManualAxisRow` をそのまま使い、送るのは `manual_move` だけ（対象軸は `manual:` を持たないので連続値の口が無い）。選択は渡さない（キーボードで軸を渡り歩くのは連続軸だけの概念）。**シーケンスに上書きされることを 1 行で断る** —— 断らないと「押したのに戻った」が故障に見える |
