...
### 操縦画面（`RobotControl`）に固定の説明文を置かない

**いつ見ても同じ文章は、操縦画面には 1 行も置かない。** 操縦者は機体を見ていて、画面へ視線を
戻せるのは一瞬しかない。読まれない文が面の下端を占めると、その一瞬で拾うべきチップが下へ
押される。**状態を伝える警告は消さずに数語のチップへ畳む** ——「未選択 — 吸着ステップは
拒否されます」は「未選択」、「安全機構の配信を読めていません。…」はチップ
「安全機構 判定不能 …」だけ、というように、**文章は消してもシグナルは消さない**
（`docs/invariants.md` の「測れない値を 0 で埋めない」に触れる表示 —— `MALFORMED`・未受信・
未選択 —— はチップとして必ず残る）。
...
| 残すもの | 理由 |
|---|---|
| 確認モーダルの本文（`先頭から再開` / ステップジャンプ） | 押す前に止まって読む場所で、視線は既に画面にある |
| 値・単位・軸名・ステップ名 | 配信そのもので、文章ではない |
| `Kbd` のキー凡例（`↑↓ 軸`） | 記号 + 1 語で、既に文章ではない |

**復旧手順（`SafetyIssue.hint` と診断の註記行）は Monitor が引き受ける** ——
`SubsystemStatus` は `concise` を渡された画面でチップと対象名だけを描き、手順の全文は
`showVerdict={false}` の Dashboard と `RobotStatusRow` が今までどおり出す。手順を読むのは
機体から目を離せる人（Monitor 役）で、操縦者ではない。
