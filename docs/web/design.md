...
操縦者の開閉操作を**上書きして**開く。同じ部品でも役割で既定を変える（操縦者の試合中は畳み、
Monitor と準備フェーズは開く）。
...
離す・ブラウザがポインタ操作を取り消す・タブが切り替わってフォーカスを失う。1 つでも拾い
損ねると**指を離したのに機体が動き続ける**。
...
3. ヘッダー直下の帯の操作ボタンは**帯の先頭**に固定する（`ModeSwitch`）。現在モードのチップの
   隣に置くと、チップの文言長がモードで変わってボタンの横位置が動く
