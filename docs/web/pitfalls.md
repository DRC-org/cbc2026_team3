...
| リミット保護の 2 欄を `?? {}` / `?? []` で埋める | 軸が止まっているのに画面は平常。**操縦者に残る手掛かりは「その向きへ指令しても動かない」だけ** | `lib/protocol.test.ts` / `lib/healthVerdict.test.ts` |
| 0 件の通知を空の `<ul>` で描く | 中身の無い色付きの帯が平常時にずっと残り、「平常時に静か」が崩れる | `diagnostics/SubsystemStatus.test.tsx` |
...
### リミット保護の 2 欄が落ちても、画面には「操作の可否」として現れない

`manual_always` や `manual.axes[].positions` の欠落は、**押せるボタンが消える**という形で
操縦者の目に触れる。`safety.limit_latched` / `limit_blind_sensors` は違う —— **保護は軸ローカル
なので他の軸は平常どおり動き、止まっている軸も「その向きへ指令しても動かない」以外の症状を
出さない。** だから欠落を黙って埋めてはならず、`safetyShapeErrors` の検査対象に入れて
`MALFORMED` へ倒す（`limit_latched` は軸名 → センサ名配列の Record なので
`isStringArrayRecord`。検査の内訳は `docs/web/data_flow.md`）。

描き方の規則は 3 つで、`diagnostics/SubsystemStatus.test.tsx` が 1 つずつ固定している:

- **ラッチは `forcedOpen` に含める**（`limitLatchedAxes().length > 0`）。今まさに軸が止まって
  いて、逆向きへ退避するまで解けない。畳んだまま見逃させない
- **保護無効（`limitBlindSensors()`）は `forcedOpen` に含めない。** `firmware_unconfirmed_motors`
  と同じ「壊れている」ではなく「検出が働いていない」の報告で、試合中ずっと変わらない状態だから
  —— 開きっぱなしにすると安全機構のパネルごと読まれなくなる。判定チップも動かさない
  （ラッチ中でも `異常なし` のまま。保護が設計どおり働いた結果であって機体の故障ではない）
- **0 件では `<ul>` ごと描かない。** テストは `queryByRole("list", { name: "リミット到達の軸" })`
  / `{ name: "リミット保護が無効なセンサ" }` で**要素が 1 つも無いこと**を見る
  （`queryByText` だけだと、中身が空でも帯だけが残る実装を通してしまう）

文面まで固定してある —— ラッチ側は「逆向きへは動きます」（復帰できない軸だと読ませない）、
保護無効側は「触れても軸は止まりません」（状態だけでは次の一手を選べない）。**軸名だけでは
どちらの端かが分からない**ので、センサ名も必ず並べる。
...
出して拒否させるのは「押した → トーストが出た → なぜ？」を操縦者に往復させるだけで、その往復は
機体を見ているべき時間から取られる。`AlwaysManualPanel` を**準備中に出さない**のがこれ ——
**動作確認の起動導線は Monitor の準備面（`MatchPrep`）にしかなく**、サーバーは動作確認の実行中
この経路を拒否する（`lib/server.py` の `_allow_manual_in_sequence`）。動作確認は弁の作動音と
コンベアの回転を操縦者が目視・打音で確かめるので、それが起きている画面に必ず拒否されるボタンが
あってはならない。準備中はシーケンスも走っていないので、モード切替で手動へ入れば失うものも無い。
**手動モード中にも出さない**（`ManualPanel` が同じ軸を出すので同じ事実が 2 箇所に並ぶ）。
塞ぐ理由は既存の `manualBlockedReason` を使い、新しい判定を増やさない。
