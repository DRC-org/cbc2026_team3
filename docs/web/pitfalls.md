...
`web/` は**テストが本体より多い**（テストだけで 60 ファイル・1 万行超）。大半は「見た目の
...
読まれない。**その註記は現在 1 行も残っていない**（操縦画面から固定の説明文を全廃した。
規則は [`design.md`](design.md)）—— 説明を落としても成立するのは、役割がモードで 1 つに
寄っているからで、**2 つの面を同時に出す形へ戻すと、今度は説明する場所ごと無い。**
