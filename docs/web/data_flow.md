...
**`state.manual.axes[].manual_always` は真偽値。** 半自動シーケンス制御（`mode: "sequence"`）の
ままでも手動指令を受け付ける軸かを表し、正は位置定数 yaml（`axes.<軸>.manual_always`）だけが
持つ。宣言できるのは到達判定を持たない単独軸（`duty` / `on_off`）で、サーバーが通すのは
`manual_move` だけ（`manual_set` / `manual_jog` は対象軸でも拒否される）。緊急停止中と
動作確認の実行中も拒否される。**UI 側で軸名から導出し直してはならない。**
...
（`AlwaysManualPanel`）。欄が落ちた配信では対象軸が 1 本も出ず、押せるボタンが 1 つも
増えない安全側へ倒れる —— **`?? true` のような向きの既定値を置いてはならない**。
`MALFORMED` へ倒さないのは、欠落が**そのまま操作の可否として画面に出る**（パネルごと消える）
ため。`safety` の 1 欄が落ちて全画面が白くなる類とは別で、`positions` の 1 要素を落とす扱いと
同じ切り分けにある。この扱いを新しい欄の既定にしてはならない。
