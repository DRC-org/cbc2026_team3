...
`match_state.court` が `null` なら、操縦者画面は手動とシーケンスの操作をチップ「コート未設定」で
閉じる（`RobotControl`）。**サーバーの理由文（`lib/commands.py`）を書き写さない** —— 操縦画面は
文章を持たないので語数が合わず、写した瞬間に片方だけ古くなる。全文は拒否トーストが出す。**受信は `=== true` で厳密に見る** —— 欠落を真へ倒すと、古いサーバーに
繋いだだけで操作が消える。

**`state.suction` は 3 値を区別する。** `null` は「このロボットに吸着パッドが無い」
（パネルを描かない）、欠落（`undefined`）は「古いサーバー」（同じく描かない）、`pads[]` の
どれかが `axis` / `label` / `enabled` を欠けば `MALFORMED`（操作を出さず、チップ「パッド状態 判定不能」だけを出す）。
